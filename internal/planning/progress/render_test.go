	...
	p := &Progress{Phases: map[string]Phase{
		"1": {Name: "Phase 1", Subphases: map[string]Subphase{
			"1.A": {Name: "Alpha", Items: []Item{{
				...
				Fixture:              "webpages/docs/content/building-gormes",
...
func TestRenderAgentQueueIncludesExecutionCardAndSkipsBlockedUmbrella(t *testing.T) {
			...
			"4.A": {Name: "Providers", Items: []Item{
				{
					...
					SourceRefs:     []string{"webpages/docs/content/upstream-hermes/source-study.md"},
					ReadyWhen:      []string{"fixtures replay"},
					NotReadyWhen:   []string{"live provider call required"},
					Acceptance:     []string{"go test ./internal/llm passes"},
					WriteScope:     []string{"internal/llm"},
					TestCommands:   []string{"go test ./internal/llm"},
	...
	for _, want := range []string{
		...
		"- Write scope: `internal/llm`",
		"- Test commands: `go test ./internal/llm`",
		"- Fixture: `internal/llm/testdata/provider_transcripts`",
...
func TestRenderBuilderLoopHandoff(t *testing.T) {
	p := &Progress{Meta: Meta{BuilderLoop: BuilderLoopMeta{
		...
		AgentQueue:      "webpages/docs/content/building-gormes/builder-loop/agent-queue.md",
		ProgressSchema:  "webpages/docs/content/building-gormes/builder-loop/progress-schema.md",
		CandidateSource: "webpages/docs/content/building-gormes/architecture_plan/progress.json",
...
func TestRenderProgressSchema(t *testing.T) {
	...
	for _, want := range []string{
		...
		"`webpages/docs/content/building-gormes/builder-loop/builder-loop-handoff.md`",
		"`webpages/docs/content/building-gormes/builder-loop/agent-queue.md`",
		"`webpages/docs/content/building-gormes/builder-loop/blocked-slices.md`",
		"`webpages/docs/content/building-gormes/builder-loop/umbrella-cleanup.md`",
