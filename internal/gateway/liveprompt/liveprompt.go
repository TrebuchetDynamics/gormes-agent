...
func DefaultSeams() Seams {
	return Seams{
		ProfileDir:    func() string { return DefaultProfileDir(DefaultCWD()) },
		CWD:           DefaultCWD,
		MemoryDir:     func() string { return DefaultMemoryDir(DefaultCWD()) },
...
func DefaultProfileDir(cwd string) string {
	if override := strings.TrimSpace(os.Getenv("GORMES_CONTEXT_HOME")); override != "" {
		return override
	}
	gormesHome := config.GormesHome()
	if root := findAncestorWith(cwd, "SOUL.md"); root != "" {
		return root
	}
	if hasFile(gormesHome, "SOUL.md") {
		return gormesHome
	}
	if migrated := filepath.Join(gormesHome, "memory"); hasFile(migrated, "SOUL.md") {
		return migrated
	}
	return gormesHome
}
...
func findAncestorWith(start, name string) string {
...
func findAncestorWithAny(start string, names ...string) string {
	start = strings.TrimSpace(start)
	if start == "" {
		return ""
	}
...
func findAncestorSubdirWithAny(start, subdir string, names ...string) string {
	start = strings.TrimSpace(start)
	if start == "" {
		return ""
	}
...
// in production).
func Assemble(seams Seams, submitText, activeSessionID, sessionBlock string) (string, llm.ContextFilesReport, llm.DurableUserContextReport) {
	var (
		contextBlock  string
...
func buildMetadataFromSeams(seams Seams, activeSessionID string) string {
...
func buildSelfHelpFromSeams(seams Seams, submitText string) string {
...
func buildToolUseEnforcementBlock(seams Seams) string {
	if seams.ActiveModel == nil {
		return ""
	}
