...
func TestSkillsSyncCommandRunsProfileSyncWithFakeRoots(t *testing.T) {
	...
	cmd := newSkillsCommandWithProfileSyncForTest(SkillsProfileSyncSeams{
...
func TestDefaultSkillSyncProfilesUsesBaseHomeFromProfileScopedProcess(t *testing.T) {
	...
	profiles, err := DefaultSkillProfileRoots()
	if err != nil {
		t.Fatalf("DefaultSkillProfileRoots(): %v", err)
	}
...
func newSkillsCommandWithProfileSyncForTest(syncSeams SkillsProfileSyncSeams) *cobra.Command {
	return NewSkillsCommand(SkillsCLICommandOptions{
		SyncSeams:       syncSeams,
