...
|---|---|---|
| `internal/memory/schema.go` | Modify | Bump `schemaVersion = "3d"`; add `migration3cTo3d` constant |
| `internal/memory/migrate.go` | Modify | Extend switch to run `3c → 3d` step |
| `internal/memory/migrate_test.go` | Modify | Append `TestMigrate_3cTo3d` + table-shape tests |
| `internal/memory/embed_client.go` | Create | `embedClient` type + `Embed(ctx, model, input)` method |
| `internal/memory/embed_client_test.go` | Create | httptest-based unit tests |
| `internal/memory/cosine.go` | Create | `l2Normalize`, `dotProduct`, `topK`, `encodeFloat32LE`, `decodeFloat32LE` pure funcs |
| `internal/memory/cosine_test.go` | Create | Pure math + roundtrip tests |
| `internal/memory/semantic_sql.go` | Create | `semanticSeeds` + in-memory cache + graph-version counter |
| `internal/memory/semantic_sql_test.go` | Create | SQL + cache tests |
| `internal/memory/embedder.go` | Create | `Embedder` background worker — poll + embed + store |
| `internal/memory/embedder_test.go` | Create | Worker lifecycle + error-path tests |
| `internal/memory/recall.go` | Modify | `RecallConfig` gains semantic fields; `Provider.GetContext` chains semantic seeds |
| `internal/memory/recall_test.go` | Modify | Append 3 hybrid-fusion tests |
| `internal/config/config.go` | Modify | `TelegramCfg` gains semantic_* + embedder_* knobs |
| `internal/config/config_test.go` | Modify | Append `TestLoad_SemanticDefaults` |
| `gormes/cmd/gormes/telegram.go` | Modify | Construct `Embedder`, thread semantic config through `memory.NewRecall` |
| `internal/memory/semantic_integration_test.go` | Create | Ollama E2E: "tell me about my projects" → fence contains "Acme" |
...
In `internal/memory/migrate.go`, the `switch v` currently handles `"3a"`, `"3b"`, `"3c"` (no-op target), default. Change to:
...
**Files:**
- Create: `internal/memory/embed_client.go`
- Create: `internal/memory/embed_client_test.go`
...
```bash
git add internal/memory/embed_client.go internal/memory/embed_client_test.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `internal/memory/cosine.go`
- Create: `internal/memory/cosine_test.go`
...
```bash
git add internal/memory/cosine.go internal/memory/cosine_test.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `internal/memory/semantic_sql.go`
- Create: `internal/memory/semantic_sql_test.go`
...
```bash
git add internal/memory/semantic_sql.go internal/memory/semantic_sql_test.go
git commit -m "$(cat <<'EOF'
...
**NOTE:** `newEmbedClient` and `newSemanticCache` are lowercase (package-private) in Tasks 2 and 4. The cmd package can't call them directly. We need to export lightweight wrappers. Update `internal/memory/embed_client.go` to also expose:
...
```bash
git add gormes/cmd/gormes/telegram.go internal/memory/embed_client.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `internal/memory/semantic_integration_test.go`
...
```bash
git add internal/memory/semantic_integration_test.go
git commit -m "$(cat <<'EOF'
