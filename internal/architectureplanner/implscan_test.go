...
func TestComputeOwnedSubphases_AllWriteScopesUnderOriginalPrefixes(t *testing.T) {
		...
		Phases: map[string]progress.Phase{
			"5": {
				Subphases: map[string]progress.Subphase{
					"5.O": {
						Items: []progress.Item{
							{Name: "planner", WriteScope: []string{"internal/architectureplanner/run.go"}},
							{Name: "autoloop", WriteScope: []string{"cmd/autoloop/main.go"}},
						},
					},
					"5.P": {
						Items: []progress.Item{
							{Name: "mixed", WriteScope: []string{"internal/architectureplanner/run.go", "internal/gateway/server.go"}},
						},
					},
					"5.Q": {
	...
	got := computeOwnedSubphases(prog, []string{"internal/architectureplanner/", "cmd/autoloop/"})
