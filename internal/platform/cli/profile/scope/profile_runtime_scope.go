...
import (
	...
	activeprofile "github.com/TrebuchetDynamics/gormes-agent/internal/platform/cli/profile/active"
...
func ResolveProfileRuntimeScope(opts ProfileRuntimeScopeOptions) (ProfileRuntimeScope, error) {
	...
	contract, err := storage.NewProfileStorageContract(baseHome)
	if err != nil {
	...
	if profileID == "" && opts.ReadActiveProfile != nil {
		...
		if err != nil && !errors.Is(err, activeprofile.ErrActiveProfileUnset) {
	...
	if err := identity.ValidateProfileName(profileID); err != nil {
