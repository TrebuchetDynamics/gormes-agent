...
func CandidateDir(root string) string {
	if root == "" {
		return ""
	}
	return filepath.Join(root, "candidates")
}

func DraftCandidate(root string, maxBytes int, draft CandidateDraft) (CandidateMetadata, error) {
	...
	dir := filepath.Join(CandidateDir(root), meta.CandidateID)
	...
	if err := os.WriteFile(filepath.Join(dir, "SKILL.md"), []byte(document.RenderDocument(skill)), 0o644); err != nil {
		return CandidateMetadata{}, err
	}
...
func PromoteCandidate(root, activeDir string, maxBytes int, candidateID string) (ActiveMetadata, error) {
	metaPath := filepath.Join(CandidateDir(root), candidateID, "meta.json")
	var meta CandidateMetadata
	if err := readJSON(metaPath, &meta); err != nil {
		return ActiveMetadata{}, err
	}

	skillRaw, err := os.ReadFile(filepath.Join(CandidateDir(root), candidateID, "SKILL.md"))
	if err != nil {
		return ActiveMetadata{}, err
	}
	skill, err := document.Parse(skillRaw, maxBytes)
	if err != nil {
		return ActiveMetadata{}, err
	}

	targetActiveDir := filepath.Join(activeDir, meta.Slug)
	if _, err := os.Stat(targetActiveDir); err == nil {
		return ActiveMetadata{}, fmt.Errorf("skills: active skill %q already exists", meta.Slug)
	} else if !os.IsNotExist(err) {
		return ActiveMetadata{}, err
	}
	if err := os.MkdirAll(targetActiveDir, 0o755); err != nil {
		return ActiveMetadata{}, err
	}
	...
	if err := writeJSON(filepath.Join(targetActiveDir, "meta.json"), activeMeta); err != nil {
		return ActiveMetadata{}, err
	}
	if err := os.WriteFile(filepath.Join(targetActiveDir, "SKILL.md"), []byte(document.RenderDocument(skill)), 0o644); err != nil {
		return ActiveMetadata{}, err
	}
