...
func (t *SubdirectoryHintTracker) loadHintForDirectory(dir string) (SubdirectoryHint, []SubdirectoryHintEvidence, bool) {
	...
	for _, filename := range subdirectoryHintFilenames {
		...
		ev := contextsource.Evidence{
			Source:         filename,
			Path:           path,
			Loaded:         true,
			OriginalLength: len([]rune(content)),
		}
		content, ev = contextsource.ScanContent(content, filename, ev)
		if !ev.Blocked {
			content, ev = contextsource.TruncateContent(content, filename, t.maxChars, ev)
		}
