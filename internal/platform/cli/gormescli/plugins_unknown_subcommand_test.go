...
func newPluginsRootCommandForTest() *cobra.Command {
	return newRootCommandWithFactoryForTest("plugins", func() *cobra.Command {
		return NewPluginsCommand(PluginsOptions{BuildProvenance: func() PluginsBuildProvenance {
			build := testBuildProvenance()
			return PluginsBuildProvenance{Version: build.Version, GitCommit: build.GitCommit}
		}})
	})
}
...
func TestPluginsUnknownSubcommand_FailsLoudly(t *testing.T) {
	...
	stdout, stderr, err := executeRootCommandForTest(
		newPluginsRootCommandForTest(), "plugins", "listt",
	)
	if err == nil {
...
func TestPluginsBareCommand_KeepsListConvenience(t *testing.T) {
	...
	stdout, stderr, err := executeRootCommandForTest(
		newPluginsRootCommandForTest(), "plugins",
	)
	if err != nil {
...
func TestPluginsExplicitListSubcommand_StillWorks(t *testing.T) {
	...
	stdout, _, err := executeRootCommandForTest(
		newPluginsRootCommandForTest(), "plugins", "list",
	)
	if err != nil {
