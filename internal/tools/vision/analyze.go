...
func (*AnalyzeTool) Name() string { return "vision_analyze" }
...
func (*AnalyzeTool) Timeout() time.Duration { return 15 * time.Second }

func (*AnalyzeTool) Spec() toolkit.OperationSpec {
	return toolkit.OperationSpec{
		ToolDescriptor: toolkit.ToolDescriptor{
			Name:        "vision_analyze",
			Description: "Load a local image path into the next model turn as native vision content.",
			Schema:      (&AnalyzeTool{}).Schema(),
		},
		Mutating:   false,
		Idempotent: true,
		PromptSafe: true,
...
func (*AnalyzeTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
