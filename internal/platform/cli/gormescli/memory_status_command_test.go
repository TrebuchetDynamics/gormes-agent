...
func newMemoryStatusRootCommandForTest() *cobra.Command {
	factories := stubRootFactories()
	factories["memory"] = func() *cobra.Command {
		return NewMemoryCommand(MemoryCommandOptions{
			Status: MemoryCommandStatusOptions{
				BuildProvenance: func() MemoryBuildProvenance {
					return MemoryBuildProvenance{Version: Version, GitCommit: "test-git"}
				},
				OpenDB: openMemoryStatusDBForTest,
			},
		})
	}
...
func openMemoryStatusDBForTest(path string) (*sql.DB, error) {
	db, err := openMemoryStatusDBRawForTest(path)
	if err == nil {
		return db, nil
	}
	...
	return openMemoryStatusDBRawForTest(path)
}

func openMemoryStatusDBRawForTest(path string) (*sql.DB, error) {
...
func TestMemoryStatusCommand_PrintsExtractorSummary(t *testing.T) {
	...
	cmd := newMemoryStatusRootCommandForTest()
