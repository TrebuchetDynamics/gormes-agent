...
import (
	...
	"github.com/TrebuchetDynamics/gormes-agent/internal/tools/filesystem/pathresolve"
)
...
func atomicReplaceTarget(targetPath string) (string, bool, error) {
	resolved, symlink, err := pathresolve.ExistingOrSymlinkTarget(targetPath)
	if err != nil {
		return "", symlink, err
	}
	...
	if symlink && resolved == filepath.Clean(targetAbs) {
		return "", true, errors.New("symlink resolves to itself")
	}
	return resolved, symlink, nil
}
