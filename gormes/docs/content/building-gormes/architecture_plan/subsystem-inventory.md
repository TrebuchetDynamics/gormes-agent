...
| Plugins architecture | `plugins/context_engine/`, `plugins/example-dashboard/` + plugin SDK | 5.I | 🔨 in progress — `internal/plugins/manifest.go` now provides the typed `plugin.yaml` contract plus deterministic manifest discovery, and `pkg/gormes` re-exports the public loader surface; runtime loading, provider activation, and concrete plugin ports remain follow-on scope |
