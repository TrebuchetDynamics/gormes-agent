...
func TestLoad_RealFile_GoscraplingCrawlerFixtureAdapterSeam(t *testing.T) {
	...
	crawler := items["Goscrapling local crawler fixture adapter seam for web_crawl"]
	if crawler.Status != StatusComplete || crawler.ContractStatus != ContractStatusValidated || crawler.Module != ModuleBrowser || crawler.ExecutionOwner != ExecutionOwnerTools {
		t.Fatalf("Goscrapling crawler seam metadata = status %q contract_status %q module %q owner %q, want complete/validated/browser/tools", crawler.Status, crawler.ContractStatus, crawler.Module, crawler.ExecutionOwner)
	}
	for _, want := range []string{
		"hermes-agent/tools/web_tools.py:web_crawl_tool",
		"hermes-agent/tests/tools/test_web_providers.py:test_unconfigured_crawl_emits_top_level_error",
		...
		"internal/tools/web_tools.go:resolveGoscraplingCrawlerBackend",
		"internal/tools/web_tools_test.go:TestWebCrawlExplicitGoscraplingCrawlerUnavailableDoesNotFallback",
	} {
		if !containsString(crawler.SourceRefs, want) {
			t.Fatalf("Goscrapling crawler seam source_refs = %v, want %q", crawler.SourceRefs, want)
		}
	}
	if len(crawler.BlockedBy) != 0 {
		t.Fatalf("Goscrapling crawler seam blocked_by = %v, want none for fixture-ready row", crawler.BlockedBy)
	}
	if !containsString(crawler.TestCommands, "go test ./internal/tools -run 'TestWebCrawlToolUsesGoscraplingLocalCrawlerFixtureAdapter|TestWebCrawlExplicitGoscraplingCrawlerUnavailableDoesNotFallback|TestResolveWebBackendSupportsGoscraplingCrawlerUnavailableGate' -count=1") {
		t.Fatalf("Goscrapling crawler seam test_commands = %v, want focused fake-adapter proof", crawler.TestCommands)
	}
}
...
func TestLoad_RealFile_Phase2Ledger(t *testing.T) {
	...
	switch got := cron.DerivedStatus(); got {
	case StatusComplete, StatusInProgress:
		// Complete historically, or reopened by the operator run report lane.
	default:
		t.Fatalf("Phase 2.D = %q, want complete or in_progress", got)
	}
...
func TestLoad_RealFile_Phase2ExecutionQueue(t *testing.T) {
	...
	switch got := operator.DerivedStatus(); got {
	case StatusComplete, StatusInProgress:
		// Complete historically, or reopened by operator report delivery evidence.
	default:
		t.Fatalf("Phase 2.F.4 = %q, want complete or in_progress", got)
	}
