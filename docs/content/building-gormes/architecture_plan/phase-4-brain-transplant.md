...
| 4.B — Context Engine + Compression | ⏳ planned | The ContextEngine status boundary is landed. The pure compressor budget slice from Hermes `5401a008` is mirrored; remaining work is real history pruning, summarization feedback, context references, and tool-result protection |
...
10. **4.H unsupported-temperature retry + Codex flush guard** — mirror Hermes `b35d692f`: retry once without `temperature` only when the first request actually sent it and the provider rejected that parameter, and prove Codex Responses fallback never emits temperature.
11. **4.D integration/status surface only** — provider caps, metadata registry, pure routing selector, and per-turn override are already complete; any new 4.D row must wire those results into operator-facing selection/status without changing the selector contract.
...
13. **4.C prompt-builder context-file discovery** — SOUL/HERMES/AGENTS/CLAUDE ordering, truncation, frontmatter stripping, and prompt-injection scans after the context-engine contract is pinned.
