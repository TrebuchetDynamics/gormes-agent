...
import (
	...
	"github.com/TrebuchetDynamics/gormes-agent/internal/platform/cli/profile/contextscaffold"
	"github.com/TrebuchetDynamics/gormes-agent/internal/platform/cli/profile/identity"
	"github.com/TrebuchetDynamics/gormes-agent/internal/platform/cli/profile/storage"
...
func CreateProfile(options ProfileCreateOptions) (ProfileCreateResult, error) {
	...
	if err := identity.ValidateProfileName(name); err != nil {
	...
	if textvalue.IsNonBlank(options.TargetRoot) {
		...
		if xdgRoot == "" {
			return ProfileCreateResult{}, storage.ErrProfileXDGRootRequired
		}
		targetRoot, err = storage.ResolveProfileRoot(name, xdgRoot)
		if err != nil {
			return ProfileCreateResult{}, err
		}
	}
	...
	if options.CloneAll {
		...
		if textvalue.IsNonBlank(options.SourceRoot) {
			...
			sourceRoot, err = storage.ResolveProfileRuntimeRoot(filepath.Join(xdgRoot, "gormes"), "main")
			if err != nil {
				return ProfileCreateResult{}, err
			}
		}
		...
		if _, err := contextscaffold.ApplyProfileContextScaffold(contextscaffold.ProfileContextScaffoldOptions{ProfileName: name, DisplayName: options.DisplayName, TargetRoot: targetRoot}); err != nil {
			_ = os.RemoveAll(targetRoot)
			return ProfileCreateResult{}, err
		}
