...
| 5.I — Plugins Architecture | 🔨 in progress | `internal/plugins/manifest.go` now ships the typed Plugin SDK baseline: `plugin.yaml` parsing/validation, `requires_env` support, and deterministic manifest discovery, with `pkg/gormes` re-exporting the public loader surface. Runtime loading, provider activation, and third-party extension wiring remain follow-on scope. |
