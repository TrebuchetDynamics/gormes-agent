...
| 2 / 2.E.3 | Durable replay and inbox message contract | Durable pause/resume intent contract | Pause/resume intent fixtures are landed and durable ledger status exposes lifecycle-control state without requiring an external worker. | - |
...
| 3 / 3.E.7 | Cross-chat operator evidence | SillyTavern persona and group-chat mapping fixtures | Cross-chat deny-path fixtures are complete, Honcho-compatible scope/source tool schemas are visible, and SillyTavern host mapping drift is either fixture-locked or explicitly degraded. | Operator-auditable search evidence |
