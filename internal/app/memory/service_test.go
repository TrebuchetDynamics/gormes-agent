...
func TestNewStatusCommandPreservesJSONFlag(t *testing.T) {
	cmd := NewStatusCommand(Options{})
	if cmd.Use != "status" || cmd.Short == "" {
		t.Fatalf("command = %q/%q", cmd.Use, cmd.Short)
	}
	flag := cmd.Flags().Lookup("json")
	if flag == nil || !strings.Contains(flag.Usage, "goncho_queue") {
		t.Fatalf("missing json flag contract: %#v", flag)
	}
}
