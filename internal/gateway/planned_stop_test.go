...
func TestPlannedStopMarkerConsumesMatchingPIDAndStartTime(t *testing.T) {
	ctx := context.Background()
	now := time.Date(2026, 5, 7, 10, 0, 0, 0, time.UTC)
	store := NewPlannedStopStore(filepath.Join(t.TempDir(), ".gateway-planned-stop.json"))
	store.now = func() time.Time { return now }
	store.pid = func() int { return 4242 }
	store.startTime = func(int) (int64, bool) { return 987654, true }

	if err := store.Write(ctx, PlannedStopMarker{
		TargetPID:       4242,
	...
	if !result.Matched || result.Status != PlannedStopConsumeMatched {
...
func TestPlannedStopMarkerStaleOrMismatchedNeverMatches(t *testing.T) {
	ctx := context.Background()
	now := time.Date(2026, 5, 7, 10, 0, 0, 0, time.UTC)
	...
	t.Run("stale marker", func(t *testing.T) {
		...
		if err := store.Write(ctx, PlannedStopMarker{
			TargetPID:       4242,
			TargetStartTime: 987654,
			WrittenAt:       now.Add(-2 * PlannedStopMarkerTTL).Format(time.RFC3339Nano),
		}); err != nil {
			t.Fatalf("Write: %v", err)
		}
		...
		if result.Matched || result.Status != PlannedStopConsumeStale {
	...
	t.Run("start time mismatch", func(t *testing.T) {
		...
		if result.Matched || result.Status != PlannedStopConsumeMismatched {
