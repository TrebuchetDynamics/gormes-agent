...
type editorRunner = ConfigEditorRunner

func withConfigEditorRunner(t interface{ Cleanup(func()) }, runner editorRunner) {
	WithConfigEditorRunner(t, runner)
}

func newConfigCommandForTest() *cobra.Command {
	return NewConfigCommand(func() BuildProvenance { return testBuildProvenance() })
}
...
func TestConfigEdit_NoEditorReportsConfigPath(t *testing.T) {
	...
	stdout, stderr, err := executeConfigCommandForTest(cmd, "edit")
	if err != nil {
...
func TestConfigEdit_CreatesConfigFileBeforeOpening(t *testing.T) {
	...
	cmd := newConfigCommandForTest()
	if _, stderr, err := executeConfigCommandForTest(cmd, "edit"); err != nil {
		t.Fatalf("Execute: %v stderr=%s", err, stderr)
	}
...
`))
	...
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "check")
	if err != nil {
		t.Fatalf("Execute: %v stderr=%s", err, stderr)
	}
...
`))
	...
	cmd := newConfigCommandForTest()
	stdout, _, err := executeConfigCommandForTest(cmd, "check")
	if err == nil {
...
`))
	...
	cmd := newConfigCommandForTest()
	stdout, _, _ := executeConfigCommandForTest(cmd, "check")
...
`)
	...
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "migrate")
	if err != nil {
		t.Fatalf("Execute: %v stderr=%s", err, stderr)
	}
...
`)
	...
	cmd := newConfigCommandForTest()
	_, _, err := executeConfigCommandForTest(cmd, "migrate")
	if err == nil {
		t.Fatalf("Execute(future version) err = nil; want non-nil")
	}
...
func TestConfigCloseout_MigrateHelpDistinguishesNativeFromCrossProduct(t *testing.T) {
	setupOneshotFlagTestEnv(t)
	cmd := newConfigCommandForTest()
	stdout, _, err := executeConfigCommandForTest(cmd, "migrate", "--help")
	if err != nil {
