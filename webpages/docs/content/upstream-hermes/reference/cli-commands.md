...
| `hermes curator` | Background skill maintenance — status, run, pause, pin. See [Curator](../../user-guide/features/curator/). |
...
All changes persist to `fallback_providers:` under `model:` in `config.yaml`. Interacts with [Credential Pools](../../user-guide/features/credential-pools/): pools rotate keys *within* a provider, fallbacks switch to a *different* provider entirely.

See [Fallback Providers](../../user-guide/features/fallback-providers/) for behavior details and interaction with `fallback_model` (legacy single-fallback key).
