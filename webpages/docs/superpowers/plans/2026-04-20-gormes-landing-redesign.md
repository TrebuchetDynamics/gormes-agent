...
**Architecture:** The page renders from `www.gormes.ai/internal/site/`. Reuse the existing Go-templated, embed-served architecture. Trim the `LandingPage` struct, replace `index.tmpl`, swap obsolete partials for two new ones, rewrite `site.css`. Acceptance criteria are the verbatim copy strings locked in the spec.

**Tech Stack:** Go 1.25, `html/template`, `embed`, plain CSS, Playwright (smoke).

**Spec:** `docs/superpowers/specs/2026-04-20-gormes-landing-redesign-design.md`
...
| Action | Path | Purpose |
|---|---|---|
| Modify | `www.gormes.ai/internal/site/render_test.go` | Assert new hero/install/features/ledger copy renders; reject old strings. |
| Modify | `www.gormes.ai/internal/site/static_export_test.go` | Same assertions against `dist/index.html`. |
| Modify | `www.gormes.ai/tests/home.spec.mjs` | Playwright smoke against new headings + install command + mobile overflow. |
| Modify | `www.gormes.ai/internal/site/content.go` | Rewrite `LandingPage` struct + `DefaultPage()`. Drop obsolete fields. Add InstallStep, FeatureCard. |
| Modify | `www.gormes.ai/internal/site/templates/layout.tmpl` | Slim header/footer, new nav links, footer with `FooterLeft`/`FooterRight`. |
| Modify | `www.gormes.ai/internal/site/templates/index.tmpl` | New section composition: hero → install → features → ledger. |
| Modify | `www.gormes.ai/internal/site/templates/partials/ship_state.tmpl` | New pill style using `Tone` field for CSS class. |
| Create | `www.gormes.ai/internal/site/templates/partials/install_step.tmpl` | Renders one numbered install step (label + command). |
| Create | `www.gormes.ai/internal/site/templates/partials/feature_card.tmpl` | Renders one feature card (title + body). |
| Delete | `www.gormes.ai/internal/site/templates/partials/command_step.tmpl` | Obsolete (replaced by install_step). |
| Delete | `www.gormes.ai/internal/site/templates/partials/proof_stat.tmpl` | Obsolete (Proof Rail dropped). |
| Delete | `www.gormes.ai/internal/site/templates/partials/ops_module.tmpl` | Obsolete (Ops modules dropped). |
| Modify | `www.gormes.ai/internal/site/static/site.css` | Full rewrite. New variables, hero/install/features/ledger sections, mobile media query. |
...
Overwrite `www.gormes.ai/internal/site/render_test.go` with:
...
Overwrite `www.gormes.ai/internal/site/static_export_test.go` with:
...
```bash
cd <repo> && \
git add www.gormes.ai/internal/site/render_test.go \
        www.gormes.ai/internal/site/static_export_test.go && \
...
- Modify: `www.gormes.ai/internal/site/templates/partials/ship_state.tmpl`
- Create: `www.gormes.ai/internal/site/templates/partials/install_step.tmpl`
- Create: `www.gormes.ai/internal/site/templates/partials/feature_card.tmpl`
- Delete: `www.gormes.ai/internal/site/templates/partials/command_step.tmpl`
- Delete: `www.gormes.ai/internal/site/templates/partials/proof_stat.tmpl`
- Delete: `www.gormes.ai/internal/site/templates/partials/ops_module.tmpl`

- [ ] **Step 1: Replace `content.go` entirely**

Overwrite `www.gormes.ai/internal/site/content.go` with:
...
Overwrite `www.gormes.ai/internal/site/templates/layout.tmpl` with:
...
Overwrite `www.gormes.ai/internal/site/templates/index.tmpl` with:
...
Overwrite `www.gormes.ai/internal/site/templates/partials/ship_state.tmpl` with:
...
```bash
cd <repo> && \
git rm www.gormes.ai/internal/site/templates/partials/command_step.tmpl \
       www.gormes.ai/internal/site/templates/partials/proof_stat.tmpl \
       www.gormes.ai/internal/site/templates/partials/ops_module.tmpl
```
...
Expected: `ok  github.com/TrebuchetDynamics/gormes-agent/www.gormes.ai/internal/site  0.0Ns`.
...
```bash
cd <repo> && \
git add www.gormes.ai/internal/site/content.go \
        ...
        www.gormes.ai/internal/site/templates/partials/ship_state.tmpl && \
...
Overwrite `www.gormes.ai/internal/site/static/site.css` with:
...
```bash
cd www.gormes.ai && go run ./cmd/www-gormes -listen :8080
```
...
```bash
cd <repo> && \
git add www.gormes.ai/internal/site/static/site.css && \
...
Overwrite `www.gormes.ai/tests/home.spec.mjs` with:
...
```bash
cd <repo> && \
git add www.gormes.ai/tests/home.spec.mjs && \
...
Expected: `ok  github.com/TrebuchetDynamics/gormes-agent/www.gormes.ai/internal/site  ...`. The two `cmd/` packages report `[no test files]` — that's fine.
...
The Cloudflare Pages workflow `Deploy gormes.ai landing` will trigger on the next push affecting `www.gormes.ai/**`. After ~1 minute it should be live at `https://gormes.ai/`.
