...
├── go.mod                       # module: github.com/TrebuchetDynamics/gormes-agent
├── go.sum
├── README.md                    # "Rosetta Stone" explainer
└── Makefile                     # build, test, test-live, lint
```

Module path: `github.com/TrebuchetDynamics/gormes-agent`. This allows `go install github.com/TrebuchetDynamics/gormes-agent/cmd/gormes@latest` to work from a single upstream.
...
**Exception:** the repo-root `README.md` may be updated exactly once to add a "Go Implementation Status" section pointing at `README.md`. That README update is a separate commit from the Gormes scaffolding work and is explicitly out-of-scope for this spec (it can happen opportunistically once M1 ships).
...
8. `docs/ARCH_PLAN.md` exists and contains all six required subsections from §21.2; the Markdown-lint test at `docs/docs_test.go` passes on `ARCH_PLAN.md` and on this spec.
...
|---|---|---|
| `README.md` | GitHub visitors | 60-second pitch; install + "hello world"; links to ARCH_PLAN |
| `docs/ARCH_PLAN.md` | Forkers, Python-side contributors, public-site readers | Executive roadmap: program vision, "Why Go", milestones, hybrid manifesto |
| `docs/superpowers/specs/*.md` | Implementation agents + reviewers | Per-milestone source of truth (what each spec builds) |
...
M0 ships `docs/ARCH_PLAN.md` containing **all** of the following, in this order:
...
- Images live at `docs/images/` and are referenced by relative paths.
...
Item 8 of §18 is extended: `docs/ARCH_PLAN.md` exists **and** contains all six required subsections from §21.2 **and** passes a lint that rejects the five items from the §21.3 avoidance list.

Lint is enforced by a simple Go test at `docs/docs_test.go` that scans `ARCH_PLAN.md` and this spec for the avoidance-list patterns and fails on any match. Adding the test is part of M0.
