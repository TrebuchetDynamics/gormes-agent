...
| 5.B — Sandboxing Backends | 🔨 in progress | `internal/tools/environments/{daytona,docker,modal}` now port the Daytona, Docker, and Modal backend seams with tested resource normalization, snapshot restore fallback, exec translation, lifecycle cleanup, and Docker hardening/persistence mounts. Singularity, SSH, and shared file-sync plumbing remain pending. |
