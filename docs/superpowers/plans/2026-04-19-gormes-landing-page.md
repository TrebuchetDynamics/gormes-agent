...
- `www.gormes.ai/internal/site/assets.go` owns the embedded filesystem.
- `www.gormes.ai/internal/site/templates/*.tmpl` and `www.gormes.ai/internal/site/static/*` are part of the binary.
- `www.gormes.ai/internal/site/content.go` carries the landing-page copy and links.
- `www.gormes.ai/internal/site/server.go` wires the route surface.
- `www.gormes.ai/tests/home.spec.mjs` provides a browser smoke test for the public page.

## Work Items

- [ ] Keep the landing-page design spec and this plan doc in `gormes/docs` so Goldmark validation covers both.
- [ ] Document the actual Go asset layout in `www.gormes.ai/README.md`, including the `www.gormes.ai/internal/site` location and `//go:embed` usage.
- [ ] Document the local verification flow in `www.gormes.ai/README.md`:
  - `go test ./...`
  - `npm run test:e2e`
...
- The static assets now live under `www.gormes.ai/internal/site` specifically so the binary can embed them cleanly.
