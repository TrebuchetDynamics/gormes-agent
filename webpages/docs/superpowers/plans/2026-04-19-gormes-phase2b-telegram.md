...
- Create: `internal/telegram/mock_test.go`
- Create: `internal/telegram/bot.go` (stub)
- Create: `internal/telegram/bot_test.go` (first test only)
...
### Step 2 — Create `internal/telegram/client.go`
...
### Step 3 — Create `internal/telegram/mock_test.go`
...
### Step 4 — Create `internal/telegram/bot.go` (minimal scaffold)
...
### Step 5 — Create `internal/telegram/bot_test.go` with the first failing test
...
```bash
git add internal/telegram/client.go internal/telegram/mock_test.go internal/telegram/bot.go internal/telegram/bot_test.go gormes/go.mod gormes/go.sum
...
**Files:**
- Create: `internal/telegram/real_client.go`

Minimal, no logic. Just the production implementation of `telegramClient`.

### Step 1 — Create `internal/telegram/real_client.go`
...
```bash
cd ..
git add internal/telegram/real_client.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `internal/telegram/coalesce.go`
- Create: `internal/telegram/coalesce_test.go`
...
### Step 2 — Implement `internal/telegram/coalesce.go`
...
```bash
cd ..
git add internal/telegram/coalesce.go internal/telegram/coalesce_test.go
git commit -m "$(cat <<'EOF'
...
- Modify: `internal/telegram/bot.go` (add outbound goroutine + coalescer lifecycle)
- Modify: `internal/telegram/bot_test.go` (add streaming tests)

### Step 1 — Create `internal/telegram/render.go`
...
### Step 2 — Create `internal/telegram/render_test.go`
...
go vet ./internal/telegram/...
cd ..
git add internal/telegram/render.go internal/telegram/render_test.go internal/telegram/bot.go internal/telegram/bot_test.go internal/telegram/coalesce.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Modify: `internal/telegram/bot.go` (extend handleUpdate)
- Modify: `internal/telegram/bot_test.go` (three new tests)
...
**Files:**
- Create: `internal/buildisolation_test.go`
...
Temporarily add `_ "github.com/TrebuchetDynamics/gormes-agent/internal/telegram"` to `cmd/gormes/main.go`'s imports. Re-run the test.
...
**Files:**
- Modify: `internal/kernel/frame.go` (+ PlatformEventResetSession enum)
- Modify: `internal/kernel/kernel.go` (+ ResetSession method + Run-loop handler)
- Create: `internal/kernel/reset_test.go`
...
```bash
cd ..
git add internal/kernel/frame.go internal/kernel/kernel.go internal/kernel/reset_test.go
git commit -m "$(cat <<'EOF'
...
- Modify: `internal/config/config.go` (+ TelegramCfg section)
- Modify: `internal/config/config_test.go` (telegram defaults test)
...
```bash
cd ..
git add gormes/cmd/gormes-telegram/ internal/config/config.go internal/config/config_test.go internal/telegram/real_client.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Modify: `internal/telegram/bot_test.go` (new test)
...
cd ..
git add internal/telegram/bot_test.go
git commit -m "$(cat <<'EOF'
