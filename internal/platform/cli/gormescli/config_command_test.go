...
func TestConfigCommand_PathAndEnvPathJSON(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{[]string{"path", "--json"}, "config", config.ConfigPath()},
		{[]string{"env-path", "--json"}, "env", config.EnvPath()},
	} {
		t.Run(tc.wantKind, func(t *testing.T) {
			cmd := newConfigCommandForTest()
			stdout, stderr, err := executeConfigCommandForTest(cmd, tc.args...)
			if err != nil {
...
func TestConfigCommand_PathSubcommandPrintsConfigPath(t *testing.T) {
	setupOneshotFlagTestEnv(t)
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "path")
	if err != nil {
		t.Fatalf("Execute: %v\nstderr=%s", err, stderr)
	}
...
func TestConfigCommand_EnvPathSubcommandPrintsDotenvPath(t *testing.T) {
	setupOneshotFlagTestEnv(t)
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "env-path")
	if err != nil {
		t.Fatalf("Execute: %v\nstderr=%s", err, stderr)
	}
	want := config.EnvPath()
...
func TestConfigCommand_SetEndpointAndModelWritesTOMLOnly(t *testing.T) {
	setupOneshotFlagTestEnv(t)
	cmd := newConfigCommandForTest()
	if _, _, err := executeConfigCommandForTest(cmd, "set", "endpoint", "https://example.invalid/v1"); err != nil {
		t.Fatalf("set endpoint: %v", err)
	}

	cmd2 := newConfigCommandForTest()
	if _, _, err := executeConfigCommandForTest(cmd2, "set", "model", "test-model"); err != nil {
...
`))
	...
	cmd := newConfigCommandForTest()
	if _, stderr, err := executeConfigCommandForTest(cmd, "set", "hermes.model", "new-model"); err != nil {
...
func TestConfigCommand_SetTerminalCWDWritesTOMLAndLoads(t *testing.T) {
	...
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "set", "terminal.cwd", projectDir)
	if err != nil {
...
func TestConfigCommand_SetVoiceRecordKeyWritesTOMLAndLoads(t *testing.T) {
	...
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "set", "voice.record_key", "ctrl+space")
	if err != nil {
...
func TestConfigCommand_SetAPIKeyWritesEnvFileNotTOML(t *testing.T) {
	setupOneshotFlagTestEnv(t)
	cmd := newConfigCommandForTest()
	if _, _, err := executeConfigCommandForTest(cmd, "set", "api_key", "sk-secret-123"); err != nil {
...
func TestConfigCommand_SetTelegramBotTokenWritesLoadableEnvName(t *testing.T) {
	...
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "set", "telegram.bot_token", secret)
	if err != nil {
...
func TestConfigCommand_SetTelegramAllowedUserIDsWritesLoadableList(t *testing.T) {
	...
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "set", "telegram.allowed_user_ids", "6586915095")
	if err != nil {
...
func TestConfigCommand_SetNestedAgentsDefaultsWorkspaceWritesTOMLAndLoads(t *testing.T) {
	...
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "set", "agents.defaults.workspace", workspace)
	if err != nil {
...
func TestConfigCommand_SetMissingValueIsRejected(t *testing.T) {
	setupOneshotFlagTestEnv(t)
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "set", "endpoint")
	if err == nil {
...
func TestConfigCommand_SetExplicitEmptyValueIsAccepted(t *testing.T) {
	setupOneshotFlagTestEnv(t)
	cmd := newConfigCommandForTest()
	if _, stderr, err := executeConfigCommandForTest(cmd, "set", "endpoint", ""); err != nil {
...
func TestConfigCommand_SetUnknownSectionIsRejected(t *testing.T) {
	setupOneshotFlagTestEnv(t)
	cmd := newConfigCommandForTest()
	_, stderr, err := executeConfigCommandForTest(cmd, "set", "weatherman.endpoint", "x")
	if err == nil {
...
`))
	...
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "show")
	if err != nil {
		t.Fatalf("Execute: %v\nstderr=%s", err, stderr)
	}
...
`))
	...
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "show", "--json")
	if err != nil {
...
`))
	...
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "check", "--json")
	if err != nil {
...
`))
	...
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "migrate", "--json")
	if err != nil {
...
func TestConfigCommand_SetJSONEmitsStructuredOutcome(t *testing.T) {
	...
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "set", "hermes.endpoint", "https://example.invalid/v1", "--json")
	if err != nil {
...
func TestConfigCommand_SetJSONSecretRoutesToDotenv(t *testing.T) {
	...
	cmd := newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "set", "api_key", "sk-must-not-leak-XYZ", "--json")
	if err != nil {
