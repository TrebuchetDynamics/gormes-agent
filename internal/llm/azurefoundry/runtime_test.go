...
func TestAzureFoundryRuntime_KnownContextLengthRecorded(t *testing.T) {
	caps := routing.StaticModelContextCaps{
		routing.ModelContextKey{Provider: "azure-foundry", Model: "gpt-5.4"}: 400_000,
	}
	...
	if got.Context.Source != routing.ModelContextSourceProviderCap {
		t.Fatalf("Context.Source = %q, want %q", got.Context.Source, routing.ModelContextSourceProviderCap)
	}
...
func TestAzureFoundryRuntime_ModelInfoFallbackStillRecordsKnownContext(t *testing.T) {
	got := ResolveAzureFoundryRuntime(AzureFoundryRuntimeInput{
		...
		Env:       envFunc(map[string]string{"AZURE_FOUNDRY_API_KEY": "key"}),
		ModelInfo: routing.ModelContextMetadata{ContextWindow: 200_000},
	})
	...
	if got.Context.Source != routing.ModelContextSourceModelsDev {
		t.Fatalf("Context.Source = %q, want %q", got.Context.Source, routing.ModelContextSourceModelsDev)
	}
...
func TestAzureFoundryRuntime_NoLiveAzureDependency(t *testing.T) {
	...
	// to a fatal error.
	failing := routing.ModelContextLookupFunc(func(routing.ModelContextQuery) (int, bool, error) {
