...
func ClassifyFailure(err error, stdout, stderr string) BackendFailure {
	detail := FailureDetail(err, stdout, stderr)
	status := "backend_failed"
	haystack := strings.ToLower(strings.Join([]string{errorText(err), stdout, stderr}, "\n"))
	switch {
	case WasKilled(haystack):
...
func FailureDetail(err error, stdout, stderr string) string {
	var parts []string
...
func WasKilled(text string) bool {
...
func HitUsageLimit(text string) bool {
