...
// Corpus is the fixed corpus of operator-facing tip strings rendered by Gormes
...
// For returns the tip selected by the provided seed. The function is pure: the
// same seed always yields the same tip, so callers can drive deterministic
// rendering in tests by passing a fixed seed and a non-deterministic display by
// passing time.Now().UnixNano(). Negative seeds are normalised before indexing
// so the function never panics on the operator's clock skew or on a freshly
// zero-valued counter.
func For(seed int64) string {
	n := int64(len(Corpus))
	idx := seed % n
	if idx < 0 {
		idx += n
