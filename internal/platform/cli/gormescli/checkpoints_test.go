...
const checkpointsCommandTestVersion = "test-version"

func newCheckpointsCommand() *cobra.Command {
	return NewCheckpointsCommand(func() BuildProvenance {
		return BuildProvenance{Version: checkpointsCommandTestVersion, GitCommit: "test-git"}
	})
}
...
func TestCheckpointsCLI_Status_JSONEmitsStructured(t *testing.T) {
	...
	if got.Build.Version != checkpointsCommandTestVersion {
		t.Errorf("got.build.version = %q, want %q", got.Build.Version, checkpointsCommandTestVersion)
	}
