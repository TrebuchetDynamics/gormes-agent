...
| 5 / 5.Q | Kernel in-session model-switch seam for the native TUI | Introduce the in-session model-switch mechanism the local Bubble Tea kernel currently lacks, so a confirmed selection (e.g. from the /model picker) changes the model used by subsequent provider calls WITHOUT resetting history. Today PlatformEventKind is {Submit,Cancel,Quit,ResetSession,Steer} and kernel.go:171 tm.SetModel(cfg.Model) runs once at construction; the only live override (internal/gateway/model_picker.go SessionModelOverride) is gateway-server-only. The seam shape (a new PlatformEventKind such as PlatformEventSetModel carrying provider+model, vs a kernel reconfigure call, vs reusing telemetry/SetModel) is an interface-design decision and MUST go through gormes-interface-designer before implementation. Also covers the model-catalog -> internal/tui data seam so a picker can be populated without internal/tui importing the cmd/gormes catalog. | - | `-` | Unblocks Native TUI /model slash command binding over the existing model picker. |
