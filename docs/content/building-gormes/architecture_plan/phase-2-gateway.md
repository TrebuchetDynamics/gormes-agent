...
| Phase 2.F.3 — Restart / Pairing / Status | 🔨 in progress | P2 | Graceful shutdown drain, adapter startup failure cleanup (`gateway.StartupCloser` + `Manager.safeAdapterDisconnect`), `/restart` takeover markers, the in-process `gateway.StatusModel` lifecycle writers, and the read-only `gormes gateway status` view over configured channels plus runtime lifecycle (`internal/gateway/statusview.go` + `cmd/gormes/gateway_status.go`) are landed; next slices are active-turn follow-up/late-arrival drain policy, drain-timeout resume recovery, pairing persistence, pairing approval/rate-limit semantics, runtime-status JSON/PID validation, and token-scoped credential locks |
...
4. **P2 — 2.F.3 startup/drain correctness before richer lifecycle UX**
   Adapter startup failure cleanup is now frozen in `internal/gateway` via the optional `StartupCloser` interface and `Manager.safeAdapterDisconnect`, which mirrors upstream `GatewayRunner._safe_adapter_disconnect` and logs cleanup errors without masking the original startup failure. Next, define active-turn follow-up queue/late-arrival behavior and drain-timeout `resume_pending` recovery. These are upstream race fixes from `gateway/platforms/base.py` and `gateway/run.py`; keeping them separate prevents the status/pairing work from hiding concurrency regressions.
5. **P2 — 2.F.3 status readout + runtime convergence**
   The read-only `gormes gateway status` view is landed in `internal/gateway/statusview.go` (deterministic `RenderStatusSummary` over configured channels + `gateway.StatusModel` lifecycle) and wired as a cobra subcommand in `cmd/gormes/gateway_status.go` that does not open transports, the session map, or the memory store. Next slices port runtime-status JSON/PID validation, add token-scoped credential locks, and replace the current `paired=unknown/pending/in-session/unpaired` projection with authoritative `pairing.json` state once 2.F.3's pairing read-model slice lands. The in-process `gateway.StatusModel` write side is already landed (`internal/gateway/status.go`), so those on-disk slices wrap the same seam rather than introducing a parallel one.
