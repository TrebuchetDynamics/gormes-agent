				...
				if len(parts) == 2 {
					...
					case "domains":
						...
						for _, domain := range domains {
							normalized := urlpolicy.NormalizeBlocklistRule(domain)
							if normalized != "" {
...
func loadRulesFromSharedFile(path string) ([]WebsitePolicyRule, error) {
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		normalized := urlpolicy.NormalizeBlocklistRule(trimmed)
		if normalized != "" {
...
func CheckWebsiteAccess(rawURL string, configPath string) *WebsitePolicyResult {
	host := urlpolicy.ExtractHostFromURL(rawURL)
	if host == "" {
		return nil
	}
	...
	return policy.blockedResult(host)
}
...
func (p *WebsitePolicy) IsWebsiteBlocked(rawURL string) *WebsitePolicyResult {
	...
	return p.blockedResult(host)
}

func (p *WebsitePolicy) blockedResult(host string) *WebsitePolicyResult {
	for _, rule := range p.Rules {
		if urlpolicy.MatchHostAgainstRule(host, rule.Pattern) {
			return newWebsitePolicyBlockResult(host, rule)
		}
	}
	return nil
}

func newWebsitePolicyBlockResult(host string, rule WebsitePolicyRule) *WebsitePolicyResult {
