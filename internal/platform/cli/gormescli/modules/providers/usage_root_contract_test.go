...
const usageCommandTestVersion = "test-version"

func setupUsageCommandTestEnv(t *testing.T) {
	t.Helper()
	root := t.TempDir()
...
func writeUsageCommandConfig(t *testing.T, data []byte) {
	t.Helper()
...
func newUsageCommandForTest() *cobra.Command {
	return NewUsageCommand(Options{BuildProvenance: func() gormescli.BuildProvenance {
		return gormescli.BuildProvenance{Version: usageCommandTestVersion, GitCommit: "test-git"}
	}})
}

func executeUsageCommandForTest(cmd *cobra.Command, args ...string) (string, string, error) {
...
func TestUsageCommand_RendersUnsupportedProviderWithoutStartingTUI(t *testing.T) {
	setupUsageCommandTestEnv(t)
	cmd := newUsageCommandForTest()
	stdout, stderr, err := executeUsageCommandForTest(cmd, "--provider", "fixture-provider")
	if err != nil {
...
func TestUsageCommand_InfersProviderFromConfiguredModel(t *testing.T) {
	...
	defer server.Close()
	writeUsageCommandConfig(t, []byte(`
[hermes]
endpoint = "`+server.URL+`"
...
`))
	...
	cmd := newUsageCommandForTest()
	stdout, stderr, err := executeUsageCommandForTest(cmd)
	if err != nil {
		t.Fatalf("Execute() error = %v\nstderr=%s\nstdout=%s", err, stderr, stdout)
	}
...
`))
	...
	cmd := newUsageCommandForTest()
	stdout, stderr, err := executeUsageCommandForTest(cmd, "--json")
	if err != nil {
	...
	if got.Build.Version != usageCommandTestVersion {
		t.Errorf("got.build.version = %q, want %q", got.Build.Version, usageCommandTestVersion)
	}
