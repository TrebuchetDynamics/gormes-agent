...
func parseURLSkillFrontmatter(skillMD []byte) (map[string]any, error) {
	if len(skillMD) > document.DefaultMaxDocumentBytes {
		return nil, fmt.Errorf("skill document too large: %d > %d bytes", len(skillMD), document.DefaultMaxDocumentBytes)
	}
	...
	if urlSkillFrontmatterString(frontmatter, "description") == "" {
...
func resolveURLSkillName(frontmatter map[string]any, parsedURL *url.URL) (name string, awaitingName bool, err error) {
	if frontmatterName := urlSkillFrontmatterString(frontmatter, "name"); frontmatterName != "" {
...
func urlSkillFrontmatterString(frontmatter map[string]any, key string) string {
