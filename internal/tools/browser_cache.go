...
import (
	...
	"github.com/TrebuchetDynamics/gormes-agent/internal/tools/browser"
)
...
type BrowserResultCache = browser.BrowserResultCache
...
func NewBrowserResultCache(ttl time.Duration) *BrowserResultCache {
	return browser.NewBrowserResultCache(ttl)
}
...
func IsCacheableBrowserAction(kind string) bool {
	return browser.IsCacheableBrowserAction(kind)
}
...
func IsInvalidatingBrowserAction(kind string) bool {
	return browser.IsInvalidatingBrowserAction(kind)
}
...
func BrowserCacheEvidence(hit bool, actionKind, url string) string {
	return browser.BrowserCacheEvidence(hit, actionKind, url)
}
