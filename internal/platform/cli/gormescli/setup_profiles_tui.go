...
func maybeRunSetupProfilesTUI(cmd *cobra.Command, pseams SetupProfilesSeams, known []string, active string) (bool, error) {
	...
	if !ok || !setupProfilesInputIsTerminal(stdin) {
		return false, nil
	}
	if cfg, err := config.Load(nil); err == nil {
		if cfg.ProfileConfigV2Available() {
			...
			if runErr != nil {
				if TUIPickShouldFallback(runErr) {
					return false, nil
				}
				return true, runErr
			}
...
func buildSetupProfilesControlCenterTUIState(cfg config.Config) setupProfilesTUIState {
	model := controlcenter.BuildControlCenterModel(cfg, controlcenter.ControlCenterModelOptions{})
...
func controlCenterSetupWorkspacePaths(workspaces []controlcenter.ControlCenterWorkspace) []string {
...
func buildSetupProfilesTUIState(pseams SetupProfilesSeams, known []string, active string) setupProfilesTUIState {
...
func applySetupProfilesControlCenterTUIResult(cmd *cobra.Command, cfg config.Config, result setupProfilesTUIResult) error {
	...
	draft := controlcenter.NewControlCenterDraft(cfg)
	...
	for _, line := range controlcenter.RenderControlCenterDraftPreview(changes) {
		fmt.Fprintf(out, "  - %s\n", line)
	}
	if len(changes) == 0 {
...
func materializeSetupProfilesControlCenterProfile(name string) error {
	seams := defaultSetupProfilesSeams()
...
func applySetupProfilesTUIResult(cmd *cobra.Command, pseams SetupProfilesSeams, active string, result setupProfilesTUIResult) error {
	out := cmd.OutOrStdout()
	createName := strings.TrimSpace(result.CreateName)
	if createName != "" {
