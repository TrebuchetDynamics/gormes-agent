...
func TestResolveTelegramAudioTranscriber_PrefersCachedWASIOverHTTPWhenNoLocalCLI(t *testing.T) {
	...
	// Configure HTTP fallback; cached local WASI should still win.
	t.Setenv("GORMES_STT_OPENAI_KEY", "")
	t.Setenv("VOICE_TOOLS_OPENAI_KEY", "")
	t.Setenv("OPENAI_API_KEY", "sk-test-not-real")
	...
	if got == nil {
		t.Fatal("expected resolver to return cached WASI when no local whisper CLI is available, got nil")
	}
	if _, ok := got.(localSTTAudioTranscriber); !ok {
		t.Fatalf("expected localSTTAudioTranscriber before HTTP, got %T", got)
	}
}

func TestResolveTelegramAudioTranscriber_DefaultsToCachedWASIWhenNoExplicitModelOrHTTP(t *testing.T) {
	cached := fakeMainAudioTranscriber{transcript: "cached wasi"}
	restore := stubTranscriberConstructors(t, nil, nil, cached, nil)
	defer restore()

	got, err := ResolveTelegramAudioTranscriber().Transcribe(context.Background(), telegram.AudioInput{})
	if err != nil {
		t.Fatalf("Transcribe cached WASI: %v", err)
	}
	if got != "cached wasi" {
		t.Fatalf("expected cached WASI default, got %q", got)
	}
}

func TestResolveTelegramAudioTranscriber_ReturnsNilWhenNeitherLocalNorCachedNorHTTPConfigured(t *testing.T) {
	restore := stubTranscriberConstructors(t, nil, nil, nil, nil)
	defer restore()

	got := ResolveTelegramAudioTranscriber()
	if got != nil {
		t.Fatalf("expected nil when neither local, cached WASI, nor HTTP is configured, got %T", got)
	}
}
...
func TestResolveTelegramAudioTranscriber_PriorityLocalWASICachedHTTP(t *testing.T) {
	local := fakeMainAudioTranscriber{transcript: "local"}
	wasi := fakeMainAudioTranscriber{transcript: "wasi"}
	cached := fakeMainAudioTranscriber{transcript: "cached"}
	http := fakeMainAudioTranscriber{transcript: "http"}
	restore := stubTranscriberConstructors(t, local, wasi, cached, http)
	defer restore()
	...
	restore()
	restore = stubTranscriberConstructors(t, nil, wasi, cached, http)
	defer restore()
	got, err = ResolveTelegramAudioTranscriber().Transcribe(context.Background(), telegram.AudioInput{})
	if err != nil {
	...
	restore()
	restore = stubTranscriberConstructors(t, nil, nil, cached, http)
	defer restore()
	got, err = ResolveTelegramAudioTranscriber().Transcribe(context.Background(), telegram.AudioInput{})
	if err != nil {
	...
	if got != "cached" {
		t.Fatalf("expected cached WASI before HTTP, got %q", got)
	}

	restore()
	restore = stubTranscriberConstructors(t, nil, nil, nil, http)
	defer restore()
	got, err = ResolveTelegramAudioTranscriber().Transcribe(context.Background(), telegram.AudioInput{})
	if err != nil {
...
func stubTranscriberConstructors(t *testing.T, local, wasi, cached, http telegram.AudioTranscriber) func() {
	t.Helper()
	oldLocal := newLocalTelegramAudioTranscriber
	oldWASI := newWASIWhisperTelegramAudioTranscriber
	oldCached := newCachedWASIWhisperTelegramAudioTranscriber
	...
	newCachedWASIWhisperTelegramAudioTranscriber = func() telegram.AudioTranscriber { return cached }
	...
	return func() {
		...
		newCachedWASIWhisperTelegramAudioTranscriber = oldCached
