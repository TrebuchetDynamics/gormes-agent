...
import (
	...
	"github.com/TrebuchetDynamics/gormes-agent/internal/llm/routing/contextwindow"
	"github.com/TrebuchetDynamics/gormes-agent/internal/llm/routing/identity"
	"github.com/TrebuchetDynamics/gormes-agent/internal/llm/routing/modelcatalog"
)
...
func normalizeFallbackRoute(route ModelRoute) (ModelRoute, bool) {
	route.Provider = identity.Provider(route.Provider)
	route.Model = identity.Text(route.Model)
...
type ModelRoutingDecision struct {
	...
	Metadata modelcatalog.ModelMetadataResult
	Context  contextwindow.ModelContextResolution
	Status   []ModelRoutingStatus
}

type ModelRouterConfig struct {
	Registry        modelcatalog.ModelRegistry
	ContextResolver contextwindow.ModelContextResolver
}
...
func NewModelRouter(config ModelRouterConfig) ModelRouter {
	if config.Registry.IsZero() {
		config.Registry = modelcatalog.DefaultModelRegistry()
	}
	if config.ContextResolver.IsZero() {
		config.ContextResolver = contextwindow.DefaultModelContextResolver()
	}
	return ModelRouter{
...
func (r ModelRouter) evaluateRoute(route ModelRoute, availability providerAvailabilityIndex, status *[]ModelRoutingStatus) evaluatedModelRoute {
	...
	if route.empty() {
		evaluated.metadata = r.registry.Lookup(modelcatalog.ModelRegistryQuery{})
		evaluated.context = r.contextResolver.Resolve(contextwindow.ModelContextQuery{})
		return evaluated
	}
	...
	evaluated.context = r.contextResolver.Resolve(contextwindow.ModelContextQuery{
		Provider: route.Provider,
		Model:    route.Model,
		ModelInfo: contextwindow.ModelContextMetadata{
...
func newProviderAvailabilityIndex(providers []ProviderAvailability) providerAvailabilityIndex {
	...
	for _, provider := range providers {
		normalized := identity.Provider(provider.Provider)
		if normalized == "" {
			continue
		}
...
func (i providerAvailabilityIndex) available(provider string) (bool, string) {
	...
	status, ok := i[identity.Provider(provider)]
	if !ok {
...
func normalizeModelRoute(route ModelRoute) ModelRoute {
	return ModelRoute{
		Provider: identity.Provider(route.Provider),
		Model:    identity.Text(route.Model),
	}
}
