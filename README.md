...
| Release artifact | Linux build ~45.8 MB; no local Python, Node, Redis, vector DB, or Docker daemon required |
...
CI runs `go test ./... -count=1`, `go run ./cmd/progress validate`, and `git diff --check`. Release assets ship for Linux, macOS, Windows, and Termux/Android with SHA-256 checksums and SBOMs. The current Linux build measures ~45.8 MB (`benchmarks.json`).
