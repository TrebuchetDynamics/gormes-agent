...
type HermesRunner struct {
	client       llm.Client
	defaultModel string
	descriptors  map[string]llm.ToolDescriptor
}

func NewHermesRunner(client llm.Client, defaultModel string, descriptors []llm.ToolDescriptor) HermesRunner {
	index := make(map[string]llm.ToolDescriptor, len(descriptors))
...
func (r HermesRunner) Run(ctx context.Context, cfg SubagentConfig, events chan<- SubagentEvent) *SubagentResult {
	...
	request := llm.ChatRequest{
		Model:    model,
		Stream:   true,
		Messages: []llm.Message{{Role: "user", Content: buildChildPrompt(cfg)}},
	...
	for {
		...
		request.Messages = append(request.Messages, llm.Message{Role: "assistant", Content: draft.String(), ToolCalls: final.ToolCalls})
		for _, tc := range final.ToolCalls {
			...
			request.Messages = append(request.Messages, llm.Message{Role: "tool", ToolCallID: tc.ID, Name: tc.Name, Content: string(out)})
		}
	}
}

func (r HermesRunner) allowlistedDescriptors(enabled []string) []llm.ToolDescriptor {
	if len(r.descriptors) == 0 {
		return nil
	}
	if len(enabled) == 0 {
		out := make([]llm.ToolDescriptor, 0, len(r.descriptors))
	...
	out := make([]llm.ToolDescriptor, 0, len(enabled))
