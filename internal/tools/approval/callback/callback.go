...
import (
	...
	"github.com/TrebuchetDynamics/gormes-agent/internal/tools/approval/mode"
...
func GuardCommandWithApproval(ctx context.Context, toolName, cmd, approvalModeValue string) safety.BlockedResult {
	approvalMode := mode.NormalizeApprovalMode(approvalModeValue)
	result := safety.GuardCommand(cmd, approvalMode.Mode)
...
func approvalCallbackDenied(result safety.BlockedResult, reason, errText string) safety.BlockedResult {
