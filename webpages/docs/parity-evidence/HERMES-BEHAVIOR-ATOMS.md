...
| Auth commands (add/list/remove/reset/status/logout/spotify) | `hermes_cli/auth_commands.py` | `cmd/gormes/auth.go` | partial | All subcommands exist (add/list/remove/reset/status/logout/spotify). Missing: `auth add --manual-paste` for loopback OAuth on remote hosts (Cloud Shell, Codespaces, EC2). |
| `login` deprecated redirect | `hermes_cli/auth.py:6658` `login_command`; `hermes_cli/subcommands/login.py` | `internal/platform/cli/commands/typo/typo.go` | partial | Message now matches Hermes three-alternative format: "removed / gormes auth / gormes model / gormes setup". Remaining gap: Hermes registers `login` as a hidden subparser (exit 0, accepts all flags without argparse error); Gormes uses the typo handler (exit 1, shows "unknown command" prefix). |
...
| `model` subcommand | `hermes_cli/subcommands/model.py` | `cmd/gormes/model.go` | partial | Interactive model picker covered. Missing flags: `--refresh` (wipe model-picker disk cache and re-fetch `/v1/models`), `--no-browser`, `--manual-paste`, `--timeout`, `--ca-bundle`, `--insecure`, `--portal-url`, `--inference-url`, `--client-id`, `--scope` (all for Nous Portal OAuth during model selection). |
...
| MCP tool | `tools/mcp_tool.py` | `internal/tools/mcp/boundary`; `internal/tools/mcp/callresult`; `internal/tools/mcp/content` | partial | Tool registration/audit has redacted `mcp_host_unavailable` evidence for list/discovery failures, invoke transport errors, and unavailable tool results; generic host auth-required outcomes carry typed/redacted `mcp_auth_required` evidence; managed MCP gateway calls negotiate and recover streamable HTTP sessions. Against Hermes `bb5fc723`, tools/call parsing and rendering now preserve ResourceLink URI/name/mime metadata, nested EmbeddedResource text, mixed block order, the exact `mcp__<server>__read_resource` guidance name, resource text from `isError` results, and size-capped binary EmbeddedResource/AudioContent materialization through caller-supplied artifact directories and `MEDIA:<path>` output. Remaining gap: full OAuth/session lifecycle parity outside the managed gateway bridge. |
...
| MCP tool (client-side) | `tools/mcp_tool.py` | `internal/tools/mcp/boundary`; `internal/tools/mcp/callresult`; `internal/tools/mcp/content`; `internal/tools/mcp/runtime` | partial | Configured enabled non-OAuth HTTP servers now discover into the real default `tools.Registry` under Hermes-compatible `mcp__<server>__<tool>` names; include-over-exclude filters (including the explicit `include=[]` no-tools case), built-in/name collisions, bounded concurrent startup, schema/prompt safety, stable redacted failures, and ordinary kernel-executor invocation are covered. Invocation opens a fresh official-Go-SDK session, calls the exact source wire name, wraps model-visible output as untrusted, renders ResourceLink/EmbeddedResource/audio semantics, and closes. Managed gateway discovery/calls separately initialize, reuse `Mcp-Session-Id`, classify auth/session failures, and recover expired sessions once. Remaining gaps: long-lived generic session ownership, late refresh/reload, utility tools/toolset aliases, authenticated OAuth transport, stdio, sampling/parallel policy, and fuller OAuth lifecycle parity. |
| MCP ResourceLink + EmbeddedResource text results | `tools/mcp_tool.py@bb5fc723` `_render_mcp_resource_block`, `_make_tool_handler`; `tests/tools/test_mcp_resource_content.py@bb5fc723` | `internal/tools/mcp/callresult/result.go`; `internal/tools/mcp/content/content.go`; `internal/tools/mcp_normalize.go` | covered | Gormes preserves ResourceLink URI/name/mime metadata, nested resource text, mixed ordering, exact provider-visible reader wire name, and `isError` resource text/fallback behavior through parser and renderer fixtures. |
| MCP binary EmbeddedResource + AudioContent materialization | `tools/mcp_tool.py@bb5fc723` `_render_mcp_resource_block`, `_cache_mcp_audio_block`; `tests/tools/test_mcp_resource_content.py@bb5fc723` | `internal/tools/mcp/callresult/result.go`; `internal/tools/mcp/content/content.go`; `internal/tools/mcp/normalize.go`; `internal/tools/mcp_normalize.go` | covered | Gormes preserves encoded blob/audio payloads through transport-free parsing; `RenderWithOptions` enforces Hermes' 50 MiB cap, decodes into unique restrictive path-safe files beneath a caller-supplied artifact directory, emits document read guidance or gateway-compatible `MEDIA:<path>`, and returns inline non-leaking degraded markers for oversize, malformed, unavailable-dir, write-failure, and hostile-name cases. Proven by `result_test.go`, `content_test.go`, and `internal/tools/mcp_normalize_test.go`. |
...
| MCP config helpers | `hermes_cli/mcp_config.py@bb5fc723` `_get_mcp_servers`, `_save_mcp_server`, `_remove_mcp_server`, `_replace_mcp_servers`, `cmd_mcp_add`, `cmd_mcp_list`; `tests/hermes_cli/test_mcp_config.py@bb5fc723` | `internal/tools/mcp/config/config.go`; `internal/config/mcpstore/store.go`; `internal/app/mcplogin/service.go`; `internal/platform/cli/gormescli/mcpconfig.go` | partial | Gormes has a pure typed YAML/JSON `mcp_servers` resolver, profile-scoped atomic TOML persistence, production default loading, approved catalog install, config-only custom HTTP add with explicit overwrite, structurally redacted deterministic HTTP/stdio list, and concrete OAuth login state machinery. Exact config remove/rm now requires explicit confirmation and reports credentials/artifacts preserved. `gormes mcp test` performs a real official-Go-SDK Streamable HTTP initialize and paginated tools/list probe with strict timeout, redirect, retry, response-size, cleanup, and untrusted-output controls; OAuth and stdio fail before side effects. Configured non-OAuth HTTP tools also reach the default runtime registry and kernel executor through bounded discovery and fresh-session calls. `gormes mcp configure|config` atomically persists a bounded exact-name whitelist, explicit none (`include=[]`), or all-tools filter clearing in the active profile without probing; output exposes only safe counts/evidence and requires runtime refresh. Long-lived reload/session ownership, custom stdio/header-secret add, direct-executable stdio/authenticated-OAuth and interactive configure probing, utility tools/toolset aliases, durable OAuth-token/catalog-artifact cleanup, enable/disable, and fuller OAuth lifecycle remain gaps. Proven by `internal/config/mcpstore/store_test.go`, `internal/app/mcplogin/service_test.go`, and `internal/platform/cli/gormescli/mcpconfig_test.go`. |
| Approved optional MCP catalog | `optional-mcps/*/manifest.yaml@bb5fc723`; `hermes_cli/mcp_catalog.py@bb5fc723`; `hermes_cli/mcp_picker.py@bb5fc723`; `tests/hermes_cli/test_mcp_catalog.py@bb5fc723` | `internal/tools/mcp/catalog`; `internal/platform/cli/gormescli/mcpcatalog.go` | partial | Gormes embeds Gormes-branded Linear, n8n, and Unreal Engine manifests; typed `fs.FS` loading enforces manifest v1, strict entry/transport/auth/env/install shapes, a 256 KiB cap, deterministic ordering, duplicate fail-closed behavior, stable invalid/future diagnostics, and defensive copies. `gormes mcp catalog` provides deterministic text and JSON discovery without profile/config/secret/network/process access. Validation: `go test ./internal/tools/mcp/catalog -count=1`; `go test ./internal/platform/cli/gormescli -run 'MCP.*Catalog|MCPCommand' -count=1`; `go test ./internal/tools/mcp/... -count=1`. Gormes now provides profile-scoped atomic TOML installation and production loading for config-safe Linear/Unreal HTTP entries without network, process, secret, or probe side effects; reinstall preserves tool selection and ineligible n8n fails closed. Validation: `go test ./internal/config/mcpstore ./internal/tools/mcp/config ./internal/app/mcplogin -count=1`; `go test ./internal/platform/cli/gormescli -run 'MCP.*(Install|Login|Catalog|Command)' -count=1`; `go test ./internal/tools/mcp/... -count=1`. Custom HTTP config add, redacted HTTP/stdio list, and exact config remove/rm with mandatory confirmation are also native and non-probing. A separate read-only `gormes mcp test` now uses the pinned official Go MCP SDK for bounded Streamable HTTP initialize and paginated tool discovery; hermetic official-SDK fixtures prove same-origin header delivery, no standalone SSE, no retries or redirects, response caps, deadline enforcement, and session cleanup. Configured non-OAuth HTTP entries additionally discover and invoke through the default runtime registry with fresh official-SDK sessions. Explicit `mcp configure|config` include/none/all persistence now preserves unrelated secret-bearing config and feeds those runtime filters without network or process side effects. Remaining gap: dependent `Hermes MCP catalog install/configure lifecycle` remains a non-executable draft umbrella for long-lived session/reload ownership, n8n secret/git/bootstrap, custom stdio/header-secret add, direct-executable stdio/authenticated-OAuth and interactive configure probing, utility tools/toolset aliases, durable token/artifact cleanup, enable/disable, and picker behavior. |
...
| Webhook declarative route filters + script transforms | `gateway/platforms/webhook_filters.py@bb5fc723` `WebhookRouteProcessor`; `gateway/platforms/webhook.py@bb5fc723` filter/script integration; `tests/gateway/test_webhook_adapter.py@bb5fc723` `TestPayloadFilters` | `internal/adapters/channels/webhook/route_filters.go`; `internal/adapters/channels/webhook/route_scripts.go`; `internal/adapters/channels/webhook/runtime.go`; `internal/adapters/channels/webhook/route_filters_test.go`; `internal/adapters/channels/webhook/route_scripts_test.go`; `internal/adapters/channels/webhook/runtime_test.go`; `internal/gateway/platforms/inventory/manifest_test.go` | covered | Gormes evaluates pure route-local operators, profile-backed `in_file` watchlists, and route payload transforms before dedupe, prompt rendering, delivery, or callbacks. File reads use injected profile confinement and a 1 MiB cap. Script transforms intentionally replace Hermes' extension-selected Bash/Python execution with owner-approved Go-native direct executables under `ProfileHome/scripts`: no shell, argv, environment expansion, or executable PATH lookup; the child receives compact JSON stdin, script-parent cwd, an allowlisted environment, cancellation plus a maximum 30-second timeout, and bounded 1 MiB stdout/64 KiB stderr. JSON objects replace the prompt payload, redacted text becomes `script_output`, and silent/sentinel/non-object/nonzero/launch/timeout/cap failures ignore without consuming delivery IDs. Operator scripts remain trusted unsandboxed code running as the Gormes OS account. Validation: `go test ./internal/adapters/channels/webhook -count=1`; `go test ./internal/platform/redaction -count=1`; `go test ./internal/gateway/platforms/inventory -run TestHermesGatewayPlatformManifestCoversUpstream -count=1`. |
| Webhook channel delivery | `gateway/platforms/webhook.py` | `internal/adapters/channels/webhook` | covered | Signed ingress, dynamic route reload, prompt/delivery bridge, rate limits, dedupe, and accepted-event callback are fixture-covered; route filters/scripts are tracked separately above. |
