...
type BrowserResultInput struct {
	...
	Output    []byte
	MediaType string
	Budget    budget.ToolResultBudgetConfig
}
...
type BrowserResultEnvelope struct {
	...
	State    BrowserPageState
	Text     string
	Tool     budget.ToolResultEvidence
	Evidence string
}
...
func ValidateBrowserAction(action BrowserAction) BrowserActionDecision {
	...
	case BrowserActionNavigate:
		...
		guard := ssrfguard.Check(action.TaskID, action.URL, action.Options)
...
func BuildBrowserResultEnvelope(input BrowserResultInput) (BrowserResultEnvelope, error) {
	...
	text, evidence, err := budget.FormatToolResult(input.Budget, output, input.MediaType)
	...
	if evidence.Code == budget.ToolResultEvidenceTruncated || evidence.Code == budget.ToolResultEvidencePersisted || evidence.Code == budget.ToolResultEvidencePersistenceFailed {
...
func NewBrowserTranscript(taskID string) *BrowserTranscript {
	return &BrowserTranscript{TaskID: navigation.NormalizeTaskID(taskID)}
}
...
func acceptBrowserAction(taskID string) BrowserActionDecision {
	return BrowserActionDecision{Allowed: true, Evidence: BrowserEvidenceActionAccepted, Route: navigation.Route{SessionKey: navigation.NormalizeTaskID(taskID)}}
}

func denyBrowserAction(evidence string) BrowserActionDecision {
	return BrowserActionDecision{Allowed: false, Evidence: evidence, Route: navigation.Route{SessionKey: navigation.DefaultTaskID}}
}

func guardEvidenceOrAccepted(guard ssrfguard.Decision) string {
	if guard.Evidence != "" {
