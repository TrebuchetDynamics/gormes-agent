...
|---|---|---|---|---|---|
| 5 / 5.O | gormes doctor ◆ Profiles section content | Add `◆ Profiles` diagnostic content to `gormes doctor` (parity with hermes_cli/doctor.py@55c9f3206:1768): per-profile gateway-running + model summary. Gormes' profile model differs from Hermes' — this row first needs the active Gormes profile inventory contract identified (owned divergence) before TDD; emit one CheckResult{Name:"Profiles"} listing each Gormes profile with its gateway state + resolved model, Gormes-owned wording/paths. Out of scope until the Gormes profile contract is mapped. | - | `internal/doctor/doctor_profiles_test.go` | Contract metadata is present; ready for a focused spec or fixture slice. |
