...
var _ protocol.Client = (*realClient)(nil)

func NewRealClient(token string) (protocol.Client, error) {
...
func (c *realClient) SetMessageHandler(handler func(protocol.InboundMessage)) {
	c.mu.Lock()
	c.handler = handler
	c.mu.Unlock()
}
...
func (c *realClient) handleMessageCreate(_ *discordgo.Session, m *discordgo.MessageCreate) {
	...
	handler(protocol.InboundMessage{
		ID:           m.ID,
