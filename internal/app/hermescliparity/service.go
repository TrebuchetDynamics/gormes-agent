...
const (
	...
	StatusRowBacked   Status = "row_backed"
...
const (
	...
	KindCommandSet     Kind = "command_set"
	KindGlobalFlag     Kind = "global_flag"
	KindSlashCommand   Kind = "slash_command"
	KindAlias          Kind = "alias"
	KindGatewayHandler Kind = "gateway_handler"
	KindPluginCommand  Kind = "plugin_command"
)
...
func Manifest() []Entry {
	entries := []Entry{
		...
		{Path: []string{"whatsapp"}, Kind: KindCommand, Status: StatusImplemented, SourceRef: "hermes_cli/main.py:whatsapp", Target: "cmd/gormes whatsapp", Row: "WhatsApp top-level pairing wizard shell", Residual: "top-level WhatsApp wizard shell and plan output are implemented; bundling/running the live Baileys QR bridge remains row-backed"},
		...
		hermesGlobalFlag("-z", "hermes_cli/main.py:--oneshot", StatusExcluded, "", "removed root flag; use `gormes chat -q`"),
		hermesGlobalFlag("--oneshot", "hermes_cli/main.py:--oneshot", StatusExcluded, "", "removed root flag; use `gormes chat -q`"),
		hermesGlobalFlag("--model", "hermes_cli/main.py:--model", StatusImplemented, "cmd/gormes --model", "model override is implemented for startup resolution"),
		hermesGlobalFlag("--provider", "hermes_cli/main.py:--provider", StatusImplemented, "cmd/gormes --provider", "provider override is implemented for startup resolution"),
		hermesGlobalFlag("--offline", "cmd/gormes/main.go:--offline", StatusOwned, "cmd/gormes --offline", "Gormes-owned local smoke-test mode"),
		hermesGlobalFlag("--remote", "cmd/gormes/main.go:--remote", StatusOwned, "cmd/gormes --remote", "Gormes-owned remote TUI connection mode"),
		hermesRowPath([]string{"migrate", "ooenclaw"}, KindCommand, "operator request compatibility", "OpenClaw migration dry-run manifest", "must suggest `gormes migrate openclaw`; must not become a silent import alias"),
	}
	...
	entries = append(entries,
		hermesDynamicPluginCommand("memory", "plugins/memory/__init__.py:discover_plugin_cli_commands", StatusImplemented, "cmd/gormes memory", "memory plugin CLI is implemented as a first-party Gormes memory command surface"),
		hermesDynamicPluginCommand("disk-cleanup", "plugins/disk-cleanup/__init__.py:PluginContext.register_command", StatusRowBacked, "", "disk cleanup dynamic plugin command remains row-backed under Plugin SDK/tool security rows"),
	)
	return cloneEntries(entries)
}

func hermesImplementedCommand(name, sourceRef, target string) Entry {
	return Entry{Path: []string{name}, Kind: KindCommand, Status: StatusImplemented, SourceRef: sourceRef, Target: target, Residual: "implemented or classified by Gormes command surface tests"}
}

func hermesRowCommand(name, sourceRef, row, residual string) Entry {
	return Entry{Path: []string{name}, Kind: KindCommand, Status: StatusRowBacked, SourceRef: sourceRef, Row: row, Residual: residual}
}

func hermesCommandSet(name, sourceRef, residual, row string) Entry {
	return Entry{Path: []string{name}, Kind: KindCommandSet, Status: StatusRowBacked, SourceRef: sourceRef, Row: row, Residual: residual}
}

func hermesOwnedCommand(name, sourceRef, residual string) Entry {
	return Entry{Path: []string{name}, Kind: KindCommand, Status: StatusOwned, SourceRef: sourceRef, Target: "cmd/gormes " + name, Residual: residual}
}

func hermesOwnedPath(path []string, sourceRef, residual string) Entry {
	return Entry{Path: slices.Clone(path), Kind: KindCommand, Status: StatusOwned, SourceRef: sourceRef, Target: "cmd/gormes " + strings.Join(path, " "), Residual: residual}
}

func hermesImplementedPath(path []string, kind Kind, sourceRef, target, residual string) Entry {
	entry := Entry{
...
func hermesExcludedCommand(name, sourceRef, residual string) Entry {
	return Entry{Path: []string{name}, Kind: KindCommand, Status: StatusExcluded, SourceRef: sourceRef, Residual: residual}
}

func hermesProviderLogoutCommand() Entry {
...
func hermesGlobalFlag(flag, sourceRef string, status Status, target, residual string) Entry {
	return Entry{Path: []string{flag}, Kind: KindGlobalFlag, Status: status, SourceRef: sourceRef, Target: target, Residual: residual}
}

func hermesRowPath(path []string, kind Kind, sourceRef, row, residual string) Entry {
	return Entry{Path: slices.Clone(path), Kind: kind, Status: StatusRowBacked, SourceRef: sourceRef, Row: row, Residual: residual}
}

func hermesNestedCommands(group, sourceRef, row string, commands []string) []Entry {
	out := make([]Entry, 0, len(commands))
	for _, command := range commands {
		entry := hermesRowPath([]string{group, command}, KindCommand, sourceRef+":"+command, row, group+" "+command+" handler remains classified by this manifest; implementation lands in its dedicated row")
		markHermesCLIEntryFlags(&entry)
...
func hermesToolsCommands() []Entry {
	return []Entry{
...
func hermesFallbackCommands() []Entry {
	return []Entry{
		hermesImplementedPath([]string{"fallback", "list"}, KindCommand, "hermes_cli/main.py:fallback_subparsers:list", "cmd/gormes fallback list", "lists the local fallback provider chain from Gormes config"),
		hermesImplementedPath([]string{"fallback", "ls"}, KindAlias, "hermes_cli/main.py:fallback_subparsers:list aliases", "cmd/gormes fallback list", "fallback ls alias resolves to fallback list"),
		hermesImplementedPath([]string{"fallback", "add"}, KindCommand, "hermes_cli/fallback_cmd.py:cmd_fallback_add", "cmd/gormes fallback add", "appends a provider/model selected through the model picker without changing the primary model"),
		hermesImplementedPath([]string{"fallback", "remove"}, KindCommand, "hermes_cli/fallback_cmd.py:cmd_fallback_remove", "cmd/gormes fallback remove", "removes a selected fallback entry from the local chain"),
		hermesImplementedPath([]string{"fallback", "rm"}, KindAlias, "hermes_cli/main.py:fallback_subparsers:remove aliases", "cmd/gormes fallback remove", "fallback rm alias resolves to fallback remove"),
		hermesImplementedPath([]string{"fallback", "clear"}, KindCommand, "hermes_cli/fallback_cmd.py:cmd_fallback_clear", "cmd/gormes fallback clear", "clears the local fallback chain after confirmation"),
	}
}

func hermesClawCommands() []Entry {
	migrate := Entry{
	...
	cleanup := hermesRowPath([]string{"claw", "cleanup"}, KindCommand, "hermes_cli/main.py:claw_subparsers:cleanup", "OpenClaw migration writer and cleanup command", "`gormes claw cleanup` delegates to the Gormes-native OpenClaw cleanup engine")
	cleanup.Status = StatusImplemented
	...
	clean.Status = StatusImplemented
	clean.Target = "cmd/gormes claw cleanup"
	return []Entry{migrate, cleanup, clean}
}

func hermesCuratorCommands() []Entry {
	...
	out := make([]Entry, 0, len(commands))
	for _, command := range commands {
		entry := hermesRowPath([]string{"curator", command}, KindCommand, source+":"+command, row, "curator "+command+" is implemented over native Gormes curator state")
...
func hermesProfileCommands() []Entry {
	...
	for _, command := range implemented {
		entry := hermesImplementedPath([]string{"profile", command.name}, KindCommand, source+":"+command.name, command.target, command.residual)
		out = append(out, entry)
	}
	for _, command := range []string{"delete", "alias", "rename", "export", "import", "install", "update"} {
		entry := hermesRowPath([]string{"profile", command}, KindCommand, source+":"+command, row, "profile "+command+" is registered in Gormes as a deterministic row-backed unavailable command; full behavior remains row-backed")
		markHermesCLIEntryFlags(&entry)
		out = append(out, entry)
	}
	return out
...
func hermesGatewayNestedCommands() []Entry {
	...
	for i := range out {
		...
		switch key {
		case "gateway status":
			out[i].Status = StatusImplemented
...
func hermesKanbanCommands() []Entry {
	...
	for _, command := range core {
		entry := hermesRowPath([]string{"kanban", command}, KindCommand, source+":"+command, row, "kanban "+command+" is implemented for the default local Gormes board")
		entry.Status = StatusImplemented
	...
	logEntry := hermesRowPath([]string{"kanban", "log"}, KindCommand, source+":log", "Kanban worker log read command", "kanban log is implemented as a read-only selected-board worker log accessor")
	logEntry.Status = StatusImplemented
	logEntry.Target = "cmd/gormes kanban log"
	out = append(out, logEntry)

	tailEntry := hermesRowPath([]string{"kanban", "tail"}, KindCommand, source+":tail", "Kanban task event tail command", "kanban tail is implemented as a read-only selected-board task event follower")
	tailEntry.Status = StatusImplemented
	tailEntry.Target = "cmd/gormes kanban tail"
	out = append(out, tailEntry)

	alias := hermesRowPath([]string{"kanban", "ls"}, KindAlias, source+":list aliases", row, "kanban ls alias remains row-backed until the CLI alias is wired")
	...
	for _, command := range residual {
		entry := hermesRowPath([]string{"kanban", command}, KindCommand, source+":"+command, row, "kanban "+command+" remains row-backed after the durable board core slice")
		markHermesCLIEntryFlags(&entry)
		out = append(out, entry)
	}
	return out
...
func hermesNestedAlias(group, alias, canonical, sourceRef, row string) Entry {
	entry := hermesRowPath([]string{group, alias}, KindAlias, sourceRef, row, group+" "+alias+" alias resolves to "+group+" "+canonical)
	entry.AliasFor = []string{group, canonical}
	markHermesCLIEntryFlags(&entry)
	return entry
}

func hermesNestedSubcommand(group, command, subcommand, sourceRef, row string) Entry {
	entry := hermesRowPath([]string{group, command, subcommand}, KindCommand, sourceRef, row, group+" "+command+" "+subcommand+" handler remains classified by this manifest; implementation lands in its dedicated row")
	markHermesCLIEntryFlags(&entry)
	return entry
}

func markHermesCLIEntryFlags(entry *Entry) {
...
func hermesProviderAuthCommands() []Entry {
	commands := []struct {
		name        string
		sourceRef   string
		residual    string
	...
	for _, command := range commands {
		...
		entry := hermesRowPath([]string{"auth", command.name}, KindCommand, command.sourceRef, rowLabel, command.residual)
		if command.status != "" {
...
func hermesGatewayHandlers() []Entry {
	...
	for _, handler := range handlers {
		...
		switch handler {
		case "status":
			status, target, residual = StatusImplemented, "cmd/gormes gateway status", "read-only gateway status command is implemented"
		case "usage":
			status, target, residual = StatusImplemented, "cmd/gormes usage and /usage", "provider account usage binding is implemented"
		case "help", "restart", "reset":
			status, target = StatusImplemented, "internal/gateway command dispatcher"
		}
		out = append(out, Entry{Path: []string{"gateway-handler", handler}, Kind: KindGatewayHandler, Status: status, SourceRef: "gateway/run.py:_handle_" + strings.ReplaceAll(handler, "-", "_") + "_command", Target: target, Row: row, Residual: residual})
	}
	return out
}

func hermesSlashRegistryEntries() []Entry {
	var out []Entry
	for _, policy := range cli.CommandRegistry {
		...
		out = append(out, Entry{Path: []string{"/" + policy.Name}, Kind: KindSlashCommand, Status: status, SourceRef: "hermes_cli/commands.py:COMMAND_REGISTRY", Target: target, Row: row, Residual: "slash command is cross-linked to internal/cli.CommandRegistry active-turn policy"})
		for _, alias := range policy.Aliases {
			out = append(out, Entry{Path: []string{"/" + alias}, Kind: KindAlias, Status: status, SourceRef: "hermes_cli/commands.py:COMMAND_REGISTRY aliases", Target: target, Row: row, AliasFor: []string{"/" + policy.Name}, Residual: "slash alias resolves to /" + policy.Name + " through the active-turn registry"})
		}
	}
	return out
}

func hermesDynamicPluginCommand(name, sourceRef string, status Status, target, residual string) Entry {
	return Entry{Path: []string{"plugins", "dynamic", name}, Kind: KindPluginCommand, Status: status, SourceRef: sourceRef, Target: target, Row: "Plugin SDK", Residual: residual, Dynamic: true}
}
