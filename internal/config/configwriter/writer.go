...
func EnvPath() string {
	return filepath.Join(paths.GormesHome(), ".env")
}
...
func WriteTOMLValue(path, key, value string) error {
	...
	if !configschema.AllowsSection(section) {
		return fmt.Errorf("config: unknown section %q in key %q (allowed: %s)", section, key, configschema.AllowedSectionsList())
	}
...
func EnsureConfigFile(path string) error {
	...
	return writeTOMLAtomic(path, configschema.DefaultDocumentV2())
}
