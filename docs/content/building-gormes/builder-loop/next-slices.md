...
|---|---|---|---|---|---|
| 4 / 4.A | Azure Foundry transport probe read model | Azure Foundry endpoint probing determines OpenAI-style, Anthropic-style, or manual-required transport from deterministic inputs without reading credentials or writing config | operator, system | `internal/hermes/azure_foundry_probe_test.go` | Unblocks Azure Foundry runtime env/config read model. |
...
| 5 / 5.Q | Native TUI /save canonical session export | Native Bubble Tea /save exports canonical persisted session history through a session-save service instead of serializing UI-shaped in-memory transcript rows | operator, system | `internal/tui/session_save_test.go` | Contract metadata is present; ready for a focused spec or fixture slice. |
<!-- PROGRESS:END -->
