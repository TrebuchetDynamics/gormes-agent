...
| 5 / 5.Q | Native TUI /save canonical session export | Native TUI slash-command dispatch table | `Native TUI slash-command dispatch table` is complete (the registry and /save stub already exist)., internal/transcript.ExportMarkdown is the canonical persisted-transcript reader — `cmd/gormes/session.go` already proves it works over a sql.DB pointing at config.MemoryDBPath()., The existing TestSlashRegistry_SaveStubReturnsNotImplemented assertion in internal/tui/slash_dispatch_test.go is the only test the prior dispatch row required to assert /save was a no-op; replacing the stub means that one named test must be deleted or rewritten in the same slice (no test outside internal/tui asserts the stub message). | - |
