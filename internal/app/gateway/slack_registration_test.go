...
func (c *gatewaySlackTestChannel) Run(ctx context.Context, _ chan<- runtimegateway.InboundEvent) error {
...
type recordingGatewayRuntimeStatus struct {
	mu      sync.Mutex
	updates []runtimegateway.RuntimeStatusUpdate
}

func (s *recordingGatewayRuntimeStatus) UpdateRuntimeStatus(_ context.Context, update runtimegateway.RuntimeStatusUpdate) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *recordingGatewayRuntimeStatus) hasPlatformState(platform string, state runtimegateway.PlatformState) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func TestGatewaySlackRegistration_RegistersSlackThroughManagerWhenEnabled(t *testing.T) {
	...
	factories := ChannelFactories{
		Slack: func(got config.Config, _ *slog.Logger) (runtimegateway.Channel, error) {
	...
	waitForGatewaySlackCondition(t, time.Second, func() bool {
		return status.hasPlatformState("slack", runtimegateway.PlatformStateRunning)
	})
	cancel()
	select {
	case err := <-done:
...
func TestGatewaySlackRegistration_MissingTokensDegradeWithoutBlockingDiscord(t *testing.T) {
	...
	factories := ChannelFactories{
		Discord: func(config.Config, *slog.Logger) (runtimegateway.Channel, error) {
			discordFactoryCalled = true
			return newGatewaySlackTestChannel("discord"), nil
		},
		Slack: func(config.Config, *slog.Logger) (runtimegateway.Channel, error) {
	...
	if !status.hasPlatformState("slack", runtimegateway.PlatformStateFailed) {
