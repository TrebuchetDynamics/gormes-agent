...
|---|---|---|---|---|
| 4 / 4.D | Provider-default model resolution at config load | Routing policy and fallback selector | Routing policy and fallback selector is validated and serves as the single resolution path the new step plugs into., Provider catalog metadata (codex curated list, etc.) is available offline so config load and tests do not require network access. | - |
