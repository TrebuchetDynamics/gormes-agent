...
|---|---|---|---|---|
| 1 / 1.E | Unified admin TUI shell with tab navigation | Shared Bubble Tea wizard step chassis under internal/tui/wizard | 1.E.1 chassis is validated and the Step interface is stable., The admin shell lives under internal/tui/admin/ as a sibling to internal/tui/wizard; it does not edit the existing chat TUI root in internal/tui/model.go., The new cmd/gormes/admin.go cobra command compiles and runs as a no-arg `gormes admin` entry point. | Admin TUI: Setup health screen with missing-config callouts, Admin TUI: Chat tab with keybinding to jump in from any screen, Admin TUI: Agents screen wired to the 2.H dynamic registry |
