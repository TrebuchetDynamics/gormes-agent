...
func prepareSkills(ctx context.Context, in []Skill, opts RuntimeOptions) ([]Skill, []SkillStatus) {
	...
	for _, skill := range in {
		...
		default:
			...
			body, err := preprocessing.PreprocessSkillContent(ctx, skill.Body, preprocessOpts)
			if err != nil {
...
func skillPolicyKeys(skill Skill) []string {
	...
	add(skill.Name)
	add(commands.NormalizeSkillCommandName(skill.Name))
	if skill.Path != "" {
...
// PrepareSkills filters and preprocesses skill documents for runtime selection.
func PrepareSkills(ctx context.Context, in []Skill, opts RuntimeOptions) ([]Skill, []SkillStatus) {
	return prepareSkills(ctx, in, opts)
}

// MissingSkillCredentials returns required credential groups not satisfied by env.
func MissingSkillCredentials(skill Skill, env map[string]string) []string {
	return missingSkillCredentials(skill, env)
}

// SkillConditionsMatch reports whether a skill's tool/toolset conditions match availability.
func SkillConditionsMatch(conditions SkillConditions, availableTools, availableToolsets []string) bool {
	return skillConditionsMatch(conditions, availableTools, availableToolsets)
}
