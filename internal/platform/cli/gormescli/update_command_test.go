...
func TestUpdateCommandRegistersNativeUpdate(t *testing.T) {
	...
	stdout, stderr, err := executeUpdateCommandForTest(cmd, "--help")
	if err != nil {
...
func TestUpdateCommand_HelpDocumentsRollbackWorkflow(t *testing.T) {
	setupOneshotFlagTestEnv(t)
	cmd := newUpdateCommand()
	stdout, _, err := executeUpdateCommandForTest(cmd, "--help")
	if err != nil {
...
func TestUpdateCommandUsesInjectedLifecycle(t *testing.T) {
	...
	stdout, stderr, err := executeUpdateCommandForTest(command, "--branch", "development", "--restart-gateway", "always")
	if err != nil {
...
func TestUpdateCommandWiresBinaryPublisher(t *testing.T) {
	...
	stdout, stderr, err := executeUpdateCommandForTest(command, "--branch", "development")
	if err != nil {
		t.Fatalf("update command: %v stderr=%s stdout=%s", err, stderr, stdout)
	}
...
func TestUpdateCommandWiresGatewayRestartRunner(t *testing.T) {
	...
	stdout, stderr, err := executeUpdateCommandForTest(command, "--restart-gateway", "always")
	if err != nil {
		t.Fatalf("update command: %v stderr=%s stdout=%s", err, stderr, stdout)
	}
...
func TestUpdateCommandWritesUpdateLogAndLedgerForRealUpdate(t *testing.T) {
	...
	stdout, stderr, err := executeUpdateCommandForTest(command, "--branch", "development", "--restart-gateway", "never")
	if err != nil {
...
func TestUpdateCommand_PrintsCuratorRecentRunSummaryOnce(t *testing.T) {
	...
	stdout, stderr, err := executeUpdateCommandForTest(command)
	if err != nil {
		t.Fatalf("update: %v stderr=%s stdout=%s", err, stderr, stdout)
	}
	for _, want := range []string{
...
func TestUpdateCommand_JSONIncludesBuildProvenance(t *testing.T) {
	...
	stdout, _, err := executeUpdateCommandForTest(command, "--check", "--json")
	if err != nil {
	...
	if got.Build.Version != currentUpdateVersionForTest() {
		t.Fatalf("got.build.version = %q, want %q (matches package-level currentUpdateVersionForTest())", got.Build.Version, currentUpdateVersionForTest())
	}
...
func TestUpdateCommand_JSONOnFailureReturnsExitNonZero(t *testing.T) {
	...
	stdout, _, err := executeUpdateCommandForTest(command, "--json")
	if err == nil {
...
`), 0o644); err != nil {
	...
	if _, stderr, err := executeUpdateCommandForTest(command, "--check"); err != nil {
...
func TestUpdateCommandCheckModeSkipsMutation(t *testing.T) {
	...
	stdout, stderr, err := executeUpdateCommandForTest(command, "--check", "--branch", "development")
	if err != nil {
		t.Fatalf("update --check: %v stderr=%s", err, stderr)
	}
	if !called {
...
func TestUpdateCommand_DefaultCheckoutDirHonorsGormesInstallDir(t *testing.T) {
	...
	stdout, stderr, err := executeUpdateCommandForTest(command, "--check")
	if err != nil {
