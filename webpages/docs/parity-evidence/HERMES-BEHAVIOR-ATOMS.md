...
| MCP tool (client-side) | `tools/mcp_tool.py` | `internal/tools/mcp/boundary`; `internal/tools/mcp/callresult`; `internal/tools/mcp/content` | partial | Tool registration and redacted unavailable/auth-required evidence exist; managed gateway discovery/calls initialize, reuse `Mcp-Session-Id`, classify auth/session failures, and recover expired sessions once. Current Hermes ResourceLink/EmbeddedResource text semantics are covered by `result_test.go`, `content_test.go`, and `internal/tools/mcp_normalize_test.go`: metadata survives parsing, mixed blocks retain order, link guidance names `mcp__<server>__read_resource`, and `isError` nested text reaches the model with a stable empty-error fallback. Remaining gaps: binary EmbeddedResource and AudioContent cache materialization, authenticated-session preflight across all generic hosts, and fuller OAuth lifecycle parity. |
...
| MCP binary EmbeddedResource + AudioContent materialization | `tools/mcp_tool.py@bb5fc723` `_render_mcp_resource_block`, `_cache_mcp_audio_block`; `tests/tools/test_mcp_resource_content.py@bb5fc723` | → `missing` | missing | Gormes does not yet size-cap/decode/cache embedded blobs or audio blocks into profile-scoped artifacts/MEDIA tags; this remains separate from the completed text/link slice. |
