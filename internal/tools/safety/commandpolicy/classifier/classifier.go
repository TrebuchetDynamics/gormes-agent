...
func NewCommandClassifierWithConfig(cfg CommandClassifierConfig) *CommandClassifier {
	classifier := &CommandClassifier{
		...
		blockedPatterns: []*regexp.Regexp{
			...
			regexp.MustCompile(`(?i)` + hardline.FindRootWalkCommand),
		},
	}
...
func (c *CommandClassifier) ClassifyToolInput(toolName string, input []byte) CommandDecision {
...
func extractToolCommand(toolName string, input []byte) string {
