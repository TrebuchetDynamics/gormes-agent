...
func NormalizeNavivoxConfig(cfg *NavivoxCfg) error {
	...
	cfg.AllowOrigins = textlist.Compact(cfg.AllowOrigins)
	cfg.AllowedTailnetIdentities = textlist.Compact(cfg.AllowedTailnetIdentities)
...
func normalizeNavivoxProfileIDs(values []string) []string {
	cleaned := textlist.Compact(values)
	if len(cleaned) == 0 {
		return nil
	}
