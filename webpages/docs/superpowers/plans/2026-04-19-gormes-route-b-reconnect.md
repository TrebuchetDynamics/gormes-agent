...
**Files:**
- Create: `internal/kernel/retry.go`
- Create: `internal/kernel/retry_test.go`

Isolates the backoff-and-jitter math from `runTurn` so it can be unit-tested without spinning up a kernel.

- [ ] **Step 1:** Write the failing tests first. Create `internal/kernel/retry_test.go`:
...
- [ ] **Step 2:** Implement `internal/kernel/retry.go`:
...
```bash
git add internal/kernel/retry.go internal/kernel/retry_test.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Modify: `internal/kernel/kernel.go` (the streaming-loop section of `runTurn`)

- [ ] **Step 1:** Read `internal/kernel/kernel.go` and locate `runTurn`. The current streaming section looks roughly like:
...
**Files:**
- Modify: `internal/kernel/reconnect_test.go`
- Modify: `internal/kernel/reconnect_helpers_test.go` (add `newRealKernel`)
...
Add the needed imports: `hermes`, `store`, `telemetry` (all under `internal/`).
...
```bash
cd ..
git add internal/kernel/reconnect_test.go internal/kernel/reconnect_helpers_test.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Modify: `docs/PHASE1_COMPLETION.md` — update criterion 14 (was 8/10 discipline tests → 9/10)
- Modify: `docs/ARCH_PLAN.md` — no change needed; Route-B is a Phase-1.5 feature, Phase 1 stays ✅

- [ ] **Step 1:** In `docs/PHASE1_COMPLETION.md` find the row for criterion 14 and update it:
