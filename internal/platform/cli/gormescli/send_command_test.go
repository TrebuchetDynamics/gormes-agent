...
type sendCommandResult = SendResult

func TestSendCommandPositionalMessageDoesNotStartTUI(t *testing.T) {
	setupOneshotFlagTestEnv(t)

	var gotTarget, gotMessage string
	cmd := newSendCommandWithDeliveryForTest(func(_ context.Context, target, message string) (sendCommandResult, error) {
		gotTarget = target
		gotMessage = message
	...
	stdout, stderr, err := executeSendCommandForTest(cmd, "--to", "telegram", "hello world")
	if err != nil {
...
func TestSendCommandReadsFileAndPreservesNewline(t *testing.T) {
	...
	stdout, stderr, err := executeSendCommandForTest(cmd, "--to", "slack:#eng", "--file", body)
	if err != nil {
		t.Fatalf("send error = %v\nstdout=%s\nstderr=%s", err, stdout, stderr)
	}
...
func TestSendCommandReadsPipedStdin(t *testing.T) {
	...
	stdout, stderr, err := executeSendCommandForTest(cmd, "--to", "discord:#ops")
	if err != nil {
		t.Fatalf("send error = %v\nstdout=%s\nstderr=%s", err, stdout, stderr)
	}
...
func TestSendCommandRejectsInvalidFileText(t *testing.T) {
	...
	stdout, stderr, err := executeSendCommandForTest(newSendCommandForTest(), "--to", "telegram", "--file", body)
	if err == nil {
...
func TestSendCommandDryRunJSONSanitizesTerminalResponses(t *testing.T) {
	...
	stdout, stderr, err := executeSendCommandForTest(newSendCommandForTest(), "--to", "telegram", "--dry-run", "--json", "hello\x1b[53;1Rworld")
	if err != nil {
...
func TestSendCommandDryRunJSONSanitizesSubjectAndTarget(t *testing.T) {
	...
	stdout, stderr, err := executeSendCommandForTest(newSendCommandForTest(), "--to", "telegram\x1b[53;1R", "--subject", "[CI]\x1b[53;1R", "--dry-run", "--json", "body")
	if err != nil {
		t.Fatalf("dry-run error = %v\nstdout=%s\nstderr=%s", err, stdout, stderr)
	}
	var got sendCommandResult
...
func TestSendCommandListsChannelDirectory(t *testing.T) {
	...
	stdout, stderr, err := executeSendCommandForTest(newSendCommandForTest(), "--list", "telegram")
	if err != nil {
...
func TestSendCommandDefaultBackendUnavailable(t *testing.T) {
	...
	stdout, stderr, err := executeSendCommandForTest(newSendCommandForTest(), "--to", "telegram", "hello")
	if err == nil {
		t.Fatalf("send error = nil\nstdout=%s\nstderr=%s", stdout, stderr)
	}
...
func TestSendCommandBackendErrorIsSanitized(t *testing.T) {
	...
	cmd := newSendCommandWithDeliveryForTest(func(context.Context, string, string) (sendCommandResult, error) {
		return sendCommandResult{}, errors.New("platform failed \x1b[53;1R")
	})
	stdout, stderr, err := executeSendCommandForTest(cmd, "--to", "telegram", "hello")
	if err == nil {
		t.Fatalf("send error = nil\nstdout=%s\nstderr=%s", stdout, stderr)
	}
...
func newSendCommandForTest() *cobra.Command {
	return NewSendCommand(SendOptions{})
}

func newSendCommandWithDeliveryForTest(deliver SendDeliveryFunc) *cobra.Command {
	return NewSendCommand(SendOptions{Deliver: deliver})
}

func executeSendCommandForTest(cmd *cobra.Command, args ...string) (string, string, error) {
	return executeCobraCommandForTest(cmd, cobraCommandExecutionOptions{SilenceUsage: true, SilenceErrors: true}, args...)
}
