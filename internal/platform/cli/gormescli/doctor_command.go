// Package gormescli provides the Cobra command wiring and CLI contract for the
// gormes agent.
package gormescli
...
// DoctorCommandOptions contains the dependencies and seams for NewDoctorCommand.
type DoctorCommandOptions struct {
	// BuildProvenance returns the running binary's version and commit metadata.
	BuildProvenance func() BuildProvenance

	// NewExitCodeError wraps an exit code and error into an error value compatible
	// with cmd/gormes exit code handling.
	NewExitCodeError func(code int, err error) error

	// BuildFirstRunPlan builds a first-run plan for the given config and target.
	// Injected to avoid an import cycle between gormescli and gormescli/tuiapp.
	BuildFirstRunPlan func(cfg config.Config, target cli.SetupTargetID, force bool) cli.FirstRunPlan

	// FirstRunGuidanceCommand returns the CLI command string for a given guidance step.
	FirstRunGuidanceCommand func(command string) string
}

// NewDoctorCommand returns a fresh doctor command. Constructor pattern (rather
// than a package-level var with init-time flag registration) avoids cross-test
// flag-value contamination on the shared cobra FlagSet — each invocation builds
// an independent instance.
func NewDoctorCommand(opts DoctorCommandOptions) *cobra.Command {
	cmd := buildDoctorCmd(opts)
...
// DoctorNewTelegramClient is the seam for creating a Telegram client during
// doctor channel checks. Tests may override this seam to avoid live network calls.
var DoctorNewTelegramClient = func(token string) error {
	_, err := telegram.NewRealClient(token)
	return err
}

// DoctorNewDiscordSession is the seam for creating a Discord session during
// doctor channel checks. Tests may override this seam to avoid live network calls.
var DoctorNewDiscordSession = func(token string) error {
	return CheckDiscordSession(token)
}

// DoctorGitHubAuthRunner is the seam for running `gh auth status` during doctor
// GitHub auth checks. Tests inject a mock runner here.
	...
	case doctor.FixClassConfigVersion:
		...
		if err != nil {
			return doctor.DoctorFixOutcome{
				Class:  class,
				Name:   "config schema",
				Detail: fmt.Sprintf("%v; fix manually with %s", err, "`gormes config migrate`"),
			}
		}
		if res.Wrote {
...
func buildDoctorCmd(opts DoctorCommandOptions) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if opts.BuildProvenance != nil {
				prov = opts.BuildProvenance()
			}
			reporter := &doctorReporter{w: out, asJSON: asJSON, fix: fix, buildProvenance: prov}
			exitCode := 0
			...
			if newExitCodeError == nil {
				newExitCodeError = func(code int, err error) error { return err }
			}
			...
			reporter.Add(doctorBuildIdentityStatus(opts))
			...
			cfg = activatedCfg
			if target != "" {
				planBuilder := tuiappBuildFirstRunPlan
				if opts.BuildFirstRunPlan != nil {
					planBuilder = opts.BuildFirstRunPlan
				}
				plan := planBuilder(cfg, target, false)
				reporter.target = doctorTargetReadinessFromPlan(plan)
				reporter.Add(doctorTargetReadinessStatus(plan, opts))
			}
			...
			if !offline {
				...
				if doctorProviderHealthUsesAuthReadiness(cfg) {
					...
					if err != nil {
						redactedErr := friendlyProviderSetupDetail(doctorRedactRuntimeSecret(err.Error(), cfg.Hermes.APIKey))
						...
						if err := c.Health(ctx); err != nil {
							...
							redactedErr := doctorRedactRuntimeSecret(err.Error(), cfg.Hermes.APIKey)
			...
			if cfg.Telegram.BotToken == "" && !cfg.Discord.Enabled() && !cfg.Slack.Enabled {
				...
				if cfg.Telegram.BotToken != "" {
					if runtimeWarning, ok := doctor.TelegramGatewayRuntimeWarning(cfg.Telegram, runtimeStatus); ok {
						reporter.Add(runtimeWarning)
					} else if offline {
						reporter.Add(doctor.CheckResult{Name: "gateway/telegram", Status: doctor.StatusPass, Summary: doctorTelegramGatewayStatusDetail(cfg.Telegram) + " (network validation skipped --offline)"})
					} else if err := DoctorNewTelegramClient(cfg.Telegram.BotToken); err != nil {
						redactedErr := doctorRedactRuntimeSecret(err.Error(), cfg.Telegram.BotToken)
						reporter.Add(doctor.CheckResult{Name: "gateway/telegram", Status: doctor.StatusFail, Summary: redactedErr})
						markFailure(2)
					} else {
						reporter.Add(doctor.CheckResult{Name: "gateway/telegram", Status: doctor.StatusPass, Summary: doctorTelegramGatewayStatusDetail(cfg.Telegram)})
					}
				} else {
				...
				if cfg.Discord.Enabled() {
					if offline {
						...
						redactedErr := doctorRedactRuntimeSecret(err.Error(), cfg.Discord.Token)
...
func doctorTargetReadinessStatus(plan cli.FirstRunPlan, opts DoctorCommandOptions) doctor.CheckResult {
	status := doctor.StatusPass
	if !plan.Ready {
		status = doctor.StatusWarn
	}
	guidance := opts.FirstRunGuidanceCommand
	if guidance == nil {
		guidance = firstRunGuidanceCommand
	}
	summary := plan.Summary
	if command := guidance(plan.NextCommand); command != "" {
		if summary == "" {
	...
	for _, step := range plan.MissingSteps {
		note := step.Detail
		if command := guidance(step.Command); command != "" {
			note += "; run: " + command
		}
...
func doctorSecurityAdvisoriesStatus(ackID string) doctor.CheckResult {
	return DoctorSecurityAdvisoriesStatus(ackID, config.GormesHome())
}
...
func doctorBuildIdentityStatus(opts DoctorCommandOptions) doctor.CheckResult {
	prov := BuildProvenance{}
	if opts.BuildProvenance != nil {
		prov = opts.BuildProvenance()
	}
	dirty := ResolveGitDirty("")
	short := ResolveGitCommit(prov.GitCommit)
	if len(short) > 12 {
		short = short[:12]
	}
	...
	if dirty {
		return doctor.CheckResult{
			Name:    "build identity",
			Status:  doctor.StatusWarn,
			Summary: fmt.Sprintf("dirty build: version=%s %s — uncommitted source at build time", prov.Version, commitLabel),
		}
	}
	return doctor.CheckResult{
		Name:    "build identity",
		Status:  doctor.StatusPass,
		Summary: fmt.Sprintf("version=%s %s", prov.Version, commitLabel),
	}
}
...
func doctorSlackGatewayConfig(cfg config.Config, runtime gateway.RuntimeStatus) doctor.CheckResult {
	...
	if missing := doctorMissingSlackCredentials(slackCfg); len(missing) > 0 {
		return doctor.CheckResult{
			Name:    "Gateway Slack",
			Status:  doctor.StatusWarn,
...
// DoctorSlackGatewayConfig is the exported test helper version of
// doctorSlackGatewayConfig. Returns a CheckResult for the Slack gateway
// configuration, suitable for test assertions.
func DoctorSlackGatewayConfig(cfg config.Config, runtime gateway.RuntimeStatus) doctor.CheckResult {
	return doctorSlackGatewayConfig(cfg, runtime)
}
...
// doctorTelegramGatewayStatusDetail renders the Telegram config detail string.
// Inlined from gormescli/modules/gateway to avoid an import cycle.
func doctorTelegramGatewayStatusDetail(cfg config.TelegramCfg) string {
	detail := "first_run_discovery=" + strconv.FormatBool(cfg.FirstRunDiscovery)
...
// doctorMissingSlackCredentials returns the list of missing Slack credential fields.
// Inlined from gormescli/modules/gateway to avoid an import cycle.
func doctorMissingSlackCredentials(cfg config.SlackCfg) []string {
	missing := []string{}
...
// doctorRedactRuntimeSecret replaces known secret values with [REDACTED] in the
// given text. Inlined from cmd/gormes main.go to avoid circular dependencies.
func doctorRedactRuntimeSecret(text string, secrets ...string) string {
	redacted := text
...
// activateGatewaySecretRuntime activates secret refs from the gateway runtime
// config. Moved from cmd/gormes/gateway.go as a thin wrapper around
// gormesruntime.ActivateGatewaySecretRefs.
...
// friendlyProviderSetupDetail normalizes provider setup error messages for
// user-facing output. Moved from cmd/gormes/main.go.
...
// firstRunGuidanceCommand is the fallback for FirstRunGuidanceCommand when
// the caller does not supply it. Returns empty to indicate no guidance.
func firstRunGuidanceCommand(_ string) string {
	return ""
}

// tuiappBuildFirstRunPlan is the fallback for BuildFirstRunPlan when
// the caller does not supply it. Returns an empty plan to avoid nil panics.
func tuiappBuildFirstRunPlan(_ config.Config, _ cli.SetupTargetID, _ bool) cli.FirstRunPlan {
	return cli.FirstRunPlan{}
}

// firstNonEmpty returns the first non-empty (after trimming) string from a list.
