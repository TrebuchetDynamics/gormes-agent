...
// ErrModelNotFound is returned when the Ollama endpoint reports 404
...
var ErrModelNotFound = errors.New("memory: embed model not loaded")
...
func (c *Client) Embed(ctx context.Context, model, input string) ([]float32, error) {
	...
	if resp.StatusCode == http.StatusNotFound {
		...
		if strings.Contains(strings.ToLower(string(raw)), "not found") {
			return nil, fmt.Errorf("%w: %s", ErrModelNotFound, string(raw))
		}
