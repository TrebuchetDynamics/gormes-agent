...
func TestChannelDirectoryStaleTargetInvalidation(t *testing.T) {
	...
	_, evidence := dir.ValidateDeliveryTarget(DeliveryTarget{Platform: "discord", ChatID: "old", IsExplicit: true})
	if evidence.Code != "channel_target_stale" {
		t.Fatalf("Validate stale evidence = %+v, want channel_target_stale", evidence)
	}
	fresh, evidence := dir.ValidateDeliveryTarget(DeliveryTarget{Platform: "discord", ChatID: "fresh", IsExplicit: true})
