...
func TestLifecycle_PlannerSelfHealingFullLoop(t *testing.T) {
	...
	candidates, err := autoloop.NormalizeCandidates(progressPath, autoloop.CandidateOptions{ActiveFirst: true})
	if err != nil {
		t.Fatalf("Run 3: NormalizeCandidates: %v", err)
	}
	var rowXCandidate autoloop.Candidate
	var foundStale bool
	...
	candidates, err = autoloop.NormalizeCandidates(progressPath, autoloop.CandidateOptions{
		ActiveFirst:        true,
	...
	var rowXOverride autoloop.Candidate
