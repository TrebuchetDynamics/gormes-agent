...
type UpdateReleaseArtifactDownloader func(context.Context, releaseplan.UpdateReleasePlan, string) (UpdateReleaseArtifact, error)

type UpdateReleaseProvenanceVerifier func(context.Context, UpdateReleaseArtifact, releaseplan.UpdateReleasePlan) error

type UpdateReleaseBinaryOptions struct {
	Plan               releaseplan.UpdateReleasePlan
...
func RunUpdateReleaseBinaryUpdate(ctx context.Context, opts UpdateReleaseBinaryOptions) UpdateReleaseBinaryReport {
	...
	if opts.Plan.Source != releaseplan.UpdateSourceGitHubRelease {
		report.Failed = true
...
func smokeUpdateReleaseBinary(ctx context.Context, runner command.Runner, binaryPath string, plan releaseplan.UpdateReleasePlan) error {
	result := runner.RunCommand(ctx, "", nil, binaryPath, "version", "--json")
	if result.Err != nil {
		return fmt.Errorf("version --json failed: %s", command.FailureDetail(result))
	}
	var got struct {
...
func createUpdateReleaseSnapshot(ctx context.Context, snapshotPath, managedBin, publishedBin string, plan releaseplan.UpdateReleasePlan) (updateReleaseSnapshot, error) {
...
func defaultUpdateReleaseArtifactDownloader(ctx context.Context, plan releaseplan.UpdateReleasePlan, stageDir string) (UpdateReleaseArtifact, error) {
