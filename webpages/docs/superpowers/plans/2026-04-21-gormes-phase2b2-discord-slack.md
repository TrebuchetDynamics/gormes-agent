...
**Spec:** [`docs/superpowers/specs/2026-04-21-gormes-phase2b2-discord-slack-design.md`](../specs/2026-04-21-gormes-phase2b2-discord-slack-design.md)
...
| `internal/config/config.go` | Modify | Add `DiscordCfg` and `SlackCfg`, defaults, env loading |
| `internal/config/config_test.go` | Modify | TDD for new config sections and env overrides |
...
| `internal/discord/client.go` | Create | Narrow adapter-facing Discord client interface and event structs |
| `internal/discord/real_client.go` | Create | `discordgo` wrapper that satisfies the narrow client interface |
| `internal/discord/render.go` | Create | Discord-specific stream/final/error text formatting |
| `internal/discord/coalesce.go` | Create | Coalesced send/edit loop for Discord streaming |
| `internal/discord/bot.go` | Create | Discord adapter run loop, filtering, command parsing, typing, persistence |
| `internal/discord/mock_test.go` | Create | Mock Discord client and scripted inbound helpers |
| `internal/discord/bot_test.go` | Create | TDD for Discord inbound/outbound behavior |
| `internal/slack/client.go` | Create | Narrow Socket Mode client interface and normalized Slack event struct |
| `internal/slack/real_client.go` | Create | `slack-go/slack` + `socketmode` wrapper |
| `internal/slack/render.go` | Create | Slack text formatting for placeholder/final/error frames |
| `internal/slack/bot.go` | Create | Slack event loop, ACK path, thread-aware reply routing, persistence |
| `internal/slack/mock_test.go` | Create | Mock Slack client with deterministic event/ack tracking |
| `internal/slack/bot_test.go` | Create | TDD for Slack ACK, filtering, thread routing, outbound behavior |
| `internal/buildisolation_test.go` | Modify | Guard `internal/kernel` from taking Discord/Slack SDK deps |
| `docs/content/building-gormes/architecture_plan/phase-2-gateway.md` | Modify | Mark Discord + Slack batch as shipped and describe the narrow shared runtime boot |
| `docs/content/building-gormes/architecture_plan/subsystem-inventory.md` | Modify | Flip Discord and Slack rows to shipped and link implementation docs/specs |
| `docs/content/building-gormes/gateway-donor-map/shared-adapter-patterns.md` | Modify | Add direct references from donor patterns to landed Gormes files |
...
Update `internal/config/config.go` with these additions:
...
```bash
cd ..
git add gormes/go.mod gormes/go.sum internal/config/config.go internal/config/config_test.go
...
- Create: `internal/discord/real_client.go`
- Create: `internal/discord/render.go`
- Create: `internal/discord/coalesce.go`
- Create: `internal/discord/bot.go`
- Create: `internal/discord/mock_test.go`
...
Create `internal/discord/bot.go`, `render.go`, `coalesce.go`, and `real_client.go` with this structure:
...
import (
	...
	"github.com/TrebuchetDynamics/gormes-agent/internal/config"
	discordadapter "github.com/TrebuchetDynamics/gormes-agent/internal/discord"
	"github.com/TrebuchetDynamics/gormes-agent/internal/kernel"
)
...
- Create: `internal/slack/real_client.go`
- Create: `internal/slack/render.go`
- Create: `internal/slack/bot.go`
- Create: `internal/slack/mock_test.go`
- Create: `internal/slack/bot_test.go`
...
Create `internal/slack/bot.go`, `render.go`, and `real_client.go` with this structure:
...
import (
	...
	"github.com/TrebuchetDynamics/gormes-agent/internal/config"
	"github.com/TrebuchetDynamics/gormes-agent/internal/kernel"
	slackadapter "github.com/TrebuchetDynamics/gormes-agent/internal/slack"
)
...
**Files:**
- Modify: `internal/buildisolation_test.go`
- Modify: `docs/content/building-gormes/architecture_plan/phase-2-gateway.md`
- Modify: `docs/content/building-gormes/architecture_plan/subsystem-inventory.md`
- Modify: `docs/content/building-gormes/gateway-donor-map/shared-adapter-patterns.md`

**Reference anchors:**
- `internal/buildisolation_test.go`
...
- `docs/content/building-gormes/gateway-donor-map/shared-adapter-patterns.md`

- [ ] **Step 1: Add the isolation guard before touching docs**

Append this test to `internal/buildisolation_test.go`:
...
In `docs/content/building-gormes/architecture_plan/phase-2-gateway.md`, replace the wider gateway row with:
...
In `docs/content/building-gormes/architecture_plan/subsystem-inventory.md`, replace the Discord and Slack rows with:

```md
| Discord | `gateway/platforms/discord.py` | 2.B.2 | ✅ shipped | `internal/discord/*`, `gormes/cmd/gormes/discord.go` |
| Slack | `gateway/platforms/slack.py` | 2.B.3 | ✅ shipped | `internal/slack/*`, `gormes/cmd/gormes/slack.go` |
```

In `docs/content/building-gormes/gateway-donor-map/shared-adapter-patterns.md`, add a short implementation map:
...
```bash
cd ..
git add internal/buildisolation_test.go \
        docs/content/building-gormes/architecture_plan/phase-2-gateway.md \
        docs/content/building-gormes/architecture_plan/subsystem-inventory.md \
        docs/content/building-gormes/gateway-donor-map/shared-adapter-patterns.md
