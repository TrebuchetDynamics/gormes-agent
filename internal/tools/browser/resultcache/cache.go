...
func key(actionKind, url string) string {
	h := sha256.New()
...
func (c *Cache) Get(actionKind, url string) (json.RawMessage, bool) {
	if c == nil || c.ttl <= 0 {
		return nil, false
	}
	c.mu.RLock()
	item, ok := c.items[key(actionKind, url)]
	c.mu.RUnlock()
	if !ok {
		return nil, false
	}
...
func (c *Cache) Set(actionKind, url string, result json.RawMessage) {
	if c == nil || c.ttl <= 0 {
		return
	}
	c.mu.Lock()
	defer c.mu.Unlock()
	c.items[key(actionKind, url)] = cachedResult{
...
func (c *Cache) InvalidateAll() {
	...
	c.mu.Lock()
	defer c.mu.Unlock()
	c.items = map[string]cachedResult{}
}

// Stats returns cache occupancy for operator diagnostics.
func (c *Cache) Stats() (entries int, ttl time.Duration) {
	if c == nil {
		return 0, 0
	}
	c.mu.RLock()
...
func IsCacheableAction(kind string) bool {
	switch kind {
...
func IsInvalidatingAction(kind string) bool {
	switch kind {
...
func Evidence(hit bool, actionKind, url string) string {
	if hit {
