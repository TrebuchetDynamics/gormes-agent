...
// ReshapeOutcome correlates one planner-recorded ledgerlog.RowChange{Kind:"spec_changed"}
...
func Evaluate(plannerLedgerPath, autoloopLedgerPath string, window time.Duration, now time.Time) ([]ReshapeOutcome, error) {
	plannerEvents, err := ledgerlog.LoadLedgerWindow(plannerLedgerPath, window, now)
	if err != nil {
	...
	type latestReshape struct {
		event  ledgerlog.LedgerEvent
		change ledgerlog.RowChange
	}
...
// autoloopEventLite is a local subset of autoloop's ledgerlog.LedgerEvent decoded only
