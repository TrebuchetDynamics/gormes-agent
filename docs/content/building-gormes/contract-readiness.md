...
| 2 / 2.B.10 | BlueBubbles iMessage bubble formatting parity — BlueBubbles outbound iMessage sends are non-editable, markdown-stripped, paragraph-split bubbles without pagination suffixes | `fixture_ready` | `gateway` | `small` | gateway, system | `internal/channels/bluebubbles/bot_test.go` | BlueBubbles remains a usable first-pass adapter, but long replies may still arrive as one stripped text send until paragraph splitting and suffix-free chunking are fixture-locked. |
