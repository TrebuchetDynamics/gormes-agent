...
| Config command and provider overrides | `hermes_cli/config.py`, `tests/hermes_cli/test_set_config_value.py`, `tests/hermes_cli/test_config_env_expansion.py` | `cmd/gormes`, `internal/config`, `internal/cli` | Keep Gormes' native XDG TOML plus dotenv contract. Add `gormes config show/path/env-path/set`, invocation-only endpoint/model/api-key flags, secret redaction, explicit-empty handling, and no implicit runtime reads from Hermes home. | Phase 5.O |
