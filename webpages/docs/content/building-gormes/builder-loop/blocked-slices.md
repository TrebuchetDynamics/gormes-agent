...
| 5 / 5.Q | TUI websocket attach transport | SSE streaming to Bubble Tea TUI | SSE streaming to Bubble Tea TUI is complete, so the native remote-TUI client already owns frame delivery and submit/cancel/resize callbacks., The slice can use httptest plus gorilla/websocket fakes; no live Hermes Python gateway, provider, terminal, browser, or operator credential is required., This row adapts the Hermes attach behavior to Gormes' native RenderFrame transport instead of importing the React/Ink client or Python tui_gateway runtime. | - |
