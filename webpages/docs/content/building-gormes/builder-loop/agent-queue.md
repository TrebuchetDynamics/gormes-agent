...
## 2. Kernel in-session model-switch seam for the native TUI
...
- Status: `planned`
- Priority: `P1`
- Contract: Add a Design-B kernel in-session model-switch: a new PlatformEventKind `PlatformEventSetModel` whose PlatformEvent carries the existing `Model string` plus a new `Provider string`, processed exactly like PlatformEventResetSession — a synchronous `ack chan error`, valid only from PhaseIdle/PhaseFailed (reject mid-turn with a typed error analogous to ErrResetDuringTurn), and the resident mutation performed on the Run loop's own goroutine to preserve the single-owner invariant. The handler sets a resident session override (k.sessionModel/k.sessionProvider); selectTurnModel (kernel.go:1431) precedence becomes per-event PlatformEvent.Model > resident session override > k.cfg.Model; when the chosen provider differs from the resident client's, a new k.client is resolved through the EXISTING FallbackClientFactory(ctx, hermes.ModelRoute) machinery (kernel.go:37) rather than a bespoke client builder. Session history is NOT cleared (unlike ResetSession). Catalog seam (2) needs NO new code: the downstream /model picker is populated from the existing internal/hermes.ListPickerProviders() (internal/tui already imports internal/hermes), so this row is kernel-only. Interface design is COMPLETE (gormes-interface-designer 2026-05-15: Design B chosen; Design A 'TUI per-submit sticky' rejected — fixed construction-time client cannot cross-provider; Design C 'out-of-band SetSessionModel method' rejected — data race vs the single-owner event loop).
- Trust class: -
- Ready when: Interface design is complete (this row records Design B; gormes-interface-designer pass 2026-05-15/tui-slash-model).
- Not ready when: The implementation deviates from the PlatformEventResetSession ack/single-owner-goroutine template (introducing a data race)., The local kernel path is bound to the gateway-only SessionModelOverride instead of the new PlatformEventSetModel., A model switch clears session history (must preserve it like a normal continuation; only ResetSession clears history)., A bespoke provider-client builder is added instead of routing through the existing FallbackClientFactory/hermes.ModelRoute.
- Degraded mode: -
- Fixture: `-`
- Write scope: `internal/kernel/frame.go`, `internal/kernel/kernel.go`, `internal/kernel/kernel_test.go`, `docs/content/building-gormes/architecture_plan/progress.json`
- Test commands: `go test ./internal/kernel -run 'SetModel\|ModelSwitch\|SessionModel' -count=1`, `go test ./internal/kernel -count=1`, `go run ./cmd/progress validate`
- Done signal: PlatformEventSetModel is fixture-proven in internal/kernel: applies on the next turn, preserves history, rejects mid-turn via ack, and swaps provider client via the existing factory; the /model binding row's blocker is cleared.
- Acceptance: A PlatformEventSetModel{Provider,Model} with a synchronous ack changes the model on the NEXT turn's provider call; the resident k.cfg.Model is not mutated and per-event PlatformEvent.Model still wins over the session override., Session history is preserved across the switch (a fixture asserts prior turns survive and the next turn continues, not resets)., Sent mid-turn (not PhaseIdle/PhaseFailed) the event is rejected via the ack channel with a typed error analogous to ErrResetDuringTurn, no partial mutation., A cross-provider switch resolves the new client through the existing FallbackClientFactory/hermes.ModelRoute (fixture proves the factory is invoked, no bespoke builder); the gateway SessionModelOverride is untouched.
- Source refs: internal/kernel/frame.go (PlatformEventKind + PlatformEvent.Model/.Provider — add PlatformEventSetModel; PlatformEvent.Model already exists as a per-submit override), internal/kernel/kernel.go:1431 selectTurnModel(residentModel, override) (precedence merge point), internal/kernel/kernel.go:37 FallbackClientFactory(ctx, hermes.ModelRoute) (existing cross-provider client-build machinery to reuse), internal/kernel/kernel.go PlatformEventResetSession + ErrResetDuringTurn + ack pattern (the proven resident-mutation template to mirror, history-clear excluded), internal/hermes/picker_providers.go ListPickerProviders() (catalog seam 2 — already importable by internal/tui, no new code), internal/gateway/model_picker.go SessionModelOverride (gateway-only prior art — NOT reused for the local kernel path)
- Unblocks: Native TUI /model slash command binding over the existing model picker
- Why now: Unblocks Native TUI /model slash command binding over the existing model picker.
