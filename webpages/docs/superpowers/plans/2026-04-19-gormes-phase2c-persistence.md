...
**Spec:** [`docs/superpowers/specs/2026-04-19-gormes-phase2c-persistence-design.md`](../specs/2026-04-19-gormes-phase2c-persistence-design.md)
...
| `internal/telegram/bot_test.go` | Modify | Add `TestBot_PersistsSessionIDToMap`, `TestBot_ResumesSessionIDAcrossRestart` |
| `internal/config/config.go` | Modify | Add `Resume` field, `--resume` flag, exported `SessionDBPath()` |
| `internal/config/config_test.go` | Modify | Add `TestLoad_ResumeFlag`, `TestSessionDBPath_HonorsXDG` |
...
| `internal/buildisolation_test.go` | Modify | Add `TestKernelHasNoSessionDep` |
...
- Create: `internal/kernel/initial_session_test.go`
...
In `internal/kernel/kernel.go`, modify the `Config` struct:
...
```bash
cd ..
git add internal/kernel/kernel.go internal/kernel/initial_session_test.go
git commit -m "$(cat <<'EOF'
...
In `internal/telegram/bot.go`, add the import and modify the struct:
...
In `internal/telegram/bot.go`, modify `runOutbound` to call a new helper `persistIfChanged` before dispatching each frame:
