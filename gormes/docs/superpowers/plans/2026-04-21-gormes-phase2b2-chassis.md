...
- Do not rewrite Go import paths such as `github.com/TrebuchetDynamics/gormes-agent/gormes/...`, and do not rewrite CLI command names like `gormes gateway`.
...
**Files:**
- Create: `gormes/internal/gateway/event.go`
- Create: `gormes/internal/gateway/event_test.go`
...
Run: `cd gormes && go test ./internal/gateway/...`
Expected: `ok  github.com/TrebuchetDynamics/gormes-agent/gormes/internal/gateway`

- [ ] **Step 1.5: Commit**

```bash
git add gormes/internal/gateway/event.go gormes/internal/gateway/event_test.go
...
**Files:**
- Create: `gormes/internal/gateway/channel.go`
- Create: `gormes/internal/gateway/channel_test.go`
- Create: `gormes/internal/gateway/fake_test.go`
...
```bash
git add gormes/internal/gateway/channel.go gormes/internal/gateway/channel_test.go gormes/internal/gateway/fake_test.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `gormes/internal/gateway/manager.go`
- Create: `gormes/internal/gateway/manager_test.go`
...
```bash
git add gormes/internal/gateway/manager.go gormes/internal/gateway/manager_test.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Delete: `gormes/internal/telegram/coalesce.go` (content absorbed)
- Delete: `gormes/internal/telegram/coalesce_test.go` (content absorbed)
- Create: `gormes/internal/gateway/coalesce.go`
- Create: `gormes/internal/gateway/coalesce_test.go`
...
```bash
rm gormes/internal/telegram/coalesce.go gormes/internal/telegram/coalesce_test.go
```

- [ ] **Step 4.5: Fix `internal/telegram/` build**

The old telegram package still references `coalescer`. Temporarily add a compatibility shim at the top of `gormes/internal/telegram/bot.go`. Replace the import block with:
...
Create `gormes/internal/telegram/coalesce.go` with the same 171 lines as the pre-delete version (copy verbatim from git: `git show HEAD:gormes/internal/telegram/coalesce.go > gormes/internal/telegram/coalesce.go`). This keeps telegram compiling without touching its behavior. It is deleted in Task 8 when telegram is refactored onto `gateway.coalescer`.

```bash
git show HEAD:gormes/internal/telegram/coalesce.go > gormes/internal/telegram/coalesce.go
git show HEAD:gormes/internal/telegram/coalesce_test.go > gormes/internal/telegram/coalesce_test.go
```
...
```bash
git add gormes/internal/gateway/coalesce.go gormes/internal/gateway/coalesce_test.go gormes/internal/telegram/coalesce.go gormes/internal/telegram/coalesce_test.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Modify: `gormes/internal/gateway/manager.go`
- Modify: `gormes/internal/gateway/manager_test.go`
...
Append to `gormes/internal/gateway/manager_test.go`:
...
Add these imports to the top of the file: `"context"`, `"strings"`, `"sync"`, `"time"`, `"log/slog"`, `"github.com/TrebuchetDynamics/gormes-agent/gormes/internal/kernel"`.
...
Edit `gormes/internal/gateway/manager.go` — replace the `Manager` type and constructors:
...
Replace the existing `Run` stub in `gormes/internal/gateway/manager.go`:
...
**Files:**
- Delete: `gormes/internal/telegram/render.go`
- Delete: `gormes/internal/telegram/render_test.go`
- Create: `gormes/internal/gateway/render.go`
- Create: `gormes/internal/gateway/render_test.go`
...
- [ ] **Step 6.3: Implement `gormes/internal/gateway/render.go`**
...
```bash
rm gormes/internal/telegram/render.go gormes/internal/telegram/render_test.go
```

- [ ] **Step 6.5: Fix telegram package references**

The old `internal/telegram/bot.go` references `formatStream`, `formatFinal`, `formatError`. Redirect these to the new gateway helpers. Open `gormes/internal/telegram/bot.go` and add to the import block:
...
```bash
git add -A gormes/internal/gateway/render.go gormes/internal/gateway/render_test.go gormes/internal/telegram/
git commit -m "$(cat <<'EOF'
...
In `gormes/internal/gateway/manager.go`:
...
**Files:**
- Move: `gormes/internal/telegram/` → `gormes/internal/channels/telegram/`
- Modify: `gormes/internal/channels/telegram/bot.go` (implements `gateway.Channel`)
- Delete: `gormes/internal/channels/telegram/coalesce.go` (chassis owns it now)
- Delete: `gormes/internal/channels/telegram/coalesce_test.go`
- Modify: `cmd/gormes/telegram.go` (one-line import path update + builder change)

- [ ] **Step 8.1: Move the package**

```bash
mkdir -p gormes/internal/channels
git mv gormes/internal/telegram gormes/internal/channels/telegram
```
...
```bash
rm gormes/internal/channels/telegram/coalesce.go gormes/internal/channels/telegram/coalesce_test.go
```

- [ ] **Step 8.4: Refactor `bot.go` to a `gateway.Channel`**

The current `Bot` type runs both inbound AND outbound (coalescer spawn + frame consumption). Strip the outbound half — Manager owns it now. Replace the entire contents of `gormes/internal/channels/telegram/bot.go` with:
...
Add imports: `"strconv"`, `"github.com/TrebuchetDynamics/gormes-agent/gormes/internal/gateway"`.
...
Edit `gormes/internal/config/config.go`. In the `Config` struct, after `Telegram TelegramCfg`:
...
**Files:**
- Create: `gormes/internal/channels/discord/client.go`
- Create: `gormes/internal/channels/discord/mock_test.go`
...
```bash
git add gormes/internal/channels/discord/client.go gormes/internal/channels/discord/mock_test.go gormes/go.mod gormes/go.sum
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `gormes/internal/channels/discord/bot.go`
- Create: `gormes/internal/channels/discord/bot_test.go`
...
```bash
git add gormes/internal/channels/discord/bot.go gormes/internal/channels/discord/bot_test.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `gormes/internal/channels/discord/real_client.go`
...
```bash
git add gormes/internal/channels/discord/real_client.go
git commit -m "$(cat <<'EOF'
...
import (
	...
	"github.com/TrebuchetDynamics/gormes-agent/gormes/internal/channels/telegram"
