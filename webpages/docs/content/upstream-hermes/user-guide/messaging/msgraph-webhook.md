...
| `client_state` | none | Shared secret Graph echoes in every notification. Compared with `hmac.compare_digest`; generate with `openssl rand -hex 32`. |
...
Empty allowlist means accept from anywhere by default, preserving dev-tunnel workflows. Invalid CIDR strings log a warning and are ignored. **Review the Microsoft IP list quarterly** because it changes.
...
- [Microsoft Teams bot setup](../teams/): the different platform that lets users chat with Hermes in Teams
