...
// Turn mirrors the subset of turns columns the extractor prompt needs.
type Turn struct {
	ID      int64
	Role    string
	Content string
...
// SystemPrompt is the verbatim system message sent to the LLM before each
// extraction batch. Any change here is a behavior change of the entire
// extractor; bump schemaVersion if you expand the predicate or type whitelist
// to stay in lockstep with the CHECK constraints.
const SystemPrompt = `You are an ontological entity extractor. You read conversation turns between a user and an AI assistant, and you emit a structured JSON summary of the entities mentioned and the relationships between them.
...
func FormatBatchPrompt(rows []Turn) string {
