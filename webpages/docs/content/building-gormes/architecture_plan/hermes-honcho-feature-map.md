...
| Navivox SSH/Flutter channel | Gormes-owned Flutter/Navivox planning docs under `flutter-navivox/`; OpenClaw pairing/Tailscale setup evidence; Hermes gateway principles from `gateway/run.py` and `gateway/platforms/base.py` | `cmd/gormes`, `internal/channels/navivox`, Flutter app under `flutter-navivox/app` | Treat Navivox as a Gormes-owned first-party channel over SSH stdio, not a Hermes platform parity port. Build it in slices: binary frame codec and stdio control-plane server, QR pairing descriptor CLI with Tailscale-first host discovery, explicit host setup UX, selected-agent event routing, chat submit/update/final gateway binding, config admin, typed tool events/artifacts, and voice frames. The first server row stops at `hello`, `server.status`, `ping`, `pong`, and `error`; QR/setup rows must avoid credential leakage and keep sudo host mutation explicit. | Phase 2.B.12 `Navivox stdio protocol control-plane tracer`, `Navivox QR pairing descriptor CLI`, `Navivox host setup apply with transient sudo` |
