#!/usr/bin/env python3
"""Gormes /goal clone for Codex/OpenClaw-style agents.

The script is intentionally dependency-free: an agent can execute it from a
skill or a skill command reference, and tests can run it directly.
"""
...
STATE_DIR = Path(os.environ.get("GORMES_GOAL_HOME", Path.home() / ".agents" / "gormes-goal"))
DB_PATH = Path(os.environ.get("GORMES_GOAL_DB", STATE_DIR / "goals.sqlite"))
...
def _term_session_id() -> str | None:
    ...
    is stable for the lifetime of the surrounding agent session. That
...
def session_id() -> str:
    ...
    Order of preference:
    - GORMES_GOAL_SESSION_ID / GORMES_SESSION_ID (explicit override)
    - TERM_SESSION_ID / ITERM_SESSION_ID (stable across subshells in one
      agent session, distinct across separate terminal tabs)
    - PWD-derived hash (last resort; drifts in subshells)
    """
    for key in ("GORMES_GOAL_SESSION_ID", "GORMES_SESSION_ID"):
        value = os.environ.get(key)
        if value:
            return value
...
def init_db(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS goals (
            ...
            source TEXT NOT NULL DEFAULT 'gormes-goal',
...
def candidate_session_ids(hook_data: dict[str, Any] | None = None) -> list[str]:
    ...
    slightly different PWD in the same agent session. The list
    ...
    sources: list[str | None] = [
        os.environ.get("GORMES_GOAL_SESSION_ID"),
        os.environ.get("GORMES_SESSION_ID"),
    ]
    if hook_data:
...
def find_goal(
    ...
    *,
    ...
    leaks goals across separate agent sessions: a paused goal in session
...
def set_goal(conn: sqlite3.Connection, sid: str, objective: str, token_budget: int | None) -> sqlite3.Row:
    ...
    if existing:
        raise ValueError("this agent session already has a goal; use: /goal clear, then set a new goal")
...
def update_status(conn: sqlite3.Connection, sid: str, status: str) -> sqlite3.Row:
    ...
    if not goal:
        raise ValueError("no goal is set for this agent session")
...
def render_goal(row: sqlite3.Row | None) -> str:
    if not row:
        return "No goal is currently set for this agent session."
    ...
    if row["token_budget"] is not None:
        parts.append(f"- Token budget: {fmt_tokens(row['token_budget'])} (soft budget; agent harness custom skills do not expose reliable live token counters)")
    return "\n".join(parts)
...
Continue working toward the active agent thread goal.
...
`python3 ~/.agents/skills/gormes-goal/scripts/gormes_goal.py complete`
...
def render_invoke_result(action: str, goal: sqlite3.Row | None, extra: str = "") -> str:
    ...
    if goal and goal["status"] == "active":
        ...
        body.extend(["", "agent instructions: Do not continue this goal until the user runs `/goal resume`."])
    elif goal and goal["status"] == "budget_limited":
        body.extend(["", "agent instructions: The soft budget is exhausted; summarize progress and ask before continuing."])
    return "\n".join(body)
...
def stop_hook() -> int:
    ...
    with sqlite_connect() as conn:
        ...
        max_continues = int(os.environ.get("ARENATON_GOAL_MAX_STOP_CONTINUES", "500"))
        ...
        if recent_count >= max_continues:
            print(
                json.dumps(
                    {
                        "continue": True,
                        "stopReason": f"/goal auto-continuation stopped after {max_continues} Stop-hook continuations. Run /goal resume or raise ARENATON_GOAL_MAX_STOP_CONTINUES to continue automatically.",
                    }
                )
            )
            return 0
...
def main(argv: list[str]) -> int:
    ...
    parser = argparse.ArgumentParser(description="agent harness /goal command")
    sub = parser.add_subparsers(dest="cmd")
    p_invoke = sub.add_parser("invoke", help="Process slash-command arguments and print agent-facing instructions")
