...
|---|---|---|---|---|---|
| 5 / 5.V | Web dashboard server shell + degraded inventory | Reconcile the Go web dashboard server shell against Hermes' FastAPI dashboard startup, localhost security guard, public API allowlist, and degraded inventory semantics before any React feature work is treated as complete. | operator, system | `internal/apiserver/dashboard_contract_test.go` | P0 handoff; needs contract proof before closeout. |
