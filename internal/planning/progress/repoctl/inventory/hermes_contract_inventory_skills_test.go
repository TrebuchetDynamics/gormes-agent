...
func TestHermesContractInventoryClassifiesSkillCatalogSurface(t *testing.T) {
	...
	bundled := catalogFamilyByID(t, families, "bundled_catalog_metadata", "skill catalog")
	...
	if !containsString(bundled.Examples, "skills/creative/popular-web-designs/SKILL.md") {
		t.Fatalf("bundled examples = %v, want popular-web-designs SKILL.md", bundled.Examples)
	}
	if !containsProgressRow(bundled.ProgressRows, "Portable SKILL.md format") {
		t.Fatalf("bundled progress rows = %+v, want Portable SKILL.md format", bundled.ProgressRows)
	}
	if !containsSourcePair(bundled.SourcePairs, "skills/creative/popular-web-designs/SKILL.md") {
		t.Fatalf("bundled source pairs = %+v, want popular-web-designs SKILL.md", bundled.SourcePairs)
	}

	triggers := catalogFamilyByID(t, families, "triggers_tags_related_skills", "skill catalog")
	for _, want := range []string{"skills/yuanbao/SKILL.md", "optional-skills/devops/pinggy-tunnel/SKILL.md"} {
		if !containsString(triggers.Examples, want) {
			t.Fatalf("triggers/tags examples = %v, want %s", triggers.Examples, want)
		}
	}

	support := catalogFamilyByID(t, families, "support_assets", "skill catalog")
			...
			"1": {
				...
				Subphases: map[string]progress.Subphase{
					"1.A": {
						Name: "Skill catalog",
						Items: []progress.Item{
							catalogProgressItem("Portable SKILL.md format", progress.StatusComplete, progress.ContractStatusValidated, "skills", "hermes-agent/skills/creative/popular-web-designs/SKILL.md", "go test ./internal/skills -run TestParseSkill -count=1"),
							catalogProgressItem("SKILL.md metadata parser fixtures", progress.StatusComplete, progress.ContractStatusValidated, "skills", "hermes-agent/skills/productivity/notion/SKILL.md", "go test ./internal/skills -run TestOptionalSkillCatalogHermesV014Metadata -count=1"),
							catalogProgressItem("Optional skill catalog metadata", progress.StatusComplete, progress.ContractStatusValidated, "skills", "hermes-agent/optional-skills/research/osint-investigation/SKILL.md", "go test ./internal/skills -run TestOptionalSkillCatalogHermesV014Metadata -count=1"),
							catalogProgressItem("Skill hub source lockfile", progress.StatusComplete, progress.ContractStatusValidated, "skills", "hermes-agent/skills/index-cache/openai_skills_skills_.json", "go test ./internal/skills -run TestSyncBundledSkillsFromManifestUsesDigestThreeWayAndConflictCopies -count=1"),
							catalogProgressItem("Skill category description catalog", progress.StatusPlanned, progress.ContractStatusFixtureReady, "skills", "hermes-agent/skills/creative/DESCRIPTION.md", "go test ./internal/skills -run TestListInstalledSkills -count=1"),
							catalogProgressItem("Skill support asset inventory", progress.StatusPlanned, progress.ContractStatusFixtureReady, "skills", "hermes-agent/optional-skills/creative/concept-diagrams/references/dashboard-patterns.md", "go test ./internal/skills -run TestPreprocessSkillContent -count=1"),
							catalogProgressItem("Voice transcription helper scripts", progress.StatusComplete, progress.ContractStatusValidated, "tools", "hermes-agent/tools/transcription_tools.py", "go test ./internal/tools -run TestTranscriptionTool -count=1"),
						},
					},
				},
			},
