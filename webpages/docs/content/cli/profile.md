...
access should be restricted to those normalized project roots, with the active
profile root still available for profile state. A local terminal must use real
confinement or fail closed under that allow-list; changing `cwd` is not enough
