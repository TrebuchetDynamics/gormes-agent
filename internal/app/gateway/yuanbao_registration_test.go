...
import (
	...
	"github.com/TrebuchetDynamics/gormes-agent/internal/adapters/channels/yuanbao"
...
func TestYuanbaoGateway_MissingCredentialsDegradeWithoutBlockingTelegram(t *testing.T) {
	...
	if !status.hasPlatformState("yuanbao", runtimegateway.PlatformStateFailed) {
...
func TestYuanbaoGateway_RegistersWhenEnabledWithCredentials(t *testing.T) {
	...
	factories := ChannelFactories{
		Yuanbao: func(got config.Config, _ *slog.Logger) (runtimegateway.Channel, error) {
