...
func TestTermuxNotificationSendUsesFakeExecWithRedactedBoundedArgs(t *testing.T) {
	...
	if result.Status != NotificationStatusSent {
		t.Fatalf("Status = %q, want %q: %+v", result.Status, NotificationStatusSent, result)
	}
	...
	if len([]rune(gotArgs[3])) > DefaultNotificationBodyRunes {
		t.Fatalf("body length = %d runes, want <= %d", len([]rune(gotArgs[3])), DefaultNotificationBodyRunes)
	}
	if !result.Redacted {
...
func TestTermuxNotificationMissingCommandReturnsUnavailableEvidence(t *testing.T) {
	...
	if result.Status != NotificationStatusUnavailable {
		t.Fatalf("Status = %q, want %q: %+v", result.Status, NotificationStatusUnavailable, result)
	}
...
func TestNotificationTermuxNonTermuxSkipsWithoutRunner(t *testing.T) {
	...
	result := sender.Notify(context.Background(), "title", "body")
	if result.Status != NotificationStatusSkipped {
		t.Fatalf("Status = %q, want %q: %+v", result.Status, NotificationStatusSkipped, result)
	}
	if called {
