...
Work from a clean dedicated worktree. The current main worktree already has unrelated edits in `README.md`, `README.md`, `docs/ARCH_PLAN.md`, and `docs/THEORETICAL_ADVANTAGES_GORMES_HERMES.md`; do not fold those into this rename.
...
Update `docs/landing_page_docs_test.go` so the active docs surface expects the `.ai` directory/module identity and the new cutover spec/plan to be present:
...
```bash
git add www.gormes.io/internal/site/render_test.go www.gormes.io/tests/home.spec.mjs docs/landing_page_docs_test.go
...
Replace the `targets` slice in `docs/docs_test.go` with:
...
In `docs/superpowers/specs/2026-04-19-gormes-landing-page-design.md`, replace the old `.io` scope/purpose references with `.ai`:
