...
// ledgerlog.DriftPromotion entries. Backward transitions are logged via the standard
...
func DiffSubphaseStates(before, after *progress.Progress) []ledgerlog.DriftPromotion {
	if before == nil || after == nil {
		return nil
	}

	var promotions []ledgerlog.DriftPromotion
	for phaseID, afterPhase := range after.Phases {
		...
		for subphaseID, afterSubphase := range afterPhase.Subphases {
			...
			promotions = append(promotions, ledgerlog.DriftPromotion{
				SubphaseID: id,
				From:       from,
				To:         to,
				Reason:     reason,
