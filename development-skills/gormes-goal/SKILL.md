...
Default repository: the current Gormes checkout (`git rev-parse --show-toplevel`).
...
4. Use logical progress rows as the canonical implementation inventory. Parity evidence docs support source classification; they are not a side backlog. Do not create side TODO queues.
...
| Creating a side backlog | Put implementation intent into logical progress rows through the planner/progress tooling. |
