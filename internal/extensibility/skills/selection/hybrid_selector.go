...
type ScoredSkill struct {
	Skill         document.Skill `json:"skill"`
...
func (s *HybridSelector) Select(skills []document.Skill, query string, max int) []ScoredSkill {
	if len(skills) == 0 {
		return nil
	}
	if max <= 0 {
...
func (s *HybridSelector) SelectNames(skills []document.Skill, query string, max int) []string {
...
func (s *HybridSelector) filterCandidates(skills []document.Skill) []document.Skill {
	out := make([]document.Skill, 0, len(skills))
...
func (s *HybridSelector) score(sk document.Skill, tokens []string, query string) ScoredSkill {
...
func sourceAdjustment(sk document.Skill) (boost float64, damping float64) {
