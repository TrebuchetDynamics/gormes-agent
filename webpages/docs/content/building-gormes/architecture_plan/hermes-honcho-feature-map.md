...
| Navivox HTTP/WS Flutter channel | Gormes-owned Flutter/Navivox planning docs; current Go channel in `internal/channels/navivox/channel.go`; current CLI setup surface in `cmd/gormes/navivox_connect_info.go` and `cmd/gormes/setup_navivox.go`; current Flutter app repo at `/home/xel/git/sages-openclaw/workspace-mineru/navivox-app/app`; Hermes gateway principles from `gateway/run.py` and `gateway/platforms/base.py` | `cmd/gormes`, `internal/channels/navivox`, Flutter app in sibling repo `/home/xel/git/sages-openclaw/workspace-mineru/navivox-app/app` | Treat Navivox as a Gormes-owned first-party channel over HTTP/WebSocket, not a Hermes platform parity port. Current slices expose `gormes navivox connect`, setup QR PNG handoff under `$GORMES_HOME/navivox/pairing.png`, `/healthz`, `/v1/navivox/status`, `/v1/navivox/sessions`, `/v1/navivox/turn`, and `/v1/navivox/stream` with token redaction, exposure validation, and safe JSON events. Next slices should prove the connect-and-talk first screen, natural-language agent seed drafts, structured tool cards, schema-driven config admin, voice run records, and BYO STT/TTS profiles before call-center breadth. | Phase 9.E `Remove SSH Navivox stdio path`, `Navivox setup QR image pairing handoff`; Phase 9.F `Navivox HTTP/WS documentation refresh`, `Navivox connect-and-talk first screen` |
