...
func newRootCommandWithRuntime(runtime rootRuntime) *cobra.Command {
	...
	root.AddCommand(doctorCmd, versionCmd, telegramCmd, gatewayCmd, sessionCmd, memoryCmd, gonchoCmd, newAgentCommand(), newUsageCommand(), newAuthCommand(), newLogoutCommand(), newConfigCommand(), newMigrateCommand(), newProfileCommand(), newModelCommand(), newSetupCommand(), newMCPCommand(), newDashboardCommand(), newUninstallCommand())
	return root
}
