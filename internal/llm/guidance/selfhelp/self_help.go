...
// GuidanceForPrompt returns the deterministic self-help prompt block only for
// user prompts that ask about operating Gormes itself.
func GuidanceForPrompt(userPrompt string) (string, bool) {
	if !shouldUseGuidance(userPrompt) {
		return "", false
	}
	return gormesSelfHelpGuidanceBlock, true
}

func shouldUseGuidance(userPrompt string) bool {
