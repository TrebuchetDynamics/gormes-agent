...
func NewStopCommand(opts Options) *cobra.Command {
	cmd := &cobra.Command{
		...
		SilenceUsage: true,
		RunE:         func(cmd *cobra.Command, args []string) error { return runGatewayStop(cmd, args, opts) },
	}
...
func runGatewayStop(cmd *cobra.Command, _ []string, opts Options) error {
	ctx := cmd.Context()
	if ctx == nil {
	...
	if err := markerStore.Write(ctx, runtimegateway.PlannedStopMarker{
		TargetPID:       pid,
...
func gatewayStopStartTime(snapshot runtimegateway.RuntimeStatusSnapshot) int64 {
...
func waitForGatewayStop(ctx context.Context, store gatewayStopRuntimeStore) (runtimegateway.RuntimeProcessValidation, error) {
	...
	for {
		...
		select {
		case <-ctx.Done():
			return runtimegateway.RuntimeProcessValidation{}, fmt.Errorf("gateway stop: timed out waiting for pid=%d to exit", gatewayStopPID(snapshot))
		case <-ticker.C:
		}
	}
}
