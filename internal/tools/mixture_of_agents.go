...
const (
	MixtureOfAgentsName     = "mixture_of_agents"
	moaDefaultTimeout       = 120 * time.Second
	moaDefaultMinReferences = 2
	moaDefaultMaxReferences = 5
)

var ErrMOAInsufficientReferences = errors.New("mixture_of_agents: insufficient successful references")
...
func (t *MOATool) Execute(ctx context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	if in.Prompt == "" {
		return nil, errors.New("mixture_of_agents: prompt is required")
	}

	models := in.Models
	if len(models) == 0 {
		return nil, errors.New("mixture_of_agents: at least one reference model is required")
	}
