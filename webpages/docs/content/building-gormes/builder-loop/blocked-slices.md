...
|---|---|---|---|---|
| 5 / 5.G | Hermes MCP catalog install/configure lifecycle | Hermes approved MCP catalog manifest + list command, Gormes MCP catalog HTTP config install + default loader, Gormes MCP custom HTTP config add + redacted list, Gormes MCP config remove with explicit confirmation, Go-native MCP HTTP probe + test command, Go-native configured MCP HTTP registry + invocation, Gormes MCP server enable/disable persistence | The approved manifest/list row is complete and validated., Planner splits config-only install from pinned-git/bootstrap execution if one fixture-ready row cannot own both safely., The config-only HTTP child is complete before planner creates secret, git/bootstrap, probe/tool-selection, uninstall, enable/disable, or picker children., The custom HTTP add/redacted list child is complete before custom stdio/header/secret, probe/test/configure, remove/token-cleanup, enable/disable, or picker children., The explicit-confirmation config-remove child is complete before any durable OAuth-token or catalog-artifact cleanup child., The Go-native bounded HTTP probe/test child is complete before stdio/OAuth/configure probe behavior is split., The configured HTTP registry/invocation tracer bullet is complete before long-lived session/reload/stdio/OAuth runtime children are split. | - |
