...
func TestToolFilter_ChannelTrustToolsetRestriction(t *testing.T) {
	...
	scoped := ToolDeclaration{
		...
		TrustClass: []TrustClass{TrustClassOperator},
		Toolset:    []string{"ops"},
	}
	other := ToolDeclaration{
		...
		TrustClass: []TrustClass{TrustClassSystem},
	...
	wrongTrust := ToolFilter{
		Channel:    "slack",
		TrustClass: TrustClassChildAgent,
		Toolset:    []string{"ops"},
	}
...
func TestMCPHostAudit_RecordsServerToolNameAndRedactionStatus(t *testing.T) {
	...
	host.tools["honcho_chat"] = fakeMCPTool{
		...
		result: MCPResult{Status: MCPResultStatusOK, Body: []byte(`{"ok":true}`)},
	}
	auditor := &fakeMCPAuditor{}
	filter := ToolFilter{Channel: "slack", TrustClass: TrustClassOperator}
	...
	if res.Status != MCPResultStatusOK {
	...
	if ev.Status != MCPResultStatusOK {
	...
	host.tools["broken"] = fakeMCPTool{
		...
		result: MCPResult{Status: MCPResultStatusError, Reason: "boom"},
	}
	auditor.events = nil
	...
	ev = auditor.events[0]
	if ev.Status != MCPResultStatusError {
...
func TestMCPHostAudit_UnavailableEvidence(t *testing.T) {
	...
	host.tools["awol"] = fakeMCPTool{
		...
		result: MCPResult{
			Status: MCPResultStatusUnavailable,
	...
	if !strings.Contains(res.Reason, MCPHostUnavailableEvidence) {
		t.Errorf("reason must contain unavailable evidence %q, got %q", MCPHostUnavailableEvidence, res.Reason)
	}
	...
	if ev.Status != MCPResultStatusUnavailable {
		t.Errorf("event status = %q, want unavailable", ev.Status)
	}
	if !strings.Contains(ev.Reason, MCPHostUnavailableEvidence) {
...
// TestMCPHost_NoNanobotImport asserts via go/parser that mcp_host.go does not
// import any nanobot config package or vendor a full MCP framework.
func TestMCPHost_NoNanobotImport(t *testing.T) {
	t.Parallel()

	fset := token.NewFileSet()
	file, err := parser.ParseFile(fset, "mcp_host.go", nil, parser.ImportsOnly)
	if err != nil {
		t.Fatalf("parse mcp_host.go: %v", err)
	}
	forbidden := []string{
		"nanobot",
	...
	for _, imp := range file.Imports {
		...
		for _, banned := range forbidden {
			if strings.Contains(path, banned) {
				t.Errorf("mcp_host.go must not import %q (banned substring %q)", path, banned)
			}
		}
	}
}
...
func (f *fakeMCPHost) Invoke(_ context.Context, server, tool string, _ map[string]any) (MCPResult, error) {
	t, ok := f.tools[tool]
	if !ok {
		return MCPResult{Status: MCPResultStatusUnavailable, Reason: "no such tool"}, nil
	}
	if t.decl.ServerName != server {
		return MCPResult{Status: MCPResultStatusError, Reason: "server mismatch"}, nil
	}
	return t.result, nil
}
...
func (f *fakeMCPAuditor) Record(ev MCPAuditEvent) {
