...
Module path prefix: `github.com/TrebuchetDynamics/gormes-agent`.
...
In `internal/config/config.go`, add the new type alongside the other `*Cfg` types (e.g. immediately after `CronCfg`):
...
// internal/tools.Tool interface and is registered when
...
```bash
git add docs/content/building-gormes/architecture_plan/_index.md \
