...
| Operator tools | `tools/{todo_tool,clarify_tool,session_search_tool,send_message_tool,debug_helpers,interrupt,ansi_strip}.py` + `TodoStore`, `_ThreadAwareEventProxy` classes | 5.N | 🔨 in progress — `internal/tools/clarify.go` now ports the clarify-tool core with validated question/choice payloads, 120s timeout, structured replies, and dormant default-registry wiring; `internal/tools/debug_helpers.go` now ports the shared redacting `DebugSession` helper with explicit-save persistence under `${XDG_DATA_HOME}/gormes/logs`; `internal/tools/session_search.go` now ports the Go-native `session_search` operator core with a host-injected `SessionSearchBackend` seam, deterministic mode/hits JSON, `[1, 5]` limit clamping, source-allowlist normalization, current-session exclusion, and dormant default-registry wiring; sibling operator tools (`todo`, `send_message`, `cronjob`, `interrupt`) remain planned |
