...
import (
	...
	skillcatalog "github.com/TrebuchetDynamics/gormes-agent/internal/extensibility/skills"
...
func NewSkillsTools(cfg SkillsToolsConfig) []toolkit.Tool {
	return []toolkit.Tool{
...
func (t *SkillViewTool) readSkill(ctx context.Context, doc skillToolDoc) skillViewToolResult {
	...
	rendered, err := skillcatalog.PreprocessSkillContent(ctx, content, skillcatalog.PreprocessOptions{SkillDir: skillDir})
	if err == nil {
		content = rendered
	}
...
func normalizeSkillsToolsConfig(cfg SkillsToolsConfig) SkillsToolsConfig {
	if cfg.Root == "" {
		cfg.Root = skillcatalog.DefaultRoot()
	}
	if cfg.MaxDocumentBytes <= 0 {
		cfg.MaxDocumentBytes = skillcatalog.DefaultMaxDocumentBytes
	}
	return cfg
}
...
func loadSkillToolDocs(cfg SkillsToolsConfig) ([]skillToolDoc, error) {
	...
	if cfg.Root != "" {
		store := skillcatalog.NewStore(cfg.Root, cfg.MaxDocumentBytes)
		...
		for _, skill := range snapshot.Skills {
			...
			docs = append(docs, skillToolDoc{
				...
				Source:   source,
				Trust:    trust,
				Status:   string(skillcatalog.SkillStatusEnabled),
			})
		}
	}
	if strings.TrimSpace(cfg.BundledRoot) != "" {
		bundled, err := skillcatalog.LoadSkillDocs(cfg.BundledRoot, cfg.MaxDocumentBytes)
		if err != nil {
			return nil, err
		}
...
func detectSkillEnvVars(sk skillcatalog.Skill) ([]envVarInfo, []string) {
