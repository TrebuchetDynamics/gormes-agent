...
// at configschema.CurrentConfigVersion with non-empty endpoint+model returns no issues
...
`)
	...
	if report.ConfigVersion != configschema.CurrentConfigVersion {
		t.Fatalf("ConfigVersion = %d, want %d", report.ConfigVersion, configschema.CurrentConfigVersion)
	}
	if report.LatestVersion != configschema.CurrentConfigVersion {
		t.Fatalf("LatestVersion = %d, want %d", report.LatestVersion, configschema.CurrentConfigVersion)
	}
...
// configschema.CurrentConfigVersion, Check returns a non-nil error and reports the field.
