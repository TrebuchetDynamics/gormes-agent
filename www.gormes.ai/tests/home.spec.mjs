...
test('homepage renders the redesigned landing', async ({ page }) => {
  ...
  await expect(page.getByText('gormes --offline', { exact: true })).toBeVisible();
  await expect(page.getByText('gormes doctor --offline', { exact: true })).toBeVisible();
  await expect(page.getByText('gormes goncho doctor --json', { exact: true })).toBeVisible();
