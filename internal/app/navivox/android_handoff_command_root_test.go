...
`)
	...
	stdout, stderr, err := executeNavivoxCommandForTest(NewCommand(CommandOptions{VPNHostList: vpnhostList}), "pair", "--host", "127.0.0.1", "--port", strconv.Itoa(port), "--no-wait", "--open-navivox")
	if err != nil {
...
func TestNavivoxPairPrintDeeplinkRequiresExplicitFlag(t *testing.T) {
	...
	port = freeLocalTCPPort(t)
	stdout, stderr, err = executeNavivoxCommandForTest(NewCommand(CommandOptions{VPNHostList: vpnhostList}), "pair", "--host", "127.0.0.1", "--port", strconv.Itoa(port), "--no-wait", "--print-deeplink")
	if err != nil {
...
`)
	...
	if err := runNavivoxConnectInfoForConfigWithOptions(cmd, cfg, ConnectInfoOptions{openNavivox: true, androidPackage: navivoxAndroidPackage}); err != nil {
