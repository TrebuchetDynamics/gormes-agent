...
func TestDecodeOGGToPCM_EmptyInput_ReturnsError(t *testing.T) {
	_, err := DecodeOGGToPCM(context.Background(), nil, AudioDecodeConfig{})
	if err == nil {
...
func TestDecodeOGGToPCM_NonOGGInput_ReturnsConvertError(t *testing.T) {
	samples, err := DecodeOGGToPCM(context.Background(), []byte("not-ogg-data"), AudioDecodeConfig{})
	if err == nil {
...
func TestDecodeOGGToPCM_FFmpegMissingEvidence(t *testing.T) {
	_, err := DecodeOGGToPCM(context.Background(), []byte("OggS\x00fake"), AudioDecodeConfig{})
	if err == nil {
