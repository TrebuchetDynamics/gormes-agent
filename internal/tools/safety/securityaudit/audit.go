...
func (b *securityAuditBuilder) auditShellBlocklist() {
	...
	for _, probe := range probes {
		result := shellblocklist.CheckShellBlocklist(probe)
		if result.Blocked {
			continue
		}
		b.addFinding(SecurityAuditFinding{
	...
	coverage := shellblocklist.GetBlocklistCoverage()
	for _, category := range []string{
		string(shellblocklist.BlocklistDestructive),
		string(shellblocklist.BlocklistNetwork),
		string(shellblocklist.BlocklistPrivilege),
		string(shellblocklist.BlocklistCryptoMine),
		string(shellblocklist.BlocklistDataExfil),
	} {
...
func (b *securityAuditBuilder) auditFilesystemScoping() {
	...
	scope := filesystem.NewFilesystemScope(cwd, fs.ReadPaths, fs.WritePaths)
