...
func TestHermesRunnerStreamsToCompletion(t *testing.T) {
	...
	mc.Script([]llm.Event{
		...
		{Kind: llm.EventToken, Token: "world"},
...
func TestHermesRunnerExecutesToolCallsAndContinues(t *testing.T) {
	...
	runner := NewHermesRunner(mc, "test-model", []llm.ToolDescriptor{{Name: "echo", Description: "echo", Schema: json.RawMessage(`{"type":"object"}`)}})
	cfg := SubagentConfig{
		Goal:          "use tool",
		MaxIterations: 4,
		EnabledTools:  []string{"echo"},
...
func (c erringClient) OpenStream(context.Context, llm.ChatRequest) (llm.Stream, error) {
	return nil, c.openErr
}
func (c erringClient) OpenRunEvents(context.Context, string) (llm.RunEventStream, error) {
