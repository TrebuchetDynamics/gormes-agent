...
func TestShellBlocklistPatterns(t *testing.T) {
	if len(safety.ShellBlocklistPatterns) < 36 {
		t.Fatalf("ShellBlocklistPatterns has %d patterns, want >= 36", len(safety.ShellBlocklistPatterns))
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			blocked := safety.IsShellCommandBlocked(tc.cmd)
