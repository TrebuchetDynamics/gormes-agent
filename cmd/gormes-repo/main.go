// Command gormes-repo wraps the internal repoctl package as a standalone
...
const usage = "usage: gormes-repo [--repo-root <path>] {benchmark record|readme update|progress seed <fleet|missing-all>|hermes-source-pairs validate|hermes-source-pairs sync-sha|hermes-source-pairs report|hermes-contract-inventory}"
...
func run(stdout, stderr io.Writer, args []string) error {
	...
	case len(args) == 3 && args[0] == "progress" && args[1] == "seed":
		...
		_, err = fmt.Fprintf(stdout, "gormes-repo: progress seed %s added=%d skipped=%d total_items=%d\n",
	...
	case len(args) >= 2 && args[0] == "hermes-source-pairs" && args[1] == "validate":
		...
		_, err = fmt.Fprintf(stdout, "gormes-repo: hermes-source-pairs validate ok pairs=%d hermes_sha=%s\n",
	...
	case len(args) >= 2 && args[0] == "hermes-source-pairs" && args[1] == "sync-sha":
		...
		_, err = fmt.Fprintf(stdout, "gormes-repo: hermes-source-pairs sync-sha ok pairs=%d hermes_sha=%s changed=%d demoted=%d\n",
	...
	case len(args) >= 2 && args[0] == "hermes-source-pairs" && args[1] == "report":
		...
		_, err = fmt.Fprintln(stdout, "gormes-repo: hermes-source-pairs report updated")
		return err
	case len(args) >= 1 && args[0] == "hermes-contract-inventory":
		...
		_, err = fmt.Fprintf(stdout, "gormes-repo: hermes-contract-inventory report updated json=%s markdown=%s hermes_sha=%s strict_failures=%d\n",
		...
		if opts.Strict && !result.Report.OK {
			return fmt.Errorf("gormes-repo: hermes-contract-inventory strict failed strict_failures=%d", result.Report.Summary.StrictFailures)
		}
		return nil
	default:
		return fmt.Errorf("%w\n%s", errParse, usage)
