...
// UsageStatePath returns the sidecar state path for root.
func UsageStatePath(root string) string { return usageStatePath(root) }

// LoadUsageState reads the sidecar usage state for root.
func LoadUsageState(root string) (map[string]SkillUsageRecord, error) { return loadUsageState(root) }

// SaveUsageState writes the sidecar usage state for root.
func SaveUsageState(root string, state map[string]SkillUsageRecord) error {
	return saveUsageState(root, state)
}

// UpdateUsageRecord updates one sidecar usage record.
func UpdateUsageRecord(root, name string, fn func(*SkillUsageRecord)) error {
	return updateUsageRecord(root, name, fn)
}

// MoveSkillDirToArchive moves a skill directory into the active archive folder.
func MoveSkillDirToArchive(root, name, skillDir string, now time.Time) (string, time.Time, error) {
	return moveSkillDirToArchive(root, name, skillDir, now)
}
