...
| Provider registry, aliases, and auth modes | `hermes_cli/providers.py:HERMES_OVERLAYS,ALIASES`, `agent/models_dev.py:PROVIDER_TO_MODELS_DEV`, `hermes_cli/main.py:--provider choices`, `hermes_cli/models.py` | `internal/hermes`, `internal/config`, `cmd/gormes` | Add a source-backed Go manifest before claiming provider coverage. It must classify every canonical provider ID, alias, transport family, auth type, env/base-url rule, and models.dev mapping as implemented, row-backed, excluded, or owned. Unsupported providers remain visible to `gormes model`, status, and docs instead of disappearing because no runtime adapter exists yet. | Phase 4.A `Hermes provider registry and alias manifest` |
...
| Browser and web research | `tools/browser_*.py`, `tools/browser_providers/*.py`, `hermes_cli/browser_connect.py`, `tools/web_tools.py`, sibling `../browser-harness/SKILL.md` | `internal/tools`, future browser package, `docs/development-skills/gormes-browser-harness` | First freeze action/event transcripts; then wire Browser Use/browser-harness, Browserbase/Firecrawl, CDP/browser-connect, and chromedp/rod runtime bridges behind the Hermes `browser_*` tool surface. | Phase 5.C |
