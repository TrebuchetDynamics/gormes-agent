...
func NewUpdateCommand(options UpdateCommandOptions) *cobra.Command {
	return newUpdateCommandWithSeams(updateCommandSeams{BuildProvenance: options.BuildProvenance})
}
...
func newUpdateCommandWithSeams(seams updateCommandSeams) *cobra.Command {
	...
	if seams.BuildProvenance == nil {
		seams.BuildProvenance = defaultUpdateBuildProvenance
	}
	...
	if seams.ReleaseSkillProfiles == nil {
		seams.ReleaseSkillProfiles = DefaultSkillProfileRoots
	}
...
`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			if rollbackSnapshot != "" {
				...
				if asJSON {
					if err := printUpdateReleaseBinaryReportJSON(cmd, seams.BuildProvenance, "update_rollback", report); err != nil {
						return err
					}
				} else {
			...
			if dryRun || (checkOnly && installKind == cli.UpdateInstallKindRelease) {
				return runUpdateReleasePlannerMode(cmd, updateReleasePlannerModeOptions{
					...
					BuildProvenance:      seams.BuildProvenance,
				})
			}
			...
			if !checkOnly {
				...
				defer closeLog()
				appendUpdateLedgerEvidence(&report, restartGateway, seams.BuildProvenance)
			}
			if asJSON {
				if err := printUpdateReportJSON(cmd, seams.BuildProvenance, report); err != nil {
					return err
				}
			} else {
...
func runUpdateReleasePlannerMode(cmd *cobra.Command, opts updateReleasePlannerModeOptions) error {
	plan, channelBlocker := buildUpdateReleaseCommandPlan(cmd.Context(), updateReleaseCommandPlanOptions{
		Current:              updateBuildIdentity(opts.BuildProvenance),
	...
	if opts.AsJSON {
		if err := printUpdateReleasePlanJSON(cmd, opts.BuildProvenance, opts.Action, plan, failed); err != nil {
			return err
		}
	} else {
...
type updateReleaseCommandPlanOptions struct {
	Current              cli.UpdateBuildIdentity
	InstallKind          cli.UpdateInstallKind
	Channel              cli.UpdateReleaseChannel
	RuntimePlatform      func() (string, string)
	LoadReleaseMetadata  func(context.Context, cli.UpdateReleaseChannel) (cli.UpdateReleaseMetadata, error)
...
func buildUpdateReleaseCommandPlan(ctx context.Context, opts updateReleaseCommandPlanOptions) (cli.UpdateReleasePlan, *cli.UpdateReleaseBlocker) {
	...
	if strings.TrimSpace(current.Version) == "" {
		current = cli.UpdateBuildIdentity{Version: defaultUpdateBuildProvenance().Version, GitCommit: defaultUpdateBuildProvenance().GitCommit}
	}
...
func runUpdateReleaseBinaryMode(cmd *cobra.Command, opts updateReleaseBinaryModeOptions) error {
	...
	if len(plan.Blockers) > 0 {
		if opts.AsJSON {
			if err := printUpdateReleasePlanJSON(cmd, opts.BuildProvenance, "update_release_binary", plan, true); err != nil {
				return err
			}
		} else {
	...
	if !plan.UpdateAvailable {
		report := runUpdateReleaseWithServiceCoordination(cmd.Context(), opts, func(ctx context.Context) cli.UpdateReleaseBinaryReport {
			report := cli.UpdateReleaseBinaryReport{
				...
				Evidence: []cli.UpdateEvidence{{
					...
					Detail: fmt.Sprintf("already current at %s", current.Version),
				}},
			}
		...
		if updateReleaseReportHasMutationEvidence(report) {
			appendUpdateReleaseLedgerEvidence(&report, opts.BuildProvenance)
		}
		if opts.AsJSON {
			if err := printUpdateReleaseBinaryReportJSON(cmd, opts.BuildProvenance, "update_release_binary", report); err != nil {
				return err
			}
		} else {
...
func appendUpdateReleaseLedgerEvidence(report *cli.UpdateReleaseBinaryReport, buildFn func() BuildProvenance) {
	path, err := appendUpdateReleaseLedger(*report, buildFn)
	if err != nil {
...
func printUpdateReleasePlanJSON(cmd *cobra.Command, buildFn func() BuildProvenance, action string, plan cli.UpdateReleasePlan, failed bool) error {
	body, err := json.MarshalIndent(updateReleasePlanReportJSON{
		Build:  updateBuildProvenance(buildFn),
		Action: action,
		Failed: failed,
		Plan:   plan,
	}, "", "  ")
...
func printUpdateReleaseBinaryReportJSON(cmd *cobra.Command, buildFn func() BuildProvenance, action string, report cli.UpdateReleaseBinaryReport) error {
...
func defaultUpdateBuildProvenance() BuildProvenance {
	return BuildProvenance{Version: "0.0.0", GitCommit: "unknown"}
}

func updateBuildProvenance(buildFn func() BuildProvenance) BuildProvenance {
	if buildFn == nil {
		buildFn = defaultUpdateBuildProvenance
	}
	build := buildFn()
	if strings.TrimSpace(build.Version) == "" {
		build.Version = defaultUpdateBuildProvenance().Version
	}
	if strings.TrimSpace(build.GitCommit) == "" {
		build.GitCommit = defaultUpdateBuildProvenance().GitCommit
	}
	return build
}

func updateBuildIdentity(buildFn func() BuildProvenance) cli.UpdateBuildIdentity {
	build := updateBuildProvenance(buildFn)
	version := strings.TrimSpace(build.Version)
	if version == "" {
		version = defaultUpdateBuildProvenance().Version
	}
	commit := strings.TrimSpace(build.GitCommit)
	if commit == "" {
		commit = defaultUpdateBuildProvenance().GitCommit
	}
	return cli.UpdateBuildIdentity{Version: version, GitCommit: commit}
}
...
func appendUpdateLedgerEvidence(report *cli.UpdateReport, restartGateway string, buildFn func() BuildProvenance) {
	path, err := appendUpdateLedger(*report, restartGateway, buildFn)
	if err != nil {
		report.Evidence = append(report.Evidence, cli.UpdateEvidence{Kind: cli.UpdateEvidenceLedgerUnavailable, Detail: err.Error()})
		return
	}
...
func appendUpdateLedger(report cli.UpdateReport, restartGateway string, buildFn func() BuildProvenance) (string, error) {
...
func appendUpdateReleaseLedger(report cli.UpdateReleaseBinaryReport, buildFn func() BuildProvenance) (string, error) {
	path, err := resolveUpdateLedgerPath()
	if err != nil {
		return "", err
	}
...
func printUpdateReportJSON(cmd *cobra.Command, buildFn func() BuildProvenance, report cli.UpdateReport) error {
	out := cmd.OutOrStdout()
...
func printCuratorRecentRunNotice(cmd *cobra.Command) {
	curatorDeps := CuratorCommandDeps{}
	root := ResolveCuratorSkillsRoot(curatorDeps)
	...
	out := cmd.OutOrStdout()
	fmt.Fprintf(out, "\nℹ Skill curator — last run %s\n", FormatCuratorTimestamp(state.LastRunAt, curatorDeps))
...
func ResolveManagedCheckoutDir() (string, error) {
	return resolveManagedCheckoutDir()
}
