...
| Memory prefetch/sync | `agent/memory_manager.py` | `internal/memory/lifecycle/provider.go:18`; `internal/kernel/turn_request_assembly.go:52`; `internal/memory/provider_lifecycle.go:KernelPrefetchAdapter` | partial | `LifecycleProvider.Prefetch` interface and `MemoryProviderLifecycle.Prefetch` fan-out exist. Kernel wires `MemoryPrefetcher` injection (Config.MemoryPrefetch): called before each LLM request, result appended as system message. `memory.KernelPrefetchAdapter` bridges the signature. `SyncTurn` post-turn hook interface exists but not yet called from kernel post-turn path. 355 tests pass. |
