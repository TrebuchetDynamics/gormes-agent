...
|---|---|---|---|---|---|
| 2 / 2.B.5 | Session metadata manual-title protection flag | Persist a manual-title flag on session.Metadata, expose a concrete SessionTitleStore adapter that surfaces it to PerformAutoTitle, and wire the gateway /title set path to mark titles as manual so future auto-title generation cannot overwrite operator-set names. | gateway, system | `internal/session/title_store_test.go` | P0 handoff; needs contract proof before closeout. |
