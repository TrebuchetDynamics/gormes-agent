...
**Files:**
- Create: `gormes/go.mod`, `gormes/.gitignore`, `gormes/Makefile`, `README.md`
...
Verify `go.mod` contains `module github.com/TrebuchetDynamics/gormes-agent` and `go 1.22`.
...
- [ ] **Step 1:** Create `docs/ARCH_PLAN.md`. Full content — paste exactly:
...
- [ ] **Step 2:** Create `docs/docs_test.go`:
...
```bash
cd ..
git add docs/docs_test.go gormes/go.mod gormes/go.sum
...
- [ ] **Step 1:** Create `internal/pybridge/pybridge.go`:
...
**Files:**
- Create: `internal/telemetry/telemetry.go`, `telemetry_test.go`

- [ ] **Step 1:** Create `internal/telemetry/telemetry_test.go`:
...
- [ ] **Step 2:** Implement `internal/telemetry/telemetry.go`:
...
**Files:**
- Create: `internal/kernel/kernel.go`, `frame.go`, `admission.go`, `provenance.go`, `kernel_test.go`
...
- [ ] **Step 2:** Create `internal/kernel/frame.go`:
...
- [ ] **Step 3:** Create `internal/kernel/admission.go`:
...
- [ ] **Step 4:** Create `internal/kernel/provenance.go`:
...
- [ ] **Step 5:** Create `internal/kernel/kernel.go`:
...
- [ ] **Step 1:** Write the ten tests. Create `internal/kernel/kernel_test.go`:
...
```bash
cd ..
git add internal/kernel/kernel_test.go internal/kernel/kernel.go
...
**Files:**
- Create: `internal/tui/model.go`, `view.go`, `update.go`
...
- [ ] **Step 4:** Create `internal/tui/update.go`:
...
cd .. && git add internal/tui/ gormes/go.mod gormes/go.sum
...
// TestNoUnboundedChannels walks every Go source file under internal/
...
```bash
cd ..
git add internal/discipline_test.go
...
Edit `docs/ARCH_PLAN.md` §4 — change `Phase 1 — The Dashboard (Face) | 🔨 in progress` to `| ✅ complete`. Commit.
