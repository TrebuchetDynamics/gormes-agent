...
|---|---|---|---|---|
| 2 / 2.B.12 | Hermes-style default prompt and image-path hints for inbound photos | Multimodal photo attachment passthrough | Multimodal photo attachment passthrough (2.B.12 row 6) is complete (true at HEAD = 0a7f8d02d)., imageContentPartsFromAttachments helper exists and is wired (true). | - |
| 4 / 4.A | Vision-unsupported provider retry (strip-images-and-resend) | Multimodal photo attachment passthrough | Multimodal photo attachment passthrough (2.B.12 row 6) is complete (true at HEAD = 0a7f8d02d)., image_shrink_retry.go pattern is stable and tests are green (true). | - |
