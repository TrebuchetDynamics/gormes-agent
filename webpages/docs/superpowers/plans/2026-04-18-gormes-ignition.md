...
```bash
cd gormes
go mod init github.com/TrebuchetDynamics/gormes-agent
```

Expected: `go.mod` is created with `module github.com/TrebuchetDynamics/gormes-agent` and `go 1.22` directive. Run `cat go.mod` to verify.
...
- [ ] **Step 5:** Create `README.md` (skeleton — the "Rosetta Stone" expansion lives in ARCH_PLAN.md):
...
```bash
cd ..
git add gormes/go.mod gormes/.gitignore gormes/Makefile README.md
git commit -m "feat(gormes): bootstrap Go module skeleton

Adds go.mod (github.com/TrebuchetDynamics/gormes-agent, go 1.22),
...
**Files:**
- Create: `docs/ARCH_PLAN.md`

- [ ] **Step 1:** Create `docs/ARCH_PLAN.md` with all six required subsections from spec §21.2. Use exactly this content:
...
- [ ] **Step 1:** Write the failing test. Create `docs/docs_test.go`:
...
```bash
cd ..
git add internal/config/ gormes/go.mod gormes/go.sum
...
- [ ] **Step 2:** Create `internal/db/migrations/0001_initial.sql`:
...
- [ ] **Step 3:** Write the failing test. Create `internal/db/db_test.go`:
...
- [ ] **Step 6:** Create `internal/db/queries.go` (query constants used by `session`):
...
```bash
cd ..
git add internal/db/ gormes/go.mod gormes/go.sum
...
- [ ] **Step 2:** Create `internal/provider/errors.go`:
...
- [ ] **Step 3:** Write a tiny test to lock behaviour. Create `internal/provider/errors_test.go`:
...
```bash
cd ..
git add internal/provider/provider.go internal/provider/errors.go internal/provider/errors_test.go
...
- Create: `internal/provider/mock_test.go`

- [ ] **Step 1:** Write the failing test. Create `internal/provider/mock_test.go`:
...
- [ ] **Step 3:** Implement `internal/provider/mock.go`:
...
```bash
cd ..
git add internal/provider/mock.go internal/provider/mock_test.go
...
- [ ] **Step 1:** Write the failing test using a `httptest` fixture. Create `internal/provider/openrouter_test.go`:
...
- [ ] **Step 3:** Implement `internal/provider/openrouter.go`:
...
- [ ] **Step 1:** Write the failing test. Create `internal/telemetry/telemetry_test.go`:
...
**Files:**
- Create: `internal/pybridge/pybridge.go`

- [ ] **Step 1:** Create `internal/pybridge/pybridge.go` (interface only; no implementation in M1):
...
```bash
cd ..
git add internal/tui/model.go gormes/go.mod gormes/go.sum
...
- [ ] **Step 1:** Create `internal/tui/update.go`:
...
- [ ] **Step 2:** Create `internal/tui/tui_test.go`:
...
```bash
cd ..
git add internal/tui/tui_test.go gormes/go.mod gormes/go.sum
...
**Files:**
- Create: `internal/provider/live_test.go`

- [ ] **Step 1:** Create `internal/provider/live_test.go`:
...
```bash
cd ..
git add internal/provider/live_test.go
...
- [ ] **Step 6:** Mark M0 and M1 as ✅ complete in `docs/ARCH_PLAN.md` section 4 (the milestone table) and commit.

```bash
# Edit docs/ARCH_PLAN.md — change:
