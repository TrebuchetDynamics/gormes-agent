...
No changes are planned under `internal/memory/` in this runtime-only slice. That avoids the current dirty files in that area.
...
In `internal/config/config.go`, add this struct near `CronCfg`:
