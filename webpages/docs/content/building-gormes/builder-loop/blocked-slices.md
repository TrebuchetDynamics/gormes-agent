...
|---|---|---|---|---|
| 5 / 5.O | gormes setup profiles — per-profile channels (telegram/whatsapp/discord/slack) | gormes setup profiles — section scaffold + per-profile workspace list | Child 1 (section scaffold + per-profile workspace list) shipped; the per-profile channel config shape confirmed and the runSetupBindingsSection reuse boundary decided (refine this row first). | - |
