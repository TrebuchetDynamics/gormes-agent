...
func TestUpdateCommandReleaseDryRunPrintsPlanWithoutLifecycleMutation(t *testing.T) {
	...
	stdout, stderr, err := executeUpdateCommandForTest(command, "--dry-run", "--json")
	if err != nil {
	...
	if got.Plan.Current.Version != currentUpdateVersionForTest() || got.Plan.Target.Version != targetVersion || got.Plan.Target.GitCommit != "newsha" {
...
func TestUpdateCommandCheckReleaseExitCodesAndJSON(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			target:     cli.UpdateReleaseMetadata{Version: currentUpdateVersionForTest(), Tag: "v" + currentUpdateVersionForTest(), GitCommit: "main"},
			wantCode:   0,
			wantUpdate: false,
		},
		{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			stdout, stderr, err := executeUpdateCommandForTest(command, "--check", "--json")
