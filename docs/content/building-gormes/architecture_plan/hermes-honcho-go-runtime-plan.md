...
| `../browser-harness/SKILL.md`, `../browser-harness/install.md`, `../browser-harness/agent-workspace/**`, `../browser-harness/interaction-skills/**` | Browser Use/CDP operator workflow and browser interaction skill donor for Phase 5.C browser harness parity. |
...
| Browser, web, media, voice, image | `tools/browser_*.py`, `tools/browser_providers/**`, `hermes_cli/browser_connect.py`, `tools/web_tools.py`, `tools/vision_tools.py`, `tools/image_generation_tool.py`, `tools/tts_tool.py`, `tools/transcription_tools.py`, `tools/voice_mode.py`, `../browser-harness/SKILL.md` | `internal/tools`, future browser/media packages, `internal/hermes` content parts, `docs/development-skills/gormes-browser-harness` | mapped-by-contract; Browser Use/browser-harness and CDP/browser-connect are now row-backed separately from Browserbase/Firecrawl | Phase 5.C, 5.D, 5.E |
