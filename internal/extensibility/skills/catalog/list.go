...
const (
	SkillStatusEnabled             = availability.SkillStatusCode("enabled")
...
func installedSkillRows(root, bundledRoot string, externalRoots []string) []SkillRow {
	skills, err := loadSkillDocsFromDir(filepath.Join(root, "active"), document.DefaultMaxDocumentBytes)
	if err != nil {
		return nil
	}
	...
	for _, skill := range skills {
		row := activeSkillRow(activeDir, skill)
...
func bundledSkillRows(root string) []SkillRow {
	...
	skills, err := loadSkillDocsFromDir(root, document.DefaultMaxDocumentBytes)
	if err != nil {
		return nil
	}
...
func baseSkillRow(skill Skill) SkillRow {
	...
	if len(availability.MissingSkillCredentials(skill, nil)) > 0 {
...
func loadSkillDocsFromDir(root string, maxBytes int) ([]Skill, error) {
	...
	for _, path := range paths {
		...
		skill, err := document.Parse(raw, maxBytes)
		if err != nil {
			return nil, err
		}
		skill.Path = path
