...
func pinThreadedTurn(m *Manager, threadID string) {
	...
	m.setPinnedTurnSession("telegram:42:"+threadID, "sess-thread", SessionSource{
		Platform: "telegram",
		ChatID:   "42",
		ThreadID: threadID,
	})
...
func TestThreadAwareFinalPageUsesPinnedTurnThread(t *testing.T) {
	...
	if got[0].ChatID != "42" || got[0].ThreadID != "777" || !strings.Contains(got[0].Text, "threaded final answer") {
		t.Fatalf("thread send = %+v, want chat 42 thread 777 final text", got[0])
	}
}
...
func TestThreadAwareToolProgressUsesPinnedTurnThread(t *testing.T) {
	...
	if got[0].ChatID != "42" || got[0].ThreadID != "888" || !strings.Contains(got[0].Text, "terminal") {
...
func TestThreadAwareTypingActionUsesPinnedTurnThread(t *testing.T) {
	...
	if got[0] != (threadActionCall{ChatID: "42", ThreadID: "999", Action: "typing"}) {
		t.Fatalf("thread action = %+v, want chat 42 thread 999 typing", got[0])
	}
}
