...
func (*MoATool) Name() string { return contract.ToolName }
...
func (*MoATool) Execute(ctx context.Context, args json.RawMessage) (json.RawMessage, error) {
	var req MoARequest
	if len(args) == 0 {
		return nil, contract.Error("missing arguments")
	}
	if err := json.Unmarshal(args, &req); err != nil {
		return nil, contract.Errorf("invalid arguments: %w", err)
	}
	if req.Prompt == "" {
	...
	if err != nil {
		return nil, contract.Errorf("marshal error: %w", err)
	}
	return out, nil
}
