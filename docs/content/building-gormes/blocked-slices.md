...
|---|---|---|---|---|
| 1 / 1.C | Soft-success-nonzero bats coverage | Orchestrator failure-row stabilization for 4-8 workers | Orchestrator failure-row stabilization remains active and try_soft_success_nonzero is testable without running the full worker loop. | Orchestrator failure-row stabilization for 4-8 workers |
...
| 4 / 4.A | Tool-call normalization + continuation contract | Provider interface + stream fixture harness | Provider interface + stream fixture harness is available for cross-provider tool continuation fixtures. | Bedrock stream event decoding (SSE fixtures), OpenRouter, Codex |
...
| 4 / 4.H | Classified provider-error taxonomy | Provider-side resilience | Provider-side resilience remains active and error taxonomy fixtures can be split from retry-policy changes. | Retry-After header parsing + HTTPError hint, Provider rate guard + budget telemetry |
