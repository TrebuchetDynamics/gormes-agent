...
import (
	...
	"github.com/TrebuchetDynamics/gormes-agent/internal/llm/azurefoundry/apimode"
...
type AzureFoundryRuntime struct {
	...
	Context        routing.ModelContextResolution
	Probe          probehttp.Result
	Evidence       []string
	Degraded       bool
}
...
func ResolveAzureFoundryRuntime(in AzureFoundryRuntimeInput) AzureFoundryRuntime {
	...
	switch {
	case in.Config.APIMode != "" && in.Config.APIMode != transport.Unknown:
		out.APIMode = in.Config.APIMode
		out.APIModeSource = AzureFoundryAPIModeSourceConfig
	case in.Probe.Transport != "" && in.Probe.Transport != transport.Unknown:
		out.APIMode = in.Probe.Transport
		out.APIModeSource = AzureFoundryAPIModeSourceProbe
	default:
		out.APIMode = transport.Unknown
	}
	if out.APIMode != transport.Anthropic {
		if inferred, ok := apimode.ForModel(out.Model); ok {
	...
	if out.APIMode == transport.Anthropic && out.BaseURL != "" {
	...
	resolver := routing.NewModelContextResolver(in.ContextLookup)
	out.Context = resolver.Resolve(routing.ModelContextQuery{
...
func buildAzureFoundryEvidence(rt AzureFoundryRuntime, probe probehttp.Result) []string {
	...
	if rt.APIMode != "" && rt.APIMode != transport.Unknown {
	...
	case rt.APIMode != "" && rt.APIMode != transport.Unknown:
	...
	if probe.Transport != "" && probe.Transport != transport.Unknown {
