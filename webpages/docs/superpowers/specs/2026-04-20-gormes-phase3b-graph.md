...
- [`docs/ARCH_PLAN.md`](../../ARCH_PLAN.md) — Phase 3 = "The Black Box (Memory): SQLite + FTS5 + ontological graph in Go". 3.A delivered the foundation (SQLite + FTS5). This spec delivers the ontological graph layer + LLM-assisted population.
...
Extractor lives in `internal/memory/extractor.go` alongside the existing persistence `worker.go`. Rationale: both workers share the `*SqliteStore` handle and the same underlying connection pool; co-locating them keeps the single-writer discipline trivially enforceable.
