...
func TestProviderHTTPClient_UsesCodexOAuthCredentialPoolWhenEndpointEmpty(t *testing.T) {
	...
	client, err := NewHTTPClient(config.Config{Hermes: config.HermesCfg{
		Model:    "gpt-5.5",
...
func TestProviderHTTPClientWithCredentialHome_FallsBackToGlobalCodexOAuthWhenAgentHomeMissing(t *testing.T) {
	...
	client, err := NewHTTPClientWithCredentialHome(config.Config{Hermes: config.HermesCfg{
		Model:    "gpt-5.5",
		Provider: "openai-codex",
	}}, "openai-codex", agentHome)
	if err != nil {
		t.Fatalf("NewHTTPClientWithCredentialHome: %v", err)
	}
...
func TestProviderHTTPClientWithCredentialHome_FallsBackToGlobalCredentialPoolForAnyAuthKind(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			endpoint, apiKey, err := ResolveCredentialsWithHome(config.Config{Hermes: config.HermesCfg{
				Model:    "fixture-model",
				Provider: tt.provider,
			}}, tt.provider, agentHome)
			if err != nil {
				t.Fatalf("ResolveCredentialsWithHome: %v", err)
			}
...
func TestProviderHTTPClient_OpenRouterDefaultRuntimeUsesOpenRouterKey(t *testing.T) {
	...
	endpoint, apiKey, err := ResolveCredentials(config.Config{Hermes: config.HermesCfg{
