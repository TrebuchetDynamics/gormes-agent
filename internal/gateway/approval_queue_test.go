...
func TestApprovalQueueResolveOldestFIFO(t *testing.T) {
	...
	err = q.ResolveGatewayApproval(context.Background(), ApprovalResolution{
		SessionKey: "session-a",
		Choice:     ApprovalChoiceSession,
	})
	if err != nil {
	...
	if firstOutcome.Request.Command != "first" || firstOutcome.Choice != ApprovalChoiceSession || firstOutcome.Canceled {
...
func TestApprovalQueueResolveAll(t *testing.T) {
	...
	count, err := q.ResolveAllGatewayApprovals("session-a", ApprovalChoiceAlways)
	if err != nil {
	...
	for _, ticket := range []GatewayApprovalTicket{first, second} {
		...
		if outcome.Choice != ApprovalChoiceAlways || outcome.Canceled {
...
func TestApprovalQueueClearSessionCancelsPending(t *testing.T) {
	...
	for _, ticket := range []GatewayApprovalTicket{first, second} {
		...
		if outcome.Choice != ApprovalChoiceDeny || !outcome.Canceled {
...
func TestApprovalQueueInvalidChoiceAndEmptySessionFailClosed(t *testing.T) {
	...
	if err := q.ResolveGatewayApproval(context.Background(), ApprovalResolution{SessionKey: "session-a", Choice: ApprovalChoice("bad")}); !errors.Is(err, ErrGatewayApprovalInvalidChoice) {
		t.Fatalf("ResolveGatewayApproval invalid choice error = %v, want ErrGatewayApprovalInvalidChoice", err)
	}
	if _, err := q.ResolveAllGatewayApprovals("session-a", ApprovalChoice("bad")); !errors.Is(err, ErrGatewayApprovalInvalidChoice) {
		t.Fatalf("ResolveAllGatewayApprovals invalid choice error = %v, want ErrGatewayApprovalInvalidChoice", err)
	}
	if err := q.ResolveGatewayApproval(context.Background(), ApprovalResolution{SessionKey: "", Choice: ApprovalChoiceOnce}); !errors.Is(err, ErrGatewayApprovalEmptySession) {
		t.Fatalf("ResolveGatewayApproval empty session error = %v, want ErrGatewayApprovalEmptySession", err)
	}
	if err := q.ResolveGatewayApproval(context.Background(), ApprovalResolution{SessionKey: "session-a", Choice: ApprovalChoiceOnce}); !errors.Is(err, ErrGatewayApprovalNotPending) {
		t.Fatalf("ResolveGatewayApproval missing pending error = %v, want ErrGatewayApprovalNotPending", err)
	}
	if count, err := q.ResolveAllGatewayApprovals("session-a", ApprovalChoiceOnce); err != nil || count != 0 {
