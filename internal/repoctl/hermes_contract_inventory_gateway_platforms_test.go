				...
				Subphases: map[string]progress.Subphase{
					"1.A": {
						...
						Items: []progress.Item{
							...
							gatewayPlatformProgressItem("Gateway platform docs evidence", progress.StatusPlanned, progress.ContractStatusFixtureReady, "channels", "hermes-agent/gateway/platforms/ADDING_A_PLATFORM.md", "go test ./internal/repoctl -run TestHermesContractInventoryClassifiesGatewayPlatformSurface -count=1"),
