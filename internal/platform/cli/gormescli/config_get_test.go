...
func TestConfigGet_NonSecretReturnsValue(t *testing.T) {
	...
	if _, _, err := executeConfigCommandForTest(
		newConfigCommandForTest(),
		"set", "hermes.model", "gpt-test-model",
	); err != nil {
		t.Fatalf("config set: %v", err)
	}

	stdout, stderr, err := executeConfigCommandForTest(
		newConfigCommandForTest(),
		"get", "hermes.model",
	)
	if err != nil {
...
func TestConfigGet_SecretKeyEmitsRedactedStatus(t *testing.T) {
	...
	if _, _, err := executeConfigCommandForTest(
		newConfigCommandForTest(),
		"set", "hermes.api_key", "sk-VERY-SECRET-VALUE-do-not-leak",
	); err != nil {
...
func TestConfigGet_NoArgErrors(t *testing.T) {
	...
	_, stderr, err := executeConfigCommandForTest(
		newConfigCommandForTest(),
		"get",
	)
	if err == nil {
