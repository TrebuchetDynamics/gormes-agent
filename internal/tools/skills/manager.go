...
func NewSkillManagerTool(cfg SkillManagerToolConfig) toolkit.Tool {
...
func (t *SkillManagerTool) pinnedRefusal(name string) *skillManageResult {
	pinned, err := skillcatalog.IsPinned(t.cfg.Root, name)
	if err != nil || !pinned {
		return nil
	}
...
func (t *SkillManagerTool) recordSuccessfulMutation(action, name string) {
	...
	case "create":
		if t.cfg.WriteOrigin == SkillWriteOriginBackgroundReview {
			_ = skillcatalog.MarkAgentCreated(t.cfg.Root, name)
		}
	case "edit", "patch", "write_file", "remove_file":
		_ = skillcatalog.BumpPatch(t.cfg.Root, name)
	case "delete":
		_ = skillcatalog.ForgetUsageRecord(t.cfg.Root, name)
	}
}
...
func (t *SkillManagerTool) handleEdit(in skillManageArgs) skillManageResult {
	...
	if skillPath == "" {
		return skillManageResult{
			Success: false,
			Error:   fmt.Sprintf("Skill %q not found. Use skills_list() to see available skillcatalog.", name),
		}
	}
