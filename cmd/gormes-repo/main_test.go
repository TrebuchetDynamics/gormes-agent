...
func TestRunHermesContractInventoryWritesReports(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "gormes-repo: hermes-contract-inventory report updated") {
...
func TestRunHermesSourcePairsValidateRoutesToManifest(t *testing.T) {
  ...
  "pairs": [
	...
	if err := os.WriteFile(filepath.Join(root, "webpages/docs/content/building-gormes/architecture_plan/hermes-source-pairs.json"), []byte(manifest), 0o644); err != nil {
	...
	if !strings.Contains(stdout.String(), "gormes-repo: hermes-source-pairs validate ok pairs=1") {
...
func TestRunHermesSourcePairsSyncSHARoutesToManifest(t *testing.T) {
  ...
  "pairs": [
	...
	if !strings.Contains(stdout.String(), "gormes-repo: hermes-source-pairs sync-sha ok pairs=1 hermes_sha=def456") {
		t.Fatalf("stdout = %q, want sync summary", stdout.String())
	}
	raw, err := os.ReadFile(filepath.Join(root, "webpages/docs/content/building-gormes/architecture_plan/hermes-source-pairs.json"))
	if err != nil {
