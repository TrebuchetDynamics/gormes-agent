...
| 9 / 9.E | Navivox VPN host enumeration helper | Remove SSH Navivox stdio path | `Remove SSH Navivox stdio path` shipped; cmd/gormes no longer holds tailscale-only host detection., `go test ./... -count=1` is green on development. | Navivox HTTP gateway mandatory-VPN bind, Navivox HTTP gateway connect-info command |
...
| 9 / 9.E | Navivox HTTP gateway connect-info command | Navivox VPN host enumeration helper, Navivox HTTP gateway mandatory-VPN bind, Remove SSH Navivox stdio path | internal/network/vpnhost shipped., Navivox HTTP gateway mandatory-VPN bind shipped., Remove SSH Navivox stdio path shipped (so `gormes navivox` has no leftover subcommands to collide with). | - |
<!-- PROGRESS:END -->
