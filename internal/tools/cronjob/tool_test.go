...
func TestCronjobTool_ErrorsAreJSONStrings(t *testing.T) {
	...
	assertCronjobToolError(t, toolcron.NewCronjobTool(toolcron.CronjobToolConfig{}), map[string]any{"action": "list"}, "cron store disabled")

	disabledRunTool := toolcron.NewCronjobTool(toolcron.CronjobToolConfig{
		Store:             store,
		ScriptsRoot:       t.TempDir(),
		Now:               fixedCronjobToolNow,
...
func execCronjobTool[T any](t *testing.T, tool *toolcron.CronjobTool, args map[string]any) T {
	t.Helper()
...
func execCronjobToolRaw(t *testing.T, tool *toolcron.CronjobTool, args map[string]any) json.RawMessage {
	t.Helper()
...
func assertCronjobToolError(t *testing.T, tool *toolcron.CronjobTool, args map[string]any, want string) {
	t.Helper()
	raw := execCronjobToolRaw(t, tool, args)
	var out struct {
		Success bool   `json:"success"`
