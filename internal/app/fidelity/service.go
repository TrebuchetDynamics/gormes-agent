...
type fidelityHermesReportJSON struct {
	...
	OK     bool                    `json:"ok"`
	Report progressfidelity.Report `json:"report,omitempty"`
...
func runFidelityHermesCommand(cmd *cobra.Command, opts fidelityHermesCommandOptions, options FidelityCommandOptions) error {
	report, err := progressfidelity.GenerateHermesReport(cmd.Context(), progressfidelity.Options{
...
func formatFidelityHermesReport(payload fidelityHermesReportJSON) string {
	...
	fmt.Fprintf(&b, "surfaces: total=%d covered=%d partial=%d planned=%d missing=%d\n",
		...
		payload.Report.Summary.ByStatus[string(progressfidelity.StatusCovered)],
		payload.Report.Summary.ByStatus[string(progressfidelity.StatusPartial)],
		payload.Report.Summary.ByStatus[string(progressfidelity.StatusPlanned)],
		payload.Report.Summary.ByStatus[string(progressfidelity.StatusMissing)],
	)
