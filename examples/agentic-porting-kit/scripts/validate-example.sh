...
example_dir="$repo_root/examples/python-greeter-to-go"
skills_dir="$repo_root/skills"
readme_path="$repo_root/README.md"
license_path="$repo_root/LICENSE"
schema_path="$repo_root/schemas/progress.schema.json"
...
if "cmd/progress" in progress_path.read_text(encoding="utf-8"):
    raise SystemExit("example progress must not depend on product-specific progress tooling")
...
for name in expected_skills:
    ...
    for token in ("docs/content/building-gormes", "cmd/progress", "github.com/TrebuchetDynamics/gormes-agent", local_home, "Gormes", "Hermes"):
        if token in text:
            raise SystemExit(f"{skill_path}: forbidden product-specific token {token!r}")
...
for token in (local_home, "docs/content/building-gormes", "cmd/progress"):
    if token in readme:
        raise SystemExit(f"README fixture contains local-only token: {token}")
for phrase in ("MIT License", "Trebuchet Dynamics", "Permission is hereby granted"):
...
echo "agentic-porting-kit public example: ok"
