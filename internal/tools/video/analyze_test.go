...
// fakeAnalyzeProvider is a test-only provider that echoes back the
// video path and prompt as proof of routing, simulating a vision-capable
// model doing video analysis.
type fakeAnalyzeProvider struct {
	available   bool
	visionCapab bool
...
func (p *fakeAnalyzeProvider) Available(_ context.Context) bool { return p.available }
func (p *fakeAnalyzeProvider) SupportsVision(_ context.Context) bool {
	return p.visionCapab
}
func (p *fakeAnalyzeProvider) Analyze(_ context.Context, path, prompt string) (AnalyzeResult, error) {
	return p.result, p.err
}

func TestVideoAnalyze_RoutesToVisionCapableProvider(t *testing.T) {
	provider := &fakeAnalyzeProvider{
		available:   true,
		visionCapab: true,
		result: AnalyzeResult{
			Success:  true,
			Evidence: AnalyzeStatusOK,
			Analysis: "fake analysis: the video shows a cat",
			Provider: "fake-vision",
		},
	}
	tool := NewAnalyzeTool(AnalyzeConfig{
		ProviderFactory: func(_ context.Context) AnalyzeProvider { return provider },
	})
...
func TestVideoAnalyze_NonVisionProviderUnsupportedError(t *testing.T) {
	...
	if result.Evidence != AnalyzeStatusUnsupportedVideo {
		t.Errorf("evidence = %q, want %q", result.Evidence, AnalyzeStatusUnsupportedVideo)
	}
...
func TestVideoAnalyze_LocalPathSanitization(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			path: "../../../etc/passwd",
			want: string(AnalyzeStatusWorkspaceViolation),
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.success && !result.Success {
				if result.Evidence == AnalyzeStatusInvalidScheme {
...
func TestVideoAnalyze_MissingRequiredArgs(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.Evidence != AnalyzeStatusInvalidArgs {
				t.Errorf("evidence = %q, want %q", result.Evidence, AnalyzeStatusInvalidArgs)
			}
		})
	}
}
