...
func sanitizeANSI(s string, keepSGR bool) string {
	var out strings.Builder
	out.Grow(len(s))
	for i := 0; i < len(s); {
		b := s[i]
		if b == ansiESC {
			next, keepStart, keepEnd := consumeANSI(s, i)
...
func consumeANSI(s string, start int) (next int, keepStart int, keepEnd int) {
	keepStart = -1
	if start+1 >= len(s) {
