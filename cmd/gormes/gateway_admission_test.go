...
func TestGatewayStartupAllowlistWarning(t *testing.T) {
	...
	report := evaluateGatewayStartupSecurity(cfg, emptyGatewaySecurityEnv)
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			report := evaluateGatewayStartupSecurity(cfg, mapGatewaySecurityEnv(tc.env))
			if gatewayStartupSecurityHasCode(report, "gateway_allowlist_missing") {
...
func gatewayStartupSecurityHasCode(report gatewayStartupSecurityReport, code string) bool {
	return countGatewayStartupSecurityCode(report, code) > 0
}

func countGatewayStartupSecurityCode(report gatewayStartupSecurityReport, code string) int {
	count := 0
	for _, evidence := range report.Evidence {
