...
func TestHermesCLIParityManifestCompletionImplemented(t *testing.T) {
	entry := requireShellCompletionHermesCLIEntry(t, []string{"completion"})
...
func newCompletionTestRoot(t *testing.T) *cobra.Command {
	...
	factories["completion"] = NewShellCompletionCommand
	return NewRootCommand(RootOptions{Version: Version}, factories)
}

func requireShellCompletionHermesCLIEntry(t *testing.T, path []string) HermesCLIParityEntry {
	t.Helper()
	want := strings.Join(path, " ")
	for _, entry := range HermesCLIParityManifest() {
		if strings.Join(entry.Path, " ") == want {
			return entry
		}
	}
	t.Fatalf("Hermes CLI manifest missing path %q", want)
