...
| 5 / 5.Q | Native TUI Shift+Enter newline CSI-u parity | Native TUI Hermes input keybinding semantics | Native TUI Hermes input keybinding semantics is complete, so the resolver and textarea submit path already distinguish newline chords from plain Enter., Bubble Tea v1.3.10 remains the local TUI input source and does not expose a KeyShiftEnter constant for CSI-u / modifyOtherKeys Enter sequences., The worker can test the normalization with hermetic byte-slice messages and pure resolver fixtures; no live terminal is required. | - |
