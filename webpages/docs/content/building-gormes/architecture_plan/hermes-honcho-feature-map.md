...
| Navivox SSH/Flutter channel | Gormes-owned Flutter/Navivox planning docs under `flutter-navivox/`; Hermes gateway principles from `gateway/run.py` and `gateway/platforms/base.py` | `cmd/gormes`, `internal/channels/navivox`, Flutter app under `flutter-navivox/app` | Treat Navivox as a Gormes-owned first-party channel over SSH stdio, not a Hermes platform parity port. Build it in slices: binary frame codec and stdio control-plane server, selected-agent event routing, chat submit/update/final gateway binding, config admin, typed tool events/artifacts, and voice frames. The first row must stop at `hello`, `server.status`, `ping`, `pong`, and `error` with fake stdio tests. | Phase 2.B.12 `Navivox stdio protocol control-plane tracer` |
