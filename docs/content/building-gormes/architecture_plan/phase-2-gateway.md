...
| Phase 2.F.3 — Restart / Pairing / Status | 🔨 in progress | P2 | Graceful shutdown drain and the adapter startup failure cleanup contract (`gateway.StartupCloser` + `Manager.safeAdapterDisconnect`) are landed in `internal/gateway`; next slices are active-turn follow-up/late-arrival drain policy, drain-timeout resume recovery, pairing persistence, pairing approval/rate-limit semantics, a read-only status command, runtime-status JSON/PID validation, token-scoped credential locks, `/restart` takeover markers, then channel lifecycle writers |
