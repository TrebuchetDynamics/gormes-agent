...
func TestConfigGet_JSONNonSecretEmitsStructuredValue(t *testing.T) {
	...
	if _, _, err := executeConfigCommandForTest(cmd, "set", "hermes.model", "test-model-xyz"); err != nil {
		t.Fatalf("config set: %v", err)
	}

	cmd = newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "get", "hermes.model", "--json")
	if err != nil {
...
func TestConfigGet_JSONSecretKeyStaysRedacted(t *testing.T) {
	...
	cmd := newConfigCommandForTest()
	if _, _, err := executeConfigCommandForTest(cmd, "set", "hermes.api_key", "sk-LEAK-DETECTOR-do-not-show"); err != nil {
		t.Fatalf("config set api_key: %v", err)
	}

	cmd = newConfigCommandForTest()
	stdout, stderr, err := executeConfigCommandForTest(cmd, "get", "hermes.api_key", "--json")
	if err != nil {
