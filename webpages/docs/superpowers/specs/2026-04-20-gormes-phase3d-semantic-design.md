...
- [`docs/ARCH_PLAN.md`](../../ARCH_PLAN.md) — Phase 3 memory trilogy. 3.A = Lattice (SQLite + FTS5). 3.B = Brain (LLM extractor). 3.C = Recall (lexical + FTS5 seed → CTE walk → fence injection). 3.D = **Semantic Fusion** (embeddings + hybrid seed selection).
