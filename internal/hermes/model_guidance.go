...
func ModelPromptRole(model string) PromptRole {
	if matchModelFamily(model, DeveloperRoleModels) {
...
func BuildModelPromptGuidance(opts ModelPromptGuidanceOptions) ModelPromptGuidanceResult {
	...
	if applyToolUse {
		if len(opts.ValidToolNames) == 0 {
			...
			blocks = append(blocks, ToolUseEnforcementGuidance)
		}
	}

	if matchModelFamily(opts.Model, []string{"gpt", "codex"}) {
		blocks = append(blocks, OpenAIModelExecutionGuidance)
	}
...
func shouldApplyToolUseEnforcement(model string, mode any) (bool, bool) {
	switch v := mode.(type) {
	case nil:
		return matchModelFamily(model, ToolUseEnforcementModels), false
	case bool:
		return v, false
	case string:
	...
	case []any:
		...
		for _, item := range v {
			s, ok := item.(string)
			if !ok {
				return matchModelFamily(model, ToolUseEnforcementModels), true
			}
