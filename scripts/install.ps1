...
$Script:GormesGoVersion   = if ($env:GORMES_GO_VERSION) { $env:GORMES_GO_VERSION } else { '1.26.4' }
...
function Test-GoVersionSupported([string]$Version) {
    ...
    return ($Version -match '^go1\.26\.([4-9]|[1-9][0-9])') -or ($Version -match '^go1\.(2[7-9]|[3-9][0-9])') -or ($Version -match '^go[2-9]')
}
...
function Ensure-Go {
    ...
    if (-not (Test-GoVersionSupported $version)) {
        Stop-GormesWithError "Go 1.26.4+ required; found $version"
    }
}
