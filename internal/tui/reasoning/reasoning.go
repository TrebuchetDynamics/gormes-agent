// Package thinking renders Hermes-compatible thinking display and tool trails.
//
...
// Styles is the subset of root TUI skin styles used by thinking renderers.
type Styles struct {
...
func RenderThinking(state ThinkingState) string {
	return renderThinkingWithStyles(state, Styles{}, false)
}

func RenderThinkingWithStyles(state ThinkingState, styles Styles) string {
	return renderThinkingWithStyles(state, styles, true)
}

func renderThinkingWithStyles(state ThinkingState, styles Styles, styled bool) string {
	if !state.Visible {
		return ""
	}
...
// TreeRails exposes the pure rail builder for root compatibility tests.
func TreeRails(depth int) string { return treeRails(depth) }

// TreeBranch exposes the pure branch marker builder for root compatibility tests.
func TreeBranch(isLast bool) string { return treeBranch(isLast) }
...
// ToolIcon exposes the pure icon selector for root compatibility tests.
func ToolIcon(name string) string { return toolIcon(name) }

// StatusIcon exposes the pure status selector for root compatibility tests.
func StatusIcon(status ToolCallStatus) string { return statusIcon(status) }
...
// FormatDuration exposes the pure duration formatter for root compatibility tests.
...
func RenderToolTrail(nodes []ToolCallNode) string {
	return renderToolTrailWithStyles(nodes, Styles{}, false)
}

func RenderToolTrailWithStyles(nodes []ToolCallNode, styles Styles) string {
	return renderToolTrailWithStyles(nodes, styles, true)
}

func renderToolTrailWithStyles(nodes []ToolCallNode, styles Styles, styled bool) string {
	if len(nodes) == 0 {
		return ""
	}
...
func renderToolNodes(b *strings.Builder, nodes []ToolCallNode, parentIsLast bool, styles Styles, styled bool) {
