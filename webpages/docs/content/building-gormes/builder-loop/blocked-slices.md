...
|---|---|---|---|---|
| 5 / 5.E | Pure-Go Whisper transcribe one WAV | whisper.cpp WASI module discovery, Whisper tiny.en model cache fetcher | Row B 'whisper.cpp WASI module discovery' is complete; whisper.wasm loads via wazero., `Whisper tiny.en model cache fetcher` is complete; tests can populate ggml-tiny.en.bin from a pinned URL, size, and SHA-256 without committing the 75 MB model to the repository or embedding it in the runtime binary. | Wire Pure-Go Whisper into Telegram resolver, Whisper benchmark harness + perf budget |
