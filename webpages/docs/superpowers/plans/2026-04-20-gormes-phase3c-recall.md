...
**Module path:** `github.com/TrebuchetDynamics/gormes-agent`

**Spec:** [`docs/superpowers/specs/2026-04-20-gormes-phase3c-recall-design.md`](../specs/2026-04-20-gormes-phase3c-recall-design.md) (approved at `67aba2a8`)
...
|---|---|---|
| `internal/memory/validator.go` | Modify | Weight floor patch: `w < 0` → `w <= 0` |
| `internal/memory/validator_test.go` | Modify | Append `TestValidate_WeightZeroPromotedToOne` |
| `internal/memory/schema.go` | Modify | Bump `schemaVersion = "3c"`, add `migration3bTo3c` constant |
| `internal/memory/migrate.go` | Modify | Extend switch to run `3b → 3c` step |
| `internal/memory/migrate_test.go` | Modify | Append `TestMigrate_3bTo3c` + `TestMigrate_ChatIDBackfills` |
| `internal/memory/worker.go` | Modify | `turnPayload` gains `ChatID` field; `handleCommand` INSERT includes chat_id |
| `internal/kernel/kernel.go` | Modify | Kernel payload INSERT adds chat_id; add `Config.ChatKey` |
| `internal/memory/recall_format.go` | Create | `extractCandidates`, `sanitizeFenceContent`, `formatContextBlock` pure funcs |
| `internal/memory/recall_format_test.go` | Create | 5 unit tests covering the three pure functions |
| `internal/memory/recall_sql.go` | Create | `seedsExactName`, `seedsFTS5`, `traverseNeighborhood`, `enumerateRelationships` SQL helpers |
| `internal/memory/recall_sql_test.go` | Create | ~8 SQL-level tests against a real tempdir DB |
| `internal/memory/recall.go` | Create | `Provider` struct + `NewRecall` + `GetContext` orchestrator |
| `internal/memory/recall_test.go` | Create | End-to-end provider tests (orchestrator-level) |
| `internal/kernel/recall.go` | Create | `RecallProvider` interface + `RecallParams` struct |
| `internal/kernel/kernel.go` | Modify | Add `Config.Recall`, `Config.RecallDeadline`; injection block at line ~214 |
| `internal/kernel/recall_test.go` | Create | `TestKernel_InjectsMemoryContextWhenRecallNonNil` + `TestKernel_NoRecallWhenProviderNil` + `TestKernel_RecallTimeoutFallsThrough` |
| `internal/config/config.go` | Modify | Add `RecallEnabled`, `RecallWeightThreshold`, `RecallMaxFacts`, `RecallDepth` to `TelegramCfg` |
| `internal/config/config_test.go` | Modify | Append `TestLoad_RecallDefaults` |
| `gormes/cmd/gormes/telegram.go` | Modify | Construct `memory.NewRecall`, pass into `kernel.Config.Recall` + set `Config.ChatKey` |
| `internal/memory/recall_integration_test.go` | Create | Ollama-backed end-to-end: extract → recall → assert fence contains entities |
...
**Files:**
- Modify: `internal/memory/validator.go`
- Modify: `internal/memory/validator_test.go`

- [ ] **Step 1: Write the failing test**

Append to `internal/memory/validator_test.go`:
...
In `internal/memory/validator.go`, find:
...
```bash
cd ..
git add internal/memory/validator.go internal/memory/validator_test.go
...
- Modify: `internal/memory/worker.go`
- Modify: `internal/kernel/kernel.go`
- Create: `internal/kernel/chat_key_test.go` (a tiny kernel test proving the payload change)
...
In `internal/memory/migrate.go`, the `switch v` block currently has `case "3a"` (runs 3a→3b) and `case "3b"` (no-op target). Change it to:
...
In `internal/memory/worker.go`, modify the `turnPayload` struct:
...
In `internal/kernel/kernel.go`, extend `Config`:
...
Create `internal/kernel/chat_key_test.go`:
...
git add \
  ...
  internal/kernel/chat_key_test.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `internal/memory/recall_format.go`
- Create: `internal/memory/recall_format_test.go`
...
```bash
cd ..
git add internal/memory/recall_format.go internal/memory/recall_format_test.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `internal/memory/recall_sql.go`
- Create: `internal/memory/recall_sql_test.go`
...
```bash
cd ..
git add internal/memory/recall_sql.go internal/memory/recall_sql_test.go
git commit -m "$(cat <<'EOF'
...
Append to `internal/memory/recall_sql_test.go`:
...
Append to `internal/memory/recall_sql.go`:
...
**Files:**
- Create: `internal/kernel/recall.go`
- Modify: `internal/kernel/kernel.go`
- Create: `internal/kernel/recall_test.go`
...
```bash
cd ..
git add internal/kernel/recall.go internal/kernel/kernel.go internal/kernel/recall_test.go
git commit -m "$(cat <<'EOF'
...
```bash
cd ..
git add internal/config/config.go internal/config/config_test.go gormes/cmd/gormes/telegram.go
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `internal/memory/recall_integration_test.go`
...
```bash
cd ..
git add internal/memory/recall_integration_test.go
git commit -m "$(cat <<'EOF'
