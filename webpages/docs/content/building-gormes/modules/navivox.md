---
title: "Navivox Module Roadmap"
---

# Navivox Module Roadmap

Generated from the single logical backlog. This page is a scoped review view; `progress.json` remains canonical.

**Module:** `navivox`
**Rows:** 22
**Status counts:** `complete`: 14 · `in_progress`: 0 · `planned`: 8
**Priority counts:** `P0`: 2 · `P1`: 12 · `P2`: 1 · `P3`: 1 · `unset`: 6

## Phase 2 — The Gateway

### 2.B.12 — Channel-Neutral Native Runtime Adapter

| Status | Priority | Module | Row |
|---|---|---|---|
| `complete` | `P1` | `navivox` | Navivox stdio protocol control-plane tracer |
| `complete` | `P1` | `navivox` | Navivox QR pairing descriptor CLI |
| `complete` | `P1` | `navivox` | Navivox Flutter voice morph surface |

## Phase 5 — The Final Purge

### 5.N — Misc Operator Tools

| Status | Priority | Module | Row |
|---|---|---|---|
| `complete` | `P1` | `navivox` | Navivox host setup apply with transient sudo |

### 5.O — Hermes CLI Parity

| Status | Priority | Module | Row |
|---|---|---|---|
| `planned` | `P2` | `navivox` | gormes setup profiles: per-profile workspaces + channels + navivox-default (Gormes-owned) |
| `planned` | `P3` | `navivox` | gormes setup profiles — all profiles navivox-accessible by default |

## Phase 9 — Design & Security Hardening

### 9.E — Navivox HTTP-only Hardening

| Status | Priority | Module | Row |
|---|---|---|---|
| `complete` | `unset` | `navivox` | Remove SSH Navivox stdio path |
| `complete` | `unset` | `navivox` | Remove Flutter Navivox fake-server mode and wire protocol |
| `complete` | `unset` | `navivox` | Remove Flutter SSH keys feature |
| `complete` | `unset` | `navivox` | Navivox VPN host enumeration helper |
| `complete` | `unset` | `navivox` | Navivox HTTP gateway mandatory-VPN bind |
| `complete` | `unset` | `navivox` | Navivox HTTP gateway connect-info command |

### 9.F — Navivox Operator Activation

| Status | Priority | Module | Row |
|---|---|---|---|
| `complete` | `P0` | `navivox` | Navivox HTTP/WS documentation refresh |
| `complete` | `P0` | `navivox` | Navivox connect-and-talk first screen |
| `complete` | `P1` | `navivox` | Navivox profile contact summary API |
| `complete` | `P1` | `navivox` | Navivox continuous voice command mode |
| `planned` | `P1` | `navivox` | Navivox Telegram-inspired chat polish |
| `planned` | `P1` | `navivox` | Navivox natural-language profile seed flow |
| `planned` | `P1` | `navivox` | Navivox structured tool event cards |
| `planned` | `P1` | `navivox` | Navivox safe config admin over HTTP |
| `planned` | `P1` | `navivox` | Navivox voice run records |
| `planned` | `P1` | `navivox` | Navivox per-profile BYO voice profiles |
