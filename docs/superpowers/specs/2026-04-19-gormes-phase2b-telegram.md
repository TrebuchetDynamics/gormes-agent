...
Enforced via `internal/buildisolation_test.go` (a Go test that runs `go list` through `os/exec` and fails the suite if `telegram-bot-api` appears anywhere in the TUI's transitive dep set).
