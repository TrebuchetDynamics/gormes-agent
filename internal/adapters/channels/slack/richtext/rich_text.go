...
const UnavailableCode = "slack_rich_text_unavailable"

type Block map[string]any

type AttachmentPreview map[string]any
...
func AugmentInboundText(text string, blocks []Block, attachments []AttachmentPreview) (string, []Evidence) {
	...
	for _, section := range renderAttachmentPreviews(attachments, &evidence) {
...
func renderSlackRichTextBlocks(blocks []Block, evidence *[]Evidence) []string {
	lines := []string{}
...
func walkRichTextElements(elements []Block, quoteDepth int, bullet string, evidence *[]Evidence, lines *[]string) {
	for _, elem := range elements {
		...
		case "rich_text_list":
			...
			for i, child := range children {
				...
				walkRichTextElements([]Block{child}, quoteDepth, itemBullet, evidence, lines)
			}
		case "rich_text_preformatted":
			...
			for _, child := range children {
				...
				if rendered := renderInlineElements([]Block{child}, evidence); strings.TrimSpace(rendered) != "" {
					codeLines = append(codeLines, rendered)
				}
			}
			if len(codeLines) == 0 {
		...
		default:
			appendRenderedLine(lines, renderInlineElements([]Block{elem}, evidence), quoteDepth, bullet)
		}
	}
}

func renderInlineElements(elements []Block, evidence *[]Evidence) string {
	pieces := []string{}
	for _, elem := range elements {
		elemType, ok := optionalString(elem, "type", "blocks", evidence)
		if !ok {
...
func renderAttachmentPreviews(attachments []AttachmentPreview, evidence *[]Evidence) []string {
	sections := []string{}
...
func blockElements(block Block, key, source string, evidence *[]Evidence) ([]Block, bool) {
	value, ok := block[key]
	if !ok || value == nil {
		return nil, true
	}
...
func blockList(value any, source string, evidence *[]Evidence) ([]Block, bool) {
	switch typed := value.(type) {
	case []Block:
		return typed, true
	case []map[string]any:
		out := make([]Block, 0, len(typed))
...
func asBlock(value any) (Block, bool) {
	switch typed := value.(type) {
	case Block:
		return typed, true
	case map[string]any:
		return Block(typed), true
	default:
		return nil, false
	}
}

func optionalString(values map[string]any, key, source string, evidence *[]Evidence) (string, bool) {
	value, ok := values[key]
	if !ok || value == nil {
		return "", true
	}
...
func optionalBool(values map[string]any, key, source string, evidence *[]Evidence) (bool, bool) {
	value, ok := values[key]
	if !ok || value == nil {
		return false, true
	}
...
func addUnavailable(evidence *[]Evidence, source, reason string) {
