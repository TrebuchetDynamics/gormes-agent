...
const OpenRouterBaseURL = openRouterBaseURL
...
var newBuildProvenance = func() gormescli.BuildProvenance { return gormescli.BuildProvenance{} }
...
func NormalizeAuthProvider(provider string) string { return normalizeAuthProvider(provider) }

func AuthProviderDefaultsToOAuth(provider string) bool { return authProviderDefaultsToOAuth(provider) }

func ProviderBaseURL(provider, override string) string { return providerBaseURL(provider, override) }

func RunAuthLogoutCommand(cmd *cobra.Command, providerInput string) error {
	return runAuthLogoutCommand(cmd, providerInput)
}

type CodexOAuthLoginRequest = codexOAuthLoginRequest

func SetCodexOAuthLoginForTest(login func(context.Context, CodexOAuthLoginRequest) (config.CodexOAuthTokens, error)) func() {
	prev := authCodexOAuthLogin
	authCodexOAuthLogin = login
	return func() { authCodexOAuthLogin = prev }
}
