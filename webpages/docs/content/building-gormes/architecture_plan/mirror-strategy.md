...
weight: 90
aliases:
  - /building-gormes/architecture_plan/mirror-strategy/
---
...
Gormes now has local rollup primitives (`internal/automation/insights/rollup.go`), `telemetry.Snapshot` bridging, and append-only JSONL persistence for daily usage records. Operators can derive usage locally and keep a durable daily audit trail without waiting for the broader Phase 4 InsightsEngine port.
