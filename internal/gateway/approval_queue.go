...
func (q *GatewayApprovalQueue) ResolveGatewayApproval(_ context.Context, res ApprovalResolution) error {
	sessionKey := strings.TrimSpace(res.SessionKey)
	if sessionKey == "" {
		return ErrGatewayApprovalEmptySession
	}
	if !validApprovalChoice(res.Choice) {
...
func (q *GatewayApprovalQueue) ResolveAllGatewayApprovals(sessionKey string, choice ApprovalChoice) (int, error) {
	sessionKey = strings.TrimSpace(sessionKey)
	if sessionKey == "" {
	...
	for _, entry := range entries {
		q.recordOutcomeLocked(entry, ApprovalResolution{SessionKey: sessionKey, Choice: choice}, false)
	}
...
func (q *GatewayApprovalQueue) ClearGatewayApprovalSession(sessionKey string) int {
	...
	delete(q.queues, sessionKey)
	for _, entry := range entries {
		q.recordOutcomeLocked(entry, ApprovalResolution{SessionKey: sessionKey, Choice: ApprovalChoiceDeny}, true)
	}
	return len(entries)
}
...
func (q *GatewayApprovalQueue) recordOutcomeLocked(entry *gatewayApprovalEntry, res ApprovalResolution, canceled bool) {
	choice := res.Choice
	if choice == "" {
	...
	q.outcomes[entry.ticket.ID] = GatewayApprovalOutcome{
		...
		Resolution: cloneApprovalResolution(res),
		Choice:     choice,
		Canceled:   canceled,
	}
}

func validApprovalChoice(choice ApprovalChoice) bool {
	switch choice {
	case ApprovalChoiceOnce, ApprovalChoiceSession, ApprovalChoiceAlways, ApprovalChoiceDeny:
		return true
	default:
		return false
	}
...
func cloneApprovalResolution(res ApprovalResolution) ApprovalResolution {
...
func cloneGatewayApprovalOutcome(outcome GatewayApprovalOutcome) GatewayApprovalOutcome {
	...
	outcome.Resolution = cloneApprovalResolution(outcome.Resolution)
	return outcome
}
