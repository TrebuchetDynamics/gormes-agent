...
func NormalizeDiscordLimit(action string, arguments map[string]any) DiscordLimitNormalization {
	...
	if limit < LimitMinimum {
		return DiscordLimitNormalization{Limit: LimitMinimum, Evidence: DiscordLimitEvidenceClamped}
	}
	if limit > LimitMaximum {
		return DiscordLimitNormalization{Limit: LimitMaximum, Evidence: DiscordLimitEvidenceClamped}
	}
